...
この引数には、 LuneScript が自動生成している test_ext_glue.c によって
Lua のスタックから値を取り出され、値が設定される。

具体的には、上記サンプル test_ext_glue.c で定義している
~lns_glue_add( lua_State * pLua )~ 関数内の
~val = luaL_checkinteger( pLua, 2 );~ がその処理である。

Lua の glue では、関数引数を得るため API をコールする必要があるが、
LuneScript で glue を生成した場合は、
glue コード内で関数引数を得る API が実行されるため、
ユーザが関数引数を得る API をコールする必要はない。


ただし、LuneScript 側で定義しているメソッドの引数の型が
int(int!), real(real!), str(str!) 以外の型だった場合、
...
具体的には lua_push 系の関数呼び出しと、
return による戻り値の個数指定である。

glue 生成後、hoge_create() と hoge_add() を別途定義することで、
test.ext モジュールが完成する。
...
ここで lns_glue_new_test_ext() は、 test.ext クラスのインスタンスを生成する関数であり、
test_ext_glue.c に生成される。

この関数は、指定サイズのデータを test.ext クラスのインスタンスとして生成し、
アドレスを返す。
上記例では、glue_t を test.ext クラスのインスタンスとして生成している。

lns_glue_get_test_ext() は、
引数で指定したインデックスの VM スタック上にある
test.ext クラスのインスタンスアドレスを取得する。

上記の例では、 add メソッドの第一引数に格納されている test.ext クラスのインスタンス
glue_t のアドレスを取得している。
