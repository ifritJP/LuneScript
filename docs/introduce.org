...
この func 引数に関数オブジェクト ~fn () { print( "hoge" );~ を与えており、
test() 関数内で func() が実行されています。
...
exp には nilable な式を指定します。
全ての exp が nil でなかった場合、 block を実行します。
いずれかの exp が nil だった場合、 elseblock を実行します。
...
when! は、unwrap 処理による条件分岐です。

#+BEGIN_SRC lns
if! var[,var,...] block [ else elseblock ];
#+END_SRC

var には変数を指定します。
全ての var が nil でなかった場合、 block を実行します。
いずれかの var が nil だった場合、 elseblock を実行します。

block 内の処理では var は unwrap された型となります。
また、block 内では var に値を代入することは出来ません。
...
os.exit() など、その関数をコールすると処理を戻さない関数があります。

そのような関数を作成する場合、
戻り値の型として次のように ~__~ を宣言する必要があります。
...
なお、 戻り値の型に ~__~ を指定した場合は、
return などで関数の処理を抜けることは出来ません。
...
abstract クラスは、そのクラス単独では new できない。
必ず継承する必要がある。
...
,,,, は、直後に続く式を評価して得られた *シンボル* を *文字列に変換* する演算子である。
,,, は、直後に続く式を評価して得られた *文字列* を *シンボルに変換* する演算子である。

~`{}~ は、 ~`{}~  内で書いたステートメントを、そのままの値とすることが出来る。
macro-statement 内で ~`{}~ で書いたステートメントは、
expand-statement で展開することができる。
~`{}~  内では変数の参照や関数の実行を書いても、
macro-statement 内では評価されない。
expand-statement で展開時に評価される。

,, は、直後に続く *式* を評価する演算子である。
,,、 ,,,、 ,,,,、 は、 macro-statement の ~`{}~  内で利用することで、
式を評価することが出来る。

macro-statement で ,, を利用すると、直後の式を評価するが、
expand-statement では、式の評価ではなく、変数の展開に限定される。
...
この関数は、指定の txt の lunescript コードをロードし、
ロード結果を返すものである。
...
_test() マクロに ~pub let val = 100;~ を与えており、
これを _lnsLoad() でロードすることで、その val に 100 が格納されたモジュールが返される。
そして、そのモジュールの val を、変数 val に代入し、
expand-statement で print() している。

なお、 _lnsLoad() の第一引数の name は、
ロードする lunescript コードの名前を指定する。
