...
1 ファイルの肥大化を防止するため、
ファイルを分割してサブモジュール化できる。

サブモジュールを利用するには、次のように宣言する。

#+BEGIN_SRC lns
submodule use foo.bar.module;
#+END_SRC

また、サブモジュール側では次のように宣言する。

#+BEGIN_SRC lns
submodule owner foo.bar.mainmodule;
#+END_SRC

これにより、
指定したモジュール(上記の例では foo.bar.module)をサブモジュールとして扱う。

モジュールの宣言は、スクリプトの先頭で行なう必要がある。
ただし、コメントは除く。


C の #include と基本的には同じと考えて良い。
ただし、次の点で #include と異なる。

- #include は、宣言した箇所にファイルを展開するが、 
  LuneScript ではファイルの最終位置に展開する。
- サブモジュールは、 statement として完結している必要がある。
  - #include のような、式の一部をサブモジュール内には書けない。
- サブモジュールとメインモジュールは 1 対 1 で対応していなければならない。
- サブモジュールから、さらにサブモジュールを利用することはできない。
