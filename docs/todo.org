...
- [ ] __func__ が nil になることがある。
- [ ] map の型宣言で、 value の型宣言しなくてもエラーにならない。 Map<int> ができてしまう。
- [ ] table.unpack を list, array のメソッドに変更する
- [ ] getter を省略して setter だけ生成できるように
...
- [ ] 戻り値を持たない関数の結果を、変数に代入できてしまう。
- [ ] foreach の変数名が shadow チェック対象になっていない。
- [ ] 型固定の可変長引数対応
...
- [ ] 配列の繰り返し要素指定定義。
- [ ] lua5.1 の対応
      - [ ] package.searchpath() が 5.1 はない。
      - [ ] string.format の引数が bool の場合、自動で and "true" or "false" を付加する。
