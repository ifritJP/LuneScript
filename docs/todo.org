...
- [ ] コンストラクタ内で、初期化していないメンバにアクセスしてもエラー検知しない。
...
- [ ] 公開マクロ内では __var のアクセスを禁止にする。
  
- [ ] __noasyncLock 内から __luago せずに、 luaval 系にアクセスできてしまう。
...
- [ ] go で list.sort のコールバックがビルドエラーする。
      ※ _env が付かない。
...
- [ ] go 版 lnsc で lnsc test/imp.lns exe が出来ない。
      動的な import が処理できていない。  
- [ ] __async のメソッドから __noasync メソッドをコールした場合、エラーにならない。
- [ ] __async から mutable な static メンバ にアクセスした場合、エラーにする。
