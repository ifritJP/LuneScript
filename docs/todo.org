...
- [ ] 本来、次は Pub でない Hoge を含んだ func が pub なので、エラーになるはず。
...
- [ ] company-mode で run-at-time のリトライ処理が走る時、
      バッファが変っているとエラー。
  
- [ ] クラス内のメソッドから、外部変数にクロージャアクセスできてしまう。  
...
- [ ] stem と any の分離
  ...
  - [ ] int 系の closure を any で扱っているが、これを val にして、
        val に stem を保持する。
  - [ ] nil を any で扱っているが、これを stem で扱うにする。
  - [ ] any から int 系を削除する。
  - [ ] enum の値のオブジェクトをグローバルで宣言しない。
