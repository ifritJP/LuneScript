...
function dumpFilter:processExpCall( node, opt )
   ...
   self:dump( opt, node, string.format( "[nilacc:%s] -> %s", tostring( node:get_nilAccess()), getTypeListTxt( node:get_expTypeList() )) )
...
function dumpFilter:processAlias( node, opt )
   ...
   self:dump( opt, node, string.format( "%s = %s", tostring( node:get_newSymbol()), node:get_typeInfo():getTxt(  )) )
end
...
function dumpFilter:processLiteralChar( node, opt )
   ...
   self:dump( opt, node, string.format( "%s(%s)", tostring( node:get_num(  )), node:get_token(  ).txt ) )
end
