...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5327, existSym = parentScope:addClass( modName, nil, typeInfo )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _6072, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
...
function TransUnit:processImport( modulePath )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _6296, metaTypeId2TypeInfoMap = self:processImport( dependName )
   ...
   for __index, _6310 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   for __index, _6366 in ipairs( nameList ) do
      self:popModule(  )
   end
