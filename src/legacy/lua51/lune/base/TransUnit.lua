...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5749, existSym = parentScope:addClass( self.processInfo, modName, nil, typeInfo )
...
function TransUnit:pushClass( errPos, classFlag, abstractFlag, baseInfo, interfaceList, genTypeList, externalFlag, name, allowMultiple, accessMode, defNamespace )
   ...
   if typeInfo ~= Ast.headTypeInfo then
      ...
      typeInfo = self.processInfo:createClass( classFlag, abstractFlag, scope, baseInfo, interfaceList, workGenTypeList, parentInfo, externalFlag, accessMode, name )
      
      parentScope:addClass( self.processInfo, name, errPos, typeInfo )
   end
...
function ImportParam.new( pos, transUnit, processInfo, typeId2Scope, typeId2TypeInfo, importedAliasMap, metaInfo, scope, moduleTypeInfo, scopeAccess, typeId2AtomMap )
   local obj = {}
   ImportParam.setmeta( obj )
   if obj.__init then
      obj:__init( pos, transUnit, processInfo, typeId2Scope, typeId2TypeInfo, importedAliasMap, metaInfo, scope, moduleTypeInfo, scopeAccess, typeId2AtomMap )
   end
   return obj
end
function ImportParam:__init( pos, transUnit, processInfo, typeId2Scope, typeId2TypeInfo, importedAliasMap, metaInfo, scope, moduleTypeInfo, scopeAccess, typeId2AtomMap )
   ...
   if self.parentId ~= Ast.rootTypeId or not Ast.getBuiltInTypeIdMap(  )[self.typeId] or self.kind == Ast.TypeInfoKind.List or self.kind == Ast.TypeInfoKind.Array or self.kind == Ast.TypeInfoKind.Map or self.kind == Ast.TypeInfoKind.Set then
      ...
      if newTypeInfo and (self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF ) then
         ...
         if self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF then
            Log.log( Log.Level.Debug, __func__, 1638, function (  )
            ...
            local workTypeInfo = Ast.NormalTypeInfo.create( param.processInfo, accessMode, self.abstractFlag, scope, baseInfo, interfaceList, parentInfo, self.staticFlag, typeInfoKind, self.txt, itemTypeInfo, argTypeInfo, retTypeInfo, self.mutMode )
            newTypeInfo = workTypeInfo
            ...
            do
               ...
               if _switchExp == Ast.TypeInfoKind.Func or _switchExp == Ast.TypeInfoKind.Method or _switchExp == Ast.TypeInfoKind.Macro or _switchExp == Ast.TypeInfoKind.Form or _switchExp == Ast.TypeInfoKind.FormFunc then
                  ...
                  workParentScope:add( param.processInfo, symbolKind, false, self.kind == Ast.TypeInfoKind.Func, self.txt, nil, workTypeInfo, accessMode, self.staticFlag, Ast.MutMode.IMut, true )
                  param.typeId2Scope[self.typeId] = scope
               end
            end
...
function _TypeInfoNormal.new( parentId, abstractFlag, baseId, txt, staticFlag, accessMode, kind, mutMode, ifList, itemTypeId, argTypeId, retTypeId, children )
   local obj = {}
   _TypeInfoNormal.setmeta( obj )
   if obj.__init then
      obj:__init( parentId, abstractFlag, baseId, txt, staticFlag, accessMode, kind, mutMode, ifList, itemTypeId, argTypeId, retTypeId, children )
   end
   return obj
end
function _TypeInfoNormal:__init( parentId, abstractFlag, baseId, txt, staticFlag, accessMode, kind, mutMode, ifList, itemTypeId, argTypeId, retTypeId, children )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _6882, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
   ...
   local function processField( name, fieldName, info, parentInfo )
      ...
      if self.targetLuaVer:isSupport( string.format( "%s.%s", name, fieldName) ) then
         if _lune.nilacc( info['type'], nil, 'item', 1) == "var" then
            local symbol = _lune.unwrap( self.scope:add( self.processInfo, Ast.SymbolKind.Var, false, true, fieldName, nil, getTypeInfo( _lune.unwrap( _lune.nilacc( info['typeInfo'], nil, 'item', 1)) ), Ast.AccessMode.Pub, true, Ast.MutMode.Mut, true ))
            ...
            local typeInfo = self.processInfo:createFunc( false, true, self.scope, kind, parentInfo, false, true, staticFlag, Ast.AccessMode.Pub, fieldName, nil, argTypeList, retTypeList, mutable )
            ...
            local symInfo = _lune.unwrap( self.scope:add( self.processInfo, symbolKind, false, kind == Ast.TypeInfoKind.Func, fieldName, nil, typeInfo, Ast.AccessMode.Pub, staticFlag, mutable and Ast.MutMode.Mut or Ast.MutMode.IMut, true ))
         ...
         if name ~= "" then
            ...
            do
               ...
               elseif _switchExp == DeclClassMode.Module then
                  ...
                  self.scope:get_parent():add( self.processInfo, Ast.SymbolKind.Typ, false, false, name, nil, parentInfo, Ast.AccessMode.Local, true, Ast.MutMode.Mut, true )
               end
            end
...
function TransUnit:processImport( modulePath, depth )
   ...
   Log.log( Log.Level.Info, __func__, 2950, function (  )
      ...
      if moduleInfo ~= nil then
         do
            ...
            if metaInfoStem ~= nil then
               Log.log( Log.Level.Info, __func__, 2962, function (  )
   ...
   local metaInfo = metaInfoStem
   Log.log( Log.Level.Debug, __func__, 2986, function (  )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _
               local _7275, metaTypeId2TypeInfoMap = self:processImport( dependName, depth + 1 )
   ...
   for __index, _7293 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   local importParam = ImportParam.new(self.parser:getLastPos(  ), self, self.processInfo, typeId2Scope, typeId2TypeInfo, {}, metaInfo, self.scope, moduleTypeInfo, self.scopeAccess, id2atomMap)
   ...
   for __index, atomInfo in ipairs( _typeInfoNormalList ) do
      if #atomInfo.children > 0 then
         ...
         for __index, childId in ipairs( atomInfo.children ) do
            ...
            if addFlag then
               scope:add( self.processInfo, symbolKind, false, typeInfo:get_kind() == Ast.TypeInfoKind.Func, typeInfo:getTxt(  ), nil, typeInfo, typeInfo:get_accessMode(), typeInfo:get_staticFlag(), typeInfo:get_mutMode(), true )
            end
   ...
   local function registMember( classTypeId )
      ...
      do
         ...
         elseif _switchExp == Ast.TypeInfoKind.Module then
            ...
            Log.log( Log.Level.Debug, __func__, 3251, function (  )
   ...
   for __index, _7429 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   local moduleInfo = Nodes.ModuleInfo.new(modulePath, nameList[#nameList], newId2OldIdMap, frontInterface.ModuleId.createIdFromTxt( metaInfo.__buildId ), importParam.importedAliasMap)
   self.importModule2ModuleInfo[moduleTypeInfo] = moduleInfo
   self.importModuleName2ModuleInfo[modulePath] = moduleInfo
   
   self.importModuleInfo:remove(  )
   
   Log.log( Log.Level.Info, __func__, 3348, function (  )
...
function TransUnit:analyzeImportFor( pos, modulePath, assignName )
   ...
   local moduleSymbolInfo, shadowing = self.scope:add( self.processInfo, moduleSymbolKind, false, false, assignName, pos, moduleTypeInfo, Ast.AccessMode.Local, true, metaInfo.__moduleMutable and Ast.MutMode.Mut or Ast.MutMode.IMut, true )
   
   if moduleSymbolInfo ~= nil then
      return Nodes.ImportNode.create( self.nodeManager, pos, self.macroCtrl:isInAnalyzeArgMode(  ), {moduleTypeInfo}, modulePath, assignName, moduleSymbolInfo, moduleTypeInfo )
   end
...
function TransUnit:analyzeImport( token )
   ...
   local node = self:analyzeImportFor( token.pos, modulePath, assignName.txt )
...
function TransUnit:analyzeTestCase( firstToken )
   ...
   local importNode = self:analyzeImportFor( firstToken.pos, "lune.base.Testing", "__t" )
...
function TransUnit:analyzeForeach( token, sortFlag )
   ...
   if sortFlag then
      return Nodes.ForsortNode.create( self.nodeManager, token.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, mainSym, subSym, exp, block, sortFlag )
   else
    
      return Nodes.ForeachNode.create( self.nodeManager, token.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, mainSym, subSym, exp, block )
   end
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if moduleName ~= nil then
      for _8075 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
   ...
   local function createId2proto( map )
      ...
      local id2proto = {}
      for protoType, _8082 in pairs( map ) do
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _8149, existSym = self.scope:addMacro( self.processInfo, nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _8307, shadowing = self.scope:addEnum( self.processInfo, accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _8319, shadowing = scope:addAlge( self.processInfo, accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeDeclMember( classTypeInfo, accessMode, staticFlag, firstToken )
   ...
   if token.txt == "{" then
      ...
      if nextToken.txt == "," then
         ...
         Log.log( Log.Level.Debug, __func__, 1651, function (  )
         ...
         do
            ...
            if overrideType ~= nil then
               if not overrideType:get_abstractFlag() then
                  funcBodyScope:add( self.processInfo, Ast.SymbolKind.Fun, false, false, "super", nil, overrideType, Ast.AccessMode.Local, false, Ast.MutMode.IMut, true )
               end
...
function TransUnit:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
   ...
   local node = Nodes.DeclClassNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {classTypeInfo}, classAccessMode, name, hasProto, gluePrefix, moduleName, false, allStmtList, declStmtList, fieldList, memberList, self.scope, initBlockInfo, advertiseList, trustList, uninitMemberList, {} )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _8756, methodNameSet = self:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
...
function TransUnit:analyzeDeclFunc( declFuncMode, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   if classTypeInfo ~= nil then
      ...
      if kind == Nodes.NodeKind.get_DeclMethod() or kind == Nodes.NodeKind.get_DeclConstr() or kind == Nodes.NodeKind.get_DeclDestr() then
         ...
         if not staticFlag then
            self.scope:add( self.processInfo, Ast.SymbolKind.Arg, false, true, "self", nil, workClass, Ast.AccessMode.Pri, false, mutable and Ast.MutMode.Mut or Ast.MutMode.IMut, true )
         end
...
function TransUnit:analyzeInitExp( firstPos, accessMode, unwrapFlag, letVarList, typeInfoList )
   ...
   if expList ~= nil then
      ...
      if unwrapFlag then
         ...
         for index, _9079 in ipairs( letVarList ) do
...
function TransUnit:analyzeExpList( allowNoneType, skipOp2Flag, canLeftExp, expNode, expectTypeList, contExpect )
   ...
   repeat 
      ...
      if self.macroCtrl:get_analyzeInfo():get_mode() == Nodes.MacroMode.AnalyzeArg and (expectType == Ast.builtinTypeExp or expectType == Ast.builtinTypeMultiExp ) then
         ...
         exp = self:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, 0, expectType )
      ...
      if not allowNoneType and not exp:canBeRight( self.processInfo ) then
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _9684 = 1, #expNode:get_expTypeList() do
      ...
      local _9687, _9688, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeExpField( firstToken, fieldToken, mode, prefixExp )
   ...
   if self.macroCtrl:get_analyzeInfo():isAnalyzingSymArg(  ) then
      ...
      return Nodes.RefFieldNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeSymbol}, fieldToken, nil, accessNil, prefixExp )
   ...
   do
      ...
      elseif _switchExp == Ast.TypeInfoKind.Map then
         ...
         return Nodes.ExpRefItemNode.create( self.nodeManager, fieldToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, prefixExp, accessNil, fieldToken.txt, nil )
      else 
   ...
   do
      ...
      if _exp ~= nil then
         return Nodes.GetFieldNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, fieldToken, accessSymbolInfo, accessNil, prefixExp, _exp )
         
      else
         return Nodes.RefFieldNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, fieldToken, accessSymbolInfo, accessNil, prefixExp )
      end
   end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _10409, _10410, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _10502, _10503, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
         ...
         if Parser.isOp2( opTxt ) then
            ...
            do
               ...
               elseif _switchExp == "~=" or _switchExp == "==" then
                  exp, exp2 = self:analyzeExpOpEquals( firstToken.pos, nextToken, exp, exp2 )
                  retType = Ast.builtinTypeBool
...
function TransUnit:analyzeExpMacroStat( firstToken )
   ...
   while true do
      ...
      if token.txt == ",," or token.txt == ",,," or token.txt == ",,,," then
         ...
         local literalStr = Nodes.LiteralStringNode.create( self.nodeManager, token.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeString}, newToken, nil, nil )
         table.insert( expStrList, literalStr )
      end
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _10774, _10775, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
      ...
      local workExp = Nodes.LiteralStringNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeString}, token, param, dddParam )
      if nextToken.txt == "[" or nextToken.txt == "$[" then
      ...
      exp = Nodes.LiteralStringNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeString}, token, nil, nil )
   end
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _10840, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _10944, _10945, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
