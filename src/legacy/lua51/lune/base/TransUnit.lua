   ...
   if tentativeSymbol:get_scope():get_validCheckingUnaccess() then
      do
         ...
         for __index, __key in ipairs( __sorted ) do
            ...
            do
               if (not symbolInfo:get_posForModToRef() or symbolInfo:get_posForModToRef() ~= symbolInfo:get_posForLatestMod() ) and symbolInfo:get_kind() == Ast.SymbolKind.Var and not symbolInfo:get_name():find( "^_" ) then
                  do
...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5408, existSym = parentScope:addClass( modName, nil, typeInfo )
...
function _TypeInfoModule:createTypeInfo( param )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         Log.log( Log.Level.Info, __func__, 1320, function (  )
   ...
   if self.parentId ~= Ast.rootTypeId or not Ast.getBuiltInTypeIdMap(  )[self.typeId] or self.kind == Ast.TypeInfoKind.List or self.kind == Ast.TypeInfoKind.Array or self.kind == Ast.TypeInfoKind.Map or self.kind == Ast.TypeInfoKind.Set then
      ...
      if newTypeInfo and (self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF ) then
         ...
         if self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF then
            Log.log( Log.Level.Info, __func__, 1423, function (  )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _6171, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
...
function TransUnit:processImport( modulePath )
   ...
   Log.log( Log.Level.Info, __func__, 2760, function (  )
      ...
      if moduleInfo ~= nil then
         do
            ...
            if metaInfoStem ~= nil then
               Log.log( Log.Level.Info, __func__, 2772, function (  )
   ...
   local metaInfo = metaInfoStem
   Log.log( Log.Level.Info, __func__, 2792, function (  )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _6395, metaTypeId2TypeInfoMap = self:processImport( dependName )
   ...
   for __index, _6409 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   for __index, _6465 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   Log.log( Log.Level.Info, __func__, 3129, function (  )
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if moduleName ~= nil then
      for _6773 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _6817, existSym = self.scope:addMacro( nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _6913, shadowing = self.scope:addEnum( accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _6924, shadowing = scope:addAlge( accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _7175, methodNameSet = self:analyzeClassBody( classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _7642 = 1, #expNode:get_expTypeList() do
      ...
      local _7643, _7644, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _8031, _8032, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
            if _8031 ~= nil and _8032 ~= nil and newExpNodeList ~= nil then
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _8071, _8072, _8073, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _8168, _8169, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _8202, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _8239, _8240, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if _8239 ~= nil and _8240 ~= nil and newExpNodeList ~= nil then
