...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5843, existSym = parentScope:addClass( self.processInfo, modName, nil, typeInfo )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _7003, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
...
function TransUnit:processImport( modulePath, depth )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _
               local _7408, metaTypeId2TypeInfoMap = self:processImport( dependName, depth + 1 )
   ...
   for __index, _7426 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   for __index, _7565 in ipairs( nameList ) do
      self:popModule(  )
   end
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if moduleName ~= nil then
      for _8233 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
   ...
   local function createId2proto( map )
      ...
      local id2proto = {}
      for protoType, _8240 in pairs( map ) do
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _8308, existSym = self.scope:addMacro( self.processInfo, nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _8471, shadowing = self.scope:addEnum( self.processInfo, accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _8483, shadowing = scope:addAlge( self.processInfo, accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _8933, methodNameSet = self:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, lazyLoad, nextToken )
...
function TransUnit:analyzeInitExp( firstPos, accessMode, unwrapFlag, letVarList, typeInfoList )
   ...
   if expList ~= nil then
      ...
      if unwrapFlag then
         ...
         for index, _9256 in ipairs( letVarList ) do
...
function TransUnit:checkMatchValType( pos, funcTypeInfo, expList, genericTypeList, genericsClass )
   ...
   local matchResult, _9845, newExpNodeList = self:checkMatchType( funcTypeInfo:getTxt(  ), pos, argTypeList, expList, false, warnForFollow, alt2typeMap )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _9880 = 1, #expNode:get_expTypeList() do
      ...
      local _9883, _9884, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _10613, _10614, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _10707, _10708, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _10982, _10983, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _11051, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _11155, _11156, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
