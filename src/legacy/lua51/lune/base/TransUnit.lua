...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5342, existSym = parentScope:addClass( modName, nil, typeInfo )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _6087, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
...
function TransUnit:processImport( modulePath )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _6311, metaTypeId2TypeInfoMap = self:processImport( dependName )
   ...
   for __index, _6325 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   for __index, _6381 in ipairs( nameList ) do
      self:popModule(  )
   end
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if moduleName ~= nil then
      for _6689 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _6733, existSym = self.scope:addMacro( nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _6829, shadowing = self.scope:addEnum( accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _6840, shadowing = scope:addAlge( accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _7091, methodNameSet = self:analyzeClassBody( classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _7557 = 1, #expNode:get_expTypeList() do
      ...
      local _7558, _7559, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _7921, _7922, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
            if _7921 ~= nil and _7922 ~= nil and newExpNodeList ~= nil then
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _7961, _7962, _7963, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _8058, _8059, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _8092, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _8129, _8130, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if _8129 ~= nil and _8130 ~= nil and newExpNodeList ~= nil then
