...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if restIndex ~= nil then
      self:write( "[]LnsAny{ " )
      for index, _5366 in ipairs( argList:get_expList() ) do
...
function convFilter:processRoot( node, opt )
   ...
   for __index, workNode in ipairs( node:get_nodeManager():getIfUnwrapNodeList(  ) ) do
      local symTypeList = {}
      for _5420 = 1, #workNode:get_varSymList() do
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in ipairs( node:get_caseList() ) do
         for _5730, symbol in ipairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _5773 in ipairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:processForsort( node, opt )
   ...
   self:pushIndent(  )
   if valSym ~= nil then
      self:writeln( string.format( "%s := %s[ _%s ]", self:getSymbolSym( valSym ), collSym, key) )
   end
   
   if keySym then
      self:writeln( string.format( "%s := _%s", key, key) )
   end
...
function convFilter:outputConstructor( node )
   ...
   for index, _5880 in ipairs( initFuncType:get_argTypeInfoList() ) do
      if index ~= 1 then
         self:write( ", " )
      end
   ...
   if not node:hasUserInit(  ) then
      ...
      for index, _5888 in ipairs( initFuncType:get_argTypeInfoList() ) do
