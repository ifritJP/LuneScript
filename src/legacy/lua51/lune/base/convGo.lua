...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if restIndex ~= nil then
      self:write( "[]LnsAny{ " )
      for index, _5340 in ipairs( argList:get_expList() ) do
...
function convFilter:outputConstructor( node )
   ...
   for index, _5786 in ipairs( initFuncType:get_argTypeInfoList() ) do
      if index ~= 1 then
         self:write( ", " )
      end
   ...
   if not node:hasUserInit(  ) then
      ...
      for index, _5794 in ipairs( initFuncType:get_argTypeInfoList() ) do
