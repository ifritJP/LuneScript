...
local function outputGoMain( mod, testing, path )
...
func main() {
   ...
   if testing then
      code = base_mainCode:gsub( "Lns_init", string.format( "Lns_%s_init", mainMod) )
      code = code:gsub( "//TEST:", "" )
      ...
      code = base_mainCode:gsub( "Lns_init%(%)", string.format( "Lns_RunMain( %s___main )", concatGLSym( mainMod, true )) )
   end
   ...
   do
      ...
      elseif _matchExp[1] == SymbolKind.Var[1] then
         ...
         if self:needPrefix( symbolInfo:getModule(  ) ) then
            symbolName = string.format( "%s.%s", getModuleName( symbolInfo:getModule(  ) ), symbolName)
         end
      ...
      elseif _matchExp[1] == SymbolKind.Type[1] then
         ...
         if needPrefix and self:needPrefix( typeInfo:getModule(  ) ) then
            symbolName = string.format( "%s.%s", getModuleName( typeInfo ), symbolName)
         end
...
function convFilter:processImport( node, opt )
   ...
   if not self:isSameModDir( node:get_moduleTypeInfo() ) and not Ast.isBuiltin( node:get_moduleTypeInfo():get_typeId() ) then
      self:write( string.format( "%s.", getModuleName( node:get_moduleTypeInfo() )) )
   end
...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if mRetIndex ~= nil then
      ...
      if restIndex ~= nil then
         self:write( "Lns_2DDD( " )
         for index, _6196 in ipairs( expList ) do
      ...
      for index, _6204 in ipairs( retTypeList ) do
         if index ~= 1 then
            self:write( ", " )
         end
...
function convFilter:outputConvExt( funcNode )
   ...
   self:writeln( ") {" )
   self:write( "    return " )
   for index, _6396 in ipairs( funcNode:get_expType():get_retTypeInfoList() ) do
      if index > 1 then
         self:write( "," )
      end
...
function convFilter:processRoot( node, opt )
   ...
   do
      local function procNode( workNode )
         ...
         local symTypeList = {}
         for _6572 = 1, #workNode:get_varSymList() do
   ...
   if self.option.mainModule == self:getCanonicalName( self.moduleTypeInfo, false ) then
      local hasMain = false
      ...
      if not hasMain then
         self:writeln( string.format( "func %s___main(args *LnsList) LnsInt {", getModuleName( self.moduleTypeInfo )) )
         self:writeln( string.format( "Lns_%s_init()", getModuleName( self.moduleTypeInfo )) )
...
function convFilter:processIfUnwrap( node, opt )
   ...
   if getExpListKind( tempTypeList, node:get_expList() ) == ExpListKind.Direct then
      for _7007 = #node:get_varSymList() + 1, #node:get_expList():get_expTypeList() do
         self:write( ", _" )
      end
   ...
   if node:get_unwrapFlag() then
      do
         ...
         if expList ~= nil and  unwrapBlock ~= nil then
            ...
            for index, _7045 in ipairs( node:get_symbolInfoList() ) do
            ...
            if getExpListKind( tmpVarTypeList, expList ) == ExpListKind.Direct then
               for _7049 = #tmpVarTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
      ...
      do
         ...
         if expList ~= nil then
            ...
            if getExpListKind( varTypeList, expList ) == ExpListKind.Direct then
               for _7079 = #varTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         ...
         if getExpListKind( typeList, expList ) == ExpListKind.Direct then
            for _7108 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
               self:write( ",_" )
            end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in ipairs( node:get_caseList() ) do
         for _7239, symbol in ipairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _7303 in ipairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _7543 in ipairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) and not symbol:get_staticFlag() then
                  ...
                  for index, _7746 in ipairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
...
function convFilter:processExpSetVal( node, opt )
   ...
   if getExpListKind( node:get_exp1():get_expTypeList(), node:get_exp2() ) == ExpListKind.Direct then
      for _8028 = #node:get_exp1():get_expTypeList() + 1, #node:get_exp2():get_expTypeList() do
         self:write( ",_" )
      end
