...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if restIndex ~= nil then
      self:write( "Lns_2DDD( " )
      for index, _5536 in ipairs( argList:get_expList() ) do
...
function convFilter:processRoot( node, opt )
   ...
   for __index, workNode in ipairs( node:get_nodeManager():getIfUnwrapNodeList(  ) ) do
      local symTypeList = {}
      for _5668 = 1, #workNode:get_varSymList() do
...
function convFilter:processIfUnwrap( node, opt )
   ...
   for index, varSym in ipairs( node:get_varSymList() ) do
      ...
      if index == #node:get_varSymList() then
         for _5856 = index + 1, #node:get_expList():get_expTypeList() do
            self:write( ", _" )
         end
               ...
               if index == #node:get_varList() then
                  do
                     ...
                     if expListNode ~= nil then
                        for _5874 = index + 1, #expListNode:get_expTypeList() do
                           self:write( ", _" )
                        end
      ...
      do
         ...
         if expList ~= nil then
            ...
            if #expList:get_expTypeList() > #node:get_symbolInfoList() then
               for _5886 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
...
function convFilter:processDeclVar( node, opt )
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         ...
         for _5903 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
            self:write( ",_" )
         end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in ipairs( node:get_caseList() ) do
         for _5997, symbol in ipairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _6040 in ipairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _6171 in ipairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) then
                  ...
                  for index, _6261 in ipairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
...
function convFilter:processExpSetVal( node, opt )
   ...
   for _6384 = #node:get_exp1():get_expTypeList() + 1, #node:get_exp2():get_expTypeList() do
      self:write( ",_" )
   end
