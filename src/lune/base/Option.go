...
// for 333
func Option_convExp880(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
// for 328
func Option_convExp887(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
// for 223
func Option_convExp680(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
...
func Option_outputLuneMod(_env *LnsEnv, path LnsAny) LnsAny {
    ...
    {
        _fileObj := Option_convExp680(Lns_2DDD(Lns_io_open(lune_path, "w")))
        if _fileObj == nil{
...
// 263: decl @lune.@base.@Option.analyze.printUsage
...
// 261: decl @lune.@base.@Option.analyze
func Option_analyze(_env *LnsEnv, argList *LnsList) *Option_Option {
    ...
    index = 1
    {
        _file := Option_convExp887(Lns_2DDD(Lns_io_open("lune.js", "r")))
        if !Lns_IsNil( _file ) {
            file := _file.(Lns_luaStream)
            {
                _projInfo := Option_convExp880(Lns_2DDD(Option_ProjInfo1199__fromStem_3_(_env, Lns_car(Json_fromStr(_env, _env.PopVal( _env.IncStack() ||
    ...
    for argList.Len() >= index {
        ...
        if Lns_isCondTrue( Lns_car(_env.LuaVM.String_find(arg,"^-", nil, nil))){
            if option.Mode != Option_ModeKind__Shebang{
                ...
                } else if _switch1 == "--enableAsyncCtl" {
                    option.TransCtrlInfo.ValidAsyncCtrl = true
                    
                } else if _switch1 == "--defaultAsync" {
                    option.TransCtrlInfo.DefaultAsync = true
                    
                } else if _switch1 == "--limitThread" {
                    var nextArg string
                ...
                } else if _switch1 == "--nodebug" {
                    Util_setDebugFlag(_env, false)
                } else if _switch1 == "--debug" {
                    Util_setDebugFlag(_env, true)
                } else if _switch1 == "-shebang" {
                    option.Mode = Option_ModeKind__Shebang
                    
                } else if _switch1 == "--version" {
                    Lns_print([]LnsAny{_env.LuaVM.String_format("LuneScript: version %s (%d:Lua%s) [%s]", []LnsAny{Ver_version, Option_getBuildCount_1_(_env), Depend_getLuaVersion(_env), Ver_metaVersion})})
                    _env.LuaVM.OS_exit(0)
                } else if _switch1 == "--projDir" {
                    option.projDir = getNextOp(_env)
                    
                } else if _switch1 == "--builtin" {
                    {
                        __forsortCollection1 := Ast_getBuiltInTypeIdMap(_env)
                        __forsortSorted1 := __forsortCollection1.CreateKeyListInt()
                        __forsortSorted1.Sort( _env, LnsItemKindInt, nil )
                        for _, _typeId := range( __forsortSorted1.Items ) {
                            builtinTypeInfo := __forsortCollection1.Items[ _typeId ].(Ast_BuiltinTypeInfoDownCast).ToAst_BuiltinTypeInfo()
                            typeId := _typeId.(LnsInt)
                            Lns_print([]LnsAny{typeId, builtinTypeInfo.FP.Get_typeInfo(_env).FP.GetTxt(_env, nil, nil, nil)})
                        }
                    }
                    _env.LuaVM.OS_exit(0)
                } else if _switch1 == "-mklunemod" {
                    var path LnsAny
                    path = getNextOp(_env)
                    {
                ...
                } else if _switch1 == "-Werror" {
                    option.TransCtrlInfo.StopByWarning = true
                    
                } else if _switch1 == "--disable-checking-define-abbr" {
                    option.TransCtrlInfo.CheckingDefineAbbr = false
                    
                } else if _switch1 == "--disable-checking-mutable" {
                ...
                } else if _switch1 == "--testing" {
                    option.Testing = true
                    
                    option.TransCtrlInfo.Testing = true
                    
                } else if _switch1 == "--enableTestBlock" {
                    option.TransCtrlInfo.Testing = true
                    
                } else if _switch1 == "--depends" {
                    option.DependsPath = getNextOp(_env)
                    
                } else if _switch1 == "--use-ipairs" {
                    option.UseIpairs = true
                    
                } else if _switch1 == "--uptodate" {
                    uptodateOpt = getNextOp(_env)
                    
                } else if _switch1 == "-langC" {
                    option.ConvTo = Types_Lang__C
                    
                    option.TransCtrlInfo.ValidLuaval = true
                    
                } else if _switch1 == "-langGo" {
                ...
                } else if _switch1 == "-ob0" || _switch1 == "-ob1" {
            ...
            if option.ScriptPath == ""{
                ...
                if _switch4 := (option.Mode); _switch4 == Option_ModeKind__Complete || _switch4 == Option_ModeKind__Inquire {
                ...
                } else if _switch4 == Option_ModeKind__Save || _switch4 == Option_ModeKind__SaveMeta || _switch4 == Option_ModeKind__Glue {
                    option.OutputDir = arg
                    
                } else if _switch4 == Option_ModeKind__Shebang {
                    if option.ShebangArgList.Len() == 0{
                        option.ShebangArgList.Insert(option.ScriptPath)
                    }
                    option.ShebangArgList.Insert(arg)
    ...
    if uptodateOpt != nil{
        uptodateOpt_271 := uptodateOpt.(string)
        if _switch5 := uptodateOpt_271; _switch5 == "force" {
            ...
            Util_errorLog(_env, "illegal mode -- " + uptodateOpt_271)
        }
    }
    ...
    Log_log(_env, Log_Level__Log, __func__, 673, Log_CreateMessage(func(_env *LnsEnv) string {
...
// 678: decl @lune.@base.@Option.createDefaultOption
func Option_createDefaultOption(_env *LnsEnv, pathList *LnsList,projDir LnsAny) *Option_Option {
    ...
    if projDir != nil{
        projDir_305 := projDir.(string)
        if projDir_305 != "/"{
            if Lns_op_not(Lns_car(_env.LuaVM.String_find(projDir_305,"/$", nil, nil))){
                option.projDir = projDir_305 + "/"
...
// 195: decl @lune.@base.@Option.Option.openDepend
func (self *Option_Option) OpenDepend(_env *LnsEnv, relPath LnsAny) LnsAny {
    {
        ...
        if !Lns_IsNil( _path ) {
            ...
            var filePath string
            if relPath != nil{
                relPath_118 := relPath.(string)
                if Lns_isCondTrue( Lns_car(_env.LuaVM.String_find(path,"/$", nil, nil))){
                    filePath = _env.LuaVM.String_format("%s%s", []LnsAny{path, relPath_118})
...
func Option_ProjInfo1199__fromMap_2_(_env,  arg1 LnsAny, paramList []Lns_ToObjParam)(LnsAny, LnsAny){
   return Option_ProjInfo1199_FromMap( arg1, paramList )
}
func Option_ProjInfo1199__fromStem_3_(_env,  arg1 LnsAny, paramList []Lns_ToObjParam)(LnsAny, LnsAny){
   return Option_ProjInfo1199_FromMap( arg1, paramList )
}
