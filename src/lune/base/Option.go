...
// for 355
func Option_convExp0_965(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
// for 360
func Option_convExp0_958(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
// for 242
func Option_convExp0_766(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
...
func Option_outputLuneMod(_env *LnsEnv, path LnsAny) LnsAny {
    ...
    {
        _fileObj := Option_convExp0_766(Lns_2DDD(Lns_io_open(lune_path, "w")))
        if _fileObj == nil{
...
func Option_analyze(_env *LnsEnv, argList *LnsList) *Option_Option {
    ...
    index = 1
    {
        _file := Option_convExp0_965(Lns_2DDD(Lns_io_open("lune.js", "r")))
        if !Lns_IsNil( _file ) {
            file := _file.(Lns_luaStream)
            {
                _projInfo := Option_convExp0_958(Lns_2DDD(Option_ProjInfo1211__fromStem_3_(_env, Lns_car(Json_fromStr(_env, _env.PopVal( _env.IncStack() ||
                    _env.SetStackVal( file.Read(_env, "*a")) ||
                    _env.SetStackVal( "") ).(string))),nil)))
                if !Lns_IsNil( _projInfo ) {
                    projInfo := _projInfo.(*Option_ProjInfo1211)
        ...
        if Lns_isCondTrue( Lns_car(_env.GetVM().String_find(arg,"^-", nil, nil))){
            if option.Mode != Option_ModeKind__Shebang{
                if _switch2 := (arg); _switch2 == "-i" {
                    ...
                    Util_println(_env, []LnsAny{_env.GetVM().String_format("LuneScript: version %s (%d:Lua%s) [%s]", []LnsAny{Ver_version, Option_getBuildCount_1_(_env), Depend_getLuaVersion(_env), Ver_metaVersion})})
                    ...
                    _ = builtin.FP.RegistBuiltInScope(_env)
                    {
                        ...
                        for _, _typeId := range( __forsortSorted0.Items ) {
                            ...
                            Util_println(_env, []LnsAny{typeId, builtinTypeInfo.FP.Get_typeInfo(_env).FP.GetTxt(_env, nil, nil, nil)})
                        }
                    }
                    _env.GetVM().OS_exit(0)
    ...
    if uptodateOpt != nil{
        uptodateOpt_314 := uptodateOpt.(string)
        if _switch4 := uptodateOpt_314; _switch4 == "force" {
            ...
            Util_errorLog(_env, "illegal mode -- " + uptodateOpt_314)
        }
    }
    ...
    Log_log(_env, Log_Level__Log, __func__, 770, Log_CreateMessage(func(_env *LnsEnv) string {
...
// 775: decl @lune.@base.@Option.createDefaultOption
func Option_createDefaultOption(_env *LnsEnv, pathList *LnsList,projDir LnsAny) *Option_Option {
    ...
    option.UseIpairs = true
    if projDir != nil{
        projDir_344 := projDir.(string)
        if projDir_344 != "/"{
            if Lns_op_not(Lns_car(_env.GetVM().String_find(projDir_344,"/$", nil, nil))){
                option.projDir = projDir_344 + "/"
            } else { 
                option.projDir = projDir_344
            }
        }
...
func (self *Option_Option) OpenDepend(_env *LnsEnv, relPath LnsAny) LnsAny {
    {
        ...
        if !Lns_IsNil( _path ) {
            ...
            var filePath string
            if relPath != nil{
                relPath_139 := relPath.(string)
                if Lns_isCondTrue( Lns_car(_env.GetVM().String_find(path,"/$", nil, nil))){
                    filePath = _env.GetVM().String_format("%s%s", []LnsAny{path, relPath_139})
                } else { 
                    filePath = _env.GetVM().String_format("%s/%s", []LnsAny{path, relPath_139})
                }
            } else {
...
type Option_ProjInfo1211Mtd interface {
    ToMap() *LnsMap
}
type Option_ProjInfo1211 struct {
    Cmd_option *LnsList
    FP Option_ProjInfo1211Mtd
}
func Option_ProjInfo12112Stem( obj LnsAny ) LnsAny {
    if obj == nil {
        return nil
    }
    return obj.(*Option_ProjInfo1211).FP
}
type Option_ProjInfo1211DownCast interface {
    ToOption_ProjInfo1211() *Option_ProjInfo1211
}
func Option_ProjInfo1211DownCastF( multi ...LnsAny ) LnsAny {
    if len( multi ) == 0 { return nil }
    obj := multi[ 0 ]
    if ddd, ok := multi[ 0 ].([]LnsAny); ok { obj = ddd[0] }
    work, ok := obj.(Option_ProjInfo1211DownCast)
    if ok { return work.ToOption_ProjInfo1211() }
    return nil
}
func (obj *Option_ProjInfo1211) ToOption_ProjInfo1211() *Option_ProjInfo1211 {
    return obj
}
func NewOption_ProjInfo1211(_env *LnsEnv, arg1 *LnsList) *Option_ProjInfo1211 {
    obj := &Option_ProjInfo1211{}
    obj.FP = obj
    obj.InitOption_ProjInfo1211(_env, arg1)
    return obj
}
func (self *Option_ProjInfo1211) InitOption_ProjInfo1211(_env *LnsEnv, arg1 *LnsList) {
    self.Cmd_option = arg1
}
func (self *Option_ProjInfo1211) ToMapSetup( obj *LnsMap ) *LnsMap {
    obj.Items["cmd_option"] = Lns_ToCollection( self.Cmd_option )
    return obj
}
func (self *Option_ProjInfo1211) ToMap() *LnsMap {
    return self.ToMapSetup( NewLnsMap( map[LnsAny]LnsAny{} ) )
}
func Option_ProjInfo1211__fromMap_2_(_env,  arg1 LnsAny, paramList []Lns_ToObjParam)(LnsAny, LnsAny){
   return Option_ProjInfo1211_FromMap( arg1, paramList )
}
func Option_ProjInfo1211__fromStem_3_(_env,  arg1 LnsAny, paramList []Lns_ToObjParam)(LnsAny, LnsAny){
   return Option_ProjInfo1211_FromMap( arg1, paramList )
}
func Option_ProjInfo1211_FromMap( obj LnsAny, paramList []Lns_ToObjParam ) (LnsAny, LnsAny) {
    _,conv,mess := Option_ProjInfo1211_FromMapSub(obj,false, paramList);
    return conv,mess
}
func Option_ProjInfo1211_FromMapSub( obj LnsAny, nilable bool, paramList []Lns_ToObjParam ) (bool, LnsAny, LnsAny) {
    var objMap *LnsMap
    ...
    newObj := &Option_ProjInfo1211{}
    newObj.FP = newObj
    return Option_ProjInfo1211_FromMapMain( newObj, objMap, paramList )
}
func Option_ProjInfo1211_FromMapMain( newObj *Option_ProjInfo1211, objMap *LnsMap, paramList []Lns_ToObjParam ) (bool, LnsAny, LnsAny) {
