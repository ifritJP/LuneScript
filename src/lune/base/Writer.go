...
func Writer_XML_convertXmlTxt_1066_(_env *LnsEnv, val LnsAny) string {
    if val == ""{
        return ""
    }
...
func (self *Writer_XML) WriteValue(_env *LnsEnv, val LnsAny) {
    self.stream.Write(_env, Writer_XML_convertXmlTxt_1066_(_env, val))
}
...
func Writer_JSON_convertJsonTxt_1276_(_env *LnsEnv, txt string) string {
    if txt == ""{
        return ""
    }
...
func (self *Writer_JSON) WriteValue(_env *LnsEnv, val LnsAny) {
    ...
    if typeId == "number"{
        ...
        txt = _env.LuaVM.String_format("\"%s\"", []LnsAny{Writer_JSON_convertJsonTxt_1276_(_env, _env.LuaVM.String_format("%s", []LnsAny{val}))})
