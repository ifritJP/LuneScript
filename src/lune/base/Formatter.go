...
func Formatter_filter_1044_(node *Nodes_Node,filter *Formatter_FormatterFilter,opt *Formatter_Opt) {
...
func (self *Formatter_FormatterFilter) ProcessBlankLine(node *Nodes_BlankLineNode,_opt LnsAny) {
    {
        var _from261 LnsInt = 1
        var _to261 LnsInt = node.FP.Get_lineNum()
        for _work261 := _from261; _work261 <= _to261; _work261++ {
            self.FP.Writeln("")
        }
    }
}
...
func (self *Formatter_FormatterFilter) ProcessRoot(node *Nodes_RootNode,_opt LnsAny) {
    ...
    for _, _child := range( node.FP.Get_children().Items ) {
        ...
        Formatter_filter_1044_(child, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
}
...
func (self *Formatter_FormatterFilter) ProcessEnv(node *Nodes_EnvNode,_opt LnsAny) {
    ...
    Formatter_filter_1044_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
}
...
func (self *Formatter_FormatterFilter) ProcessBlockSub(node *Nodes_BlockNode,_opt LnsAny) {
    ...
    for _, _statement := range( node.FP.Get_stmtList().Items ) {
        ...
        Formatter_filter_1044_(statement, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
    self.FP.PopIndent()
    if _switch500 := node.FP.Get_blockKind(); _switch500 == Nodes_BlockKind__LetUnwrap || _switch500 == Nodes_BlockKind__Repeat {
        self.FP.Write("}")
    } else {
        self.FP.Writeln("}")
    }
...
func (self *Formatter_FormatterFilter) ProcessStmtExp(node *Nodes_StmtExpNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_exp(), self, opt.FP.NextOpt(&node.Nodes_Node))
    {
...
func (self *Formatter_FormatterFilter) ProcessNewAlgeVal(node *Nodes_NewAlgeValNode,_opt LnsAny) {
    ...
    for _, _exp := range( node.FP.Get_paramList().Items ) {
        ...
        Formatter_filter_1044_(exp, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
}

// 187: decl @lune.@base.@Formatter.FormatterFilter.outputDeclClass
func (self *Formatter_FormatterFilter) outputDeclClass(protoFlag bool,classType *Ast_TypeInfo,gluePrefix LnsAny,moduleName LnsAny) {
    ...
    if _switch763 := classType.FP.Get_kind(); _switch763 == Ast_TypeInfoKind__Class {
        if Lns_isCondTrue( moduleName){
            self.FP.Write("module ")
        } else { 
            self.FP.Write("class ")
        }
    } else if _switch763 == Ast_TypeInfoKind__IF {
    ...
    if moduleName != nil{
        moduleName_3393 := moduleName.(*Types_Token)
        self.FP.Write(" require ")
        self.FP.Write(Lns_getVM().String_format("%s ", []LnsAny{moduleName_3393.Txt}))
        if gluePrefix != nil{
            gluePrefix_3395 := gluePrefix.(string)
            self.FP.Write("glue ")
            self.FP.Write(gluePrefix_3395)
        }
    }
...
func (self *Formatter_FormatterFilter) ProcessDeclClass(node *Nodes_DeclClassNode,_opt LnsAny) {
    ...
    for _, _stmt := range( node.FP.Get_allStmtList().Items ) {
        ...
        Formatter_filter_1044_(stmt, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
    self.FP.PopIndent()
    self.FP.Writeln("}")
}
...
func (self *Formatter_FormatterFilter) ProcessDeclMember(node *Nodes_DeclMemberNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    if _switch1176 := node.FP.Get_accessMode(); _switch1176 == Ast_AccessMode__Pub || _switch1176 == Ast_AccessMode__Pro || _switch1176 == Ast_AccessMode__None || _switch1176 == Ast_AccessMode__Local {
    ...
    self.FP.Write(":")
    Formatter_filter_1044_(&node.FP.Get_refType().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
...
func (self *Formatter_FormatterFilter) ProcessExpMacroStat(node *Nodes_ExpMacroStatNode,_opt LnsAny) {
    ...
    for _, _strNode := range( node.FP.Get_expStrList().Items ) {
        ...
        Formatter_filter_1044_(strNode, self, opt.FP.NextOpt(strNode))
    }
}
...
func (self *Formatter_FormatterFilter) ProcessUnwrapSet(node *Nodes_UnwrapSetNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(&node.FP.Get_dstExpList().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    Formatter_filter_1044_(&node.FP.Get_srcExpList().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    if Lns_isCondTrue( node.FP.Get_unwrapBlock()){
        Formatter_filter_1044_(&Lns_unwrap( node.FP.Get_unwrapBlock()).(*Nodes_BlockNode).Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
}
...
func (self *Formatter_FormatterFilter) ProcessIfUnwrap(node *Nodes_IfUnwrapNode,_opt LnsAny) {
    ...
    Formatter_filter_1044_(&node.FP.Get_expList().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    Formatter_filter_1044_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    if Lns_isCondTrue( node.FP.Get_nilBlock()){
        self.FP.Write("else ")
        Formatter_filter_1044_(&Lns_unwrap( node.FP.Get_nilBlock()).(*Nodes_BlockNode).Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
}
...
func (self *Formatter_FormatterFilter) ProcessWhen(node *Nodes_WhenNode,_opt LnsAny) {
    ...
    Formatter_filter_1044_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    {
        ...
        if !Lns_IsNil( __exp ) {
            ...
            self.FP.Write("else ")
            Formatter_filter_1044_(&_exp.Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
        }
    }
}
...
func (self *Formatter_FormatterFilter) ProcessDeclVar(node *Nodes_DeclVarNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    if _switch1874 := node.FP.Get_accessMode(); _switch1874 == Ast_AccessMode__Pub {
        self.FP.Write("pub ")
    }
    if _switch1917 := node.FP.Get_mode(); _switch1917 == Nodes_DeclVarMode__Let {
        if node.FP.Get_unwrapFlag(){
            self.FP.Write("let! ")
        } else { 
            self.FP.Write("let ")
        }
    } else if _switch1917 == Nodes_DeclVarMode__Unwrap {
        ...
        if node.FP.Get_varList().Len() >= index{
            ...
            varInfo = node.FP.Get_varList().GetAt(index).(Nodes_VarInfoDownCast).ToNodes_VarInfo()
            {
                ...
                if !Lns_IsNil( _varType ) {
                    ...
                    self.FP.Write(":")
                    Formatter_filter_1044_(&varType.Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
                }
            }
        }
    }
...
func (self *Formatter_FormatterFilter) ProcessDeclArg(node *Nodes_DeclArgNode,_opt LnsAny) {
    ...
    self.FP.Write(":")
    {
        ...
        if !Lns_IsNil( _refType ) {
            ...
            Formatter_filter_1044_(&refType.Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
        } else {
...
// 548: decl @lune.@base.@Formatter.FormatterFilter.processDeclFuncInfo
func (self *Formatter_FormatterFilter) processDeclFuncInfo(node *Nodes_Node,declInfo *Nodes_DeclFuncInfo,opt *Formatter_Opt) {
    ...
    if Lns_isCondTrue( Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
        ...
        for _index, _arg := range( argList.Items ) {
            ...
            Formatter_filter_1044_(arg, self, opt.FP.NextOpt(node))
        }
        if argList.Len() != 0{
            self.FP.Write(" ")
        }
    ...
    }
    {
        ...
        if !Lns_IsNil( __exp ) {
            _exp := __exp.(*Nodes_BlockNode)
            self.FP.Write(" ")
            Formatter_filter_1044_(&_exp.Nodes_Node, self, opt.FP.NextOpt(node))
        } else {
            self.FP.Writeln(";")
        }
    }
...
func (self *Formatter_FormatterFilter) ProcessExpCallSuperCtor(node *Nodes_ExpCallSuperCtorNode,_opt LnsAny) {
    ...
    self.FP.Write("super(")
    {
        ...
        if !Lns_IsNil( _expListNode ) {
            ...
            Formatter_filter_1044_(&expListNode.Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
        }
    }
    self.FP.Writeln(");")
}
...
func (self *Formatter_FormatterFilter) ProcessRefType(node *Nodes_RefTypeNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    {
        ...
        if !Lns_IsNil( _mutMode ) {
            ...
            if _switch2860 := mutMode; _switch2860 == Ast_MutMode__IMut {
                self.FP.Write("&")
            } else if _switch2860 == Ast_MutMode__AllMut {
                self.FP.Write("allmut")
            }
        }
    }
    Formatter_filter_1044_(node.FP.Get_name(), self, opt.FP.NextOpt(&node.Nodes_Node))
    var expType *Ast_TypeInfo
    expType = node.FP.Get_expType().FP.Get_nonnilableType()
    if _switch3008 := expType.FP.Get_kind(); _switch3008 == Ast_TypeInfoKind__List || _switch3008 == Ast_TypeInfoKind__Set {
        self.FP.Write("<")
        Formatter_filter_1044_(node.FP.Get_itemNodeList().GetAt(1).(Nodes_NodeDownCast).ToNodes_Node(), self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write(">")
    } else if _switch3008 == Ast_TypeInfoKind__Map {
        self.FP.Write("<")
        Formatter_filter_1044_(node.FP.Get_itemNodeList().GetAt(1).(Nodes_NodeDownCast).ToNodes_Node(), self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write(",")
        Formatter_filter_1044_(node.FP.Get_itemNodeList().GetAt(2).(Nodes_NodeDownCast).ToNodes_Node(), self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write(">")
    }
...
// 691: decl @lune.@base.@Formatter.FormatterFilter.processIf
func (self *Formatter_FormatterFilter) ProcessIf(node *Nodes_IfNode,_opt LnsAny) {
    ...
    for _, _stmt := range( stmtList.Items ) {
        ...
        if _switch3083 := stmt.FP.Get_kind(); _switch3083 == Nodes_IfKind__If {
            self.FP.Write("if ")
        } else if _switch3083 == Nodes_IfKind__ElseIf {
            self.FP.Write("elseif ")
        } else if _switch3083 == Nodes_IfKind__Else {
            self.FP.Write("else ")
        }
        Formatter_filter_1044_(stmt.FP.Get_exp(), self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write(" ")
        Formatter_filter_1044_(&stmt.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
}

// 712: decl @lune.@base.@Formatter.FormatterFilter.processSwitch
func (self *Formatter_FormatterFilter) ProcessSwitch(node *Nodes_SwitchNode,_opt LnsAny) {
    ...
    for _, _caseInfo := range( caseList.Items ) {
        ...
        Formatter_filter_1044_(&caseInfo.FP.Get_expList().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
        Formatter_filter_1044_(&caseInfo.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
    }
    {
...
// 726: decl @lune.@base.@Formatter.FormatterFilter.processMatch
func (self *Formatter_FormatterFilter) ProcessMatch(node *Nodes_MatchNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_val(), self, opt.FP.NextOpt(&node.Nodes_Node))
    var caseList *LnsList
    caseList = node.FP.Get_caseList()
    for _, _caseInfo := range( caseList.Items ) {
    ...
    }
    {
        ...
        if !Lns_IsNil( __exp ) {
            ...
            Formatter_filter_1044_(_exp, self, opt.FP.NextOpt(&node.Nodes_Node))
        }
    }
}

// 739: decl @lune.@base.@Formatter.FormatterFilter.processWhile
...
// 747: decl @lune.@base.@Formatter.FormatterFilter.processRepeat
...
// 756: decl @lune.@base.@Formatter.FormatterFilter.processFor
func (self *Formatter_FormatterFilter) ProcessFor(node *Nodes_ForNode,_opt LnsAny) {
    ...
    Formatter_filter_1044_(node.FP.Get_init(), self, opt.FP.NextOpt(&node.Nodes_Node))
    self.FP.Write(", ")
    Formatter_filter_1044_(node.FP.Get_to(), self, opt.FP.NextOpt(&node.Nodes_Node))
    {
...
// 770: decl @lune.@base.@Formatter.FormatterFilter.processApply
...
// 786: decl @lune.@base.@Formatter.FormatterFilter.processForeach
...
// 800: decl @lune.@base.@Formatter.FormatterFilter.processForsort
...
// 815: decl @lune.@base.@Formatter.FormatterFilter.processExpUnwrap
...
// 837: decl @lune.@base.@Formatter.FormatterFilter.processExpCall
func (self *Formatter_FormatterFilter) ProcessExpCall(node *Nodes_ExpCallNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_func(), self, opt.FP.NextOpt(&node.Nodes_Node))
...
// 854: decl @lune.@base.@Formatter.FormatterFilter.processExpList
...
// 926: decl @lune.@base.@Formatter.FormatterFilter.processExpSetVal
func (self *Formatter_FormatterFilter) ProcessExpSetVal(node *Nodes_ExpSetValNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_exp1(), self, opt.FP.NextOpt(&node.Nodes_Node))
    self.FP.Write(" = ")
    Formatter_filter_1044_(&node.FP.Get_exp2().Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
}

// 933: decl @lune.@base.@Formatter.FormatterFilter.processExpSetItem
func (self *Formatter_FormatterFilter) ProcessExpSetItem(node *Nodes_ExpSetItemNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_val(), self, opt.FP.NextOpt(&node.Nodes_Node))
    switch _exp4625 := node.FP.Get_index().(type) {
    case *Nodes_IndexVal__NodeIdx:
    index := _exp4625.Val1
        self.FP.Write("[")
        Formatter_filter_1044_(index, self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write("]")
    ...
    self.FP.Write(" = ")
    Formatter_filter_1044_(node.FP.Get_exp2(), self, opt.FP.NextOpt(&node.Nodes_Node))
}

// 950: decl @lune.@base.@Formatter.FormatterFilter.processExpOp2
...
// 957: decl @lune.@base.@Formatter.FormatterFilter.processExpNew
func (self *Formatter_FormatterFilter) ProcessExpNew(node *Nodes_ExpNewNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    self.FP.Write("new ")
    Formatter_filter_1044_(node.FP.Get_symbol(), self, opt.FP.NextOpt(&node.Nodes_Node))
    self.FP.Write("(")
    {
        __exp := node.FP.Get_argList()
        if !Lns_IsNil( __exp ) {
...
// 969: decl @lune.@base.@Formatter.FormatterFilter.processExpRef
func (self *Formatter_FormatterFilter) ProcessExpRef(node *Nodes_ExpRefNode,_opt LnsAny) {
    self.FP.Write(node.FP.Get_symbolInfo().FP.Get_name())
}

// 974: decl @lune.@base.@Formatter.FormatterFilter.processExpRefItem
...
// 993: decl @lune.@base.@Formatter.FormatterFilter.processRefField
func (self *Formatter_FormatterFilter) ProcessRefField(node *Nodes_RefFieldNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_prefix(), self, opt.FP.NextOpt(&node.Nodes_Node))
    if node.FP.Get_nilAccess(){
        self.FP.Write("$.")
    } else { 
        self.FP.Write(".")
...
// 1005: decl @lune.@base.@Formatter.FormatterFilter.processExpOmitEnum
...
// 1012: decl @lune.@base.@Formatter.FormatterFilter.processGetField
...
// 1023: decl @lune.@base.@Formatter.FormatterFilter.processReturn
...
// 1035: decl @lune.@base.@Formatter.FormatterFilter.processProvide
func (self *Formatter_FormatterFilter) ProcessProvide(node *Nodes_ProvideNode,_opt LnsAny) {
}

// 1041: decl @lune.@base.@Formatter.FormatterFilter.processAlias
func (self *Formatter_FormatterFilter) ProcessAlias(node *Nodes_AliasNode,_opt LnsAny) {
    ...
    Formatter_filter_1044_(node.FP.Get_srcNode(), self, opt.FP.NextOpt(&node.Nodes_Node))
    self.FP.Writeln(";")
}

// 1049: decl @lune.@base.@Formatter.FormatterFilter.processTestCase
...
// 1057: decl @lune.@base.@Formatter.FormatterFilter.processBoxing
func (self *Formatter_FormatterFilter) ProcessBoxing(node *Nodes_BoxingNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_src(), self, opt.FP.NextOpt(&node.Nodes_Node))
}

// 1064: decl @lune.@base.@Formatter.FormatterFilter.processUnboxing
func (self *Formatter_FormatterFilter) ProcessUnboxing(node *Nodes_UnboxingNode,_opt LnsAny) {
    opt := _opt.(*Formatter_Opt)
    Formatter_filter_1044_(node.FP.Get_src(), self, opt.FP.NextOpt(&node.Nodes_Node))
}

// 1071: decl @lune.@base.@Formatter.FormatterFilter.processLiteralList
...
// 1084: decl @lune.@base.@Formatter.FormatterFilter.processLiteralSet
...
// 1097: decl @lune.@base.@Formatter.FormatterFilter.processLiteralMap
func (self *Formatter_FormatterFilter) ProcessLiteralMap(node *Nodes_LiteralMapNode,_opt LnsAny) {
    ...
    for _index, _pair := range( pairList.Items ) {
        ...
        Formatter_filter_1044_(pair.FP.Get_key(), self, opt.FP.NextOpt(&node.Nodes_Node))
        self.FP.Write(":")
        Formatter_filter_1044_(pair.FP.Get_val(), self, opt.FP.NextOpt(&node.Nodes_Node))
    }
    self.FP.Write("}")
}

// 1115: decl @lune.@base.@Formatter.FormatterFilter.processLiteralArray
...
// 1123: decl @lune.@base.@Formatter.FormatterFilter.processLiteralChar
...
// 1128: decl @lune.@base.@Formatter.FormatterFilter.processLiteralInt
...
// 1133: decl @lune.@base.@Formatter.FormatterFilter.processLiteralReal
func (self *Formatter_FormatterFilter) ProcessLiteralReal(node *Nodes_LiteralRealNode,_opt LnsAny) {
    self.FP.Write(node.FP.Get_token().Txt)
}

// 1138: decl @lune.@base.@Formatter.FormatterFilter.processLiteralString
func (self *Formatter_FormatterFilter) ProcessLiteralString(node *Nodes_LiteralStringNode,_opt LnsAny) {
    ...
    self.FP.Write(node.FP.Get_token().Txt)
    {
        ...
        if !Lns_IsNil( _expList ) {
            ...
            self.FP.Write(" ( ")
            Formatter_filter_1044_(&expList.Nodes_Node, self, opt.FP.NextOpt(&node.Nodes_Node))
            self.FP.Write(" )")
        }
    }
}

// 1150: decl @lune.@base.@Formatter.FormatterFilter.processLiteralBool
func (self *Formatter_FormatterFilter) ProcessLiteralBool(node *Nodes_LiteralBoolNode,_opt LnsAny) {
    self.FP.Write(node.FP.Get_token().Txt)
}

// 1155: decl @lune.@base.@Formatter.FormatterFilter.processLiteralNil
func (self *Formatter_FormatterFilter) ProcessLiteralNil(node *Nodes_LiteralNilNode,_opt LnsAny) {
    self.FP.Write("nil")
}

// 1160: decl @lune.@base.@Formatter.FormatterFilter.processBreak
func (self *Formatter_FormatterFilter) ProcessBreak(node *Nodes_BreakNode,_opt LnsAny) {
    self.FP.Writeln("break;")
}

// 1166: decl @lune.@base.@Formatter.FormatterFilter.processLiteralSymbol
func (self *Formatter_FormatterFilter) ProcessLiteralSymbol(node *Nodes_LiteralSymbolNode,_opt LnsAny) {
}

// 1172: decl @lune.@base.@Formatter.FormatterFilter.processAbbr
