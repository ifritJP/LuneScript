...
func (self *Formatter_FormatterFilter) outputDeclClass(protoFlag bool,classType *Ast_TypeInfo,gluePrefix LnsAny,moduleName LnsAny) {
    ...
    if moduleName != nil{
        moduleName_5222 := moduleName.(*Types_Token)
        self.FP.Write(" require ")
        self.FP.Write(Lns_getVM().String_format("%s ", []LnsAny{moduleName_5222.Txt}))
        if gluePrefix != nil{
            gluePrefix_5224 := gluePrefix.(string)
            self.FP.Write("glue ")
            self.FP.Write(gluePrefix_5224)
        }
    }
