...
// for 6213
func convGo_convExp0_10458(arg1 []LnsAny) bool {
    ...
    opt := _opt.(*ConvGo_Opt)
    {
        ...
        if !Lns_IsNil( _symbolInfo ) {
            ...
            if symbolInfo.FP.Get_staticFlag(_env){
                ...
                var closeParen bool
                closeParen = convGo_convExp0_10458(Lns_2DDD(self.FP.OutputCallPrefix(_env, node.FP.GetIdTxt(_env), &node.Nodes_Node, node.FP.Get_prefix(_env), symbolInfo)))
                self.FP.WriteRaw(_env, _env.GetVM().String_format("(%s)", []LnsAny{convGo_getAddEnvArg_6_(_env, 0, self.option.FP.Get_addEnvArg(_env))}))
                var retType *Ast_TypeInfo
