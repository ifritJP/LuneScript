...
func (self *convGo_convFilter) getSymbol(kind LnsAny,name string) string {
    ...
    typeInfo := _exp2638.Val1
        if typeInfo.FP.Get_kind() == Ast_TypeInfoKind__Method{
            ...
            if prefix != nil{
                prefix_5994 := prefix.(string)
                symbolName = Lns_getVM().String_format("%s.%s", []LnsAny{prefix_5994, symbolName})
...
func (self *convGo_convFilter) processConvExp(nodeId LnsInt,dstTypeList *LnsList,argListNode LnsAny) {
    ...
    if mRetIndex != nil{
        mRetIndex_6348 := mRetIndex.(LnsInt)
        ...
        for _index, _retType := range( retTypeList.Items ) {
            ...
            if index >= mRetIndex_6348{
                ...
                if Lns_op_not(wrote){
                    self.FP.Write(Lns_getVM().String_format("Lns_getFromMulti( arg%d, %d )", []LnsAny{mRetIndex_6348, index - mRetIndex_6348}))
        ...
        if restIndex != nil{
            restIndex_6365 := restIndex.(LnsInt)
            self.FP.Write("Lns_2DDD( ")
            for _index, _ := range( expList.Items ) {
                index := _index + 1
                if index >= restIndex_6365{
                    if index < expList.Len(){
                        self.FP.Write(Lns_getVM().String_format("arg%d", []LnsAny{index}))
                    } else { 
                        self.FP.Write(Lns_getVM().String_format("arg%d[%d:]", []LnsAny{mRetIndex_6348, index - mRetIndex_6348}))
                        break
                    }
                }
            }
...
func (self *convGo_convFilter) OutputDeclFunc(funcInfo LnsAny) *convGo_FuncConv {
    ...
    if convGo_isClosure_1183_(typeInfo){
        ...
        if typeInfo.FP.Get_extedType().FP.Get_kind() != Ast_TypeInfoKind__FormFunc{
            if name != nil{
                name_6479 := name.(string)
                self.FP.outputSymbol(&convGo_SymbolKind__Func{typeInfo}, name_6479)
            }
        }
    }
    self.FP.Write("(")
...
func (self *convGo_convFilter) ProcessForsort(node *Nodes_ForsortNode,_opt LnsAny) {
    ...
    if keySym != nil{
        keySym_7597 := keySym.(*Ast_SymbolInfo)
        key = Lns_getVM().String_format("%s", []LnsAny{self.FP.getSymbolSym(keySym_7597)})
    ...
    self.FP.PushIndent(nil)
    if valSym != nil{
        valSym_7599 := valSym.(*Ast_SymbolInfo)
        if Lns_isCondTrue( valSym_7599.FP.Get_posForModToRef()){
            self.FP.Write(Lns_getVM().String_format("%s := %s.Items[ _%s ]", []LnsAny{self.FP.getSymbolSym(valSym_7599), collSym, key}))
            self.FP.outputStem2Type(valSym_7599.FP.Get_typeInfo())
            self.FP.Writeln("")
        }
    }
    if keySym != nil{
        keySym_7602 := keySym.(*Ast_SymbolInfo)
        if Lns_isCondTrue( keySym_7602.FP.Get_posForModToRef()){
            self.FP.Write(Lns_getVM().String_format("%s := _%s", []LnsAny{key, key}))
            self.FP.outputStem2Type(keySym_7602.FP.Get_typeInfo())
            self.FP.Writeln("")
        }
    }
    self.FP.PopIndent()
...
func (self *convGo_convFilter) outputConvItemType(typeInfo *Ast_TypeInfo,alt2type LnsAny) {
    ...
    if typeInfo.FP.Get_srcTypeInfo().FP.Get_nonnilableType().FP.Get_kind() == Ast_TypeInfoKind__Alternate{
        if alt2type != nil{
            alt2type_7811 := alt2type.(*LnsMap)
            {
                _alt := alt2type_7811.Items[workTypeInfo]
                if _alt != nil {
...
func (self *convGo_convFilter) OutputCallPrefix(threading bool,callId LnsInt,node *Nodes_Node,prefixNode LnsAny,funcSymbol *Ast_SymbolInfo)(bool, LnsAny) {
    ...
    if prefixNode != nil{
        prefixNode_7972 := prefixNode.(*Nodes_Node)
        extCallFlag = prefixNode_7972.FP.Get_expType().FP.Get_nonnilableType().FP.Get_kind() == Ast_TypeInfoKind__Ext
    ...
    closeParen = false
    if prefixNode != nil{
        prefixNode_7997 := prefixNode.(*Nodes_Node)
        ...
        var prefixType *Ast_TypeInfo
        prefixType = prefixNode_7997.FP.Get_expType().FP.Get_nonnilableType()
        if prefixType == Ast_builtinTypeString{
            ...
            }
            {
                ...
                if _runtime != nil {
                    ...
                    callKind = &convGo_CallKind__RuntimeCall{prefixNode_7997}
            ...
            if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
                ...
                if node.FP.HasNilAccess(){
                    if Lns_op_not(prefixNode_7997.FP.HasNilAccess()){
                        ...
                        convGo_filter_1177_(prefixNode_7997, self, node)
                        self.FP.Writeln(") && ")
                    } else { 
                        convGo_filter_1177_(prefixNode_7997, self, node)
                        self.FP.Writeln("&&")
...
func (self *convGo_convFilter) ProcessExpCall(node *Nodes_ExpCallNode,_opt LnsAny) {
    ...
    if closeTxt != nil{
        closeTxt_8089 := closeTxt.(string)
        self.FP.Write(closeTxt_8089)
    }
    self.FP.Write(")")
    if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
