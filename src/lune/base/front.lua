...
local function createModuleInfo( ast, mod, moduleId )
   ...
   local rootNode = _lune.unwrap( _lune.__Cast( ast:get_node(), 3, Nodes.RootNode ))
   for __index, node in ipairs( rootNode:get_nodeManager():getAliasNodeList(  ) ) do
      importedAliasMap[node:get_typeInfo()] = _lune.__Cast( node:get_expType(), 3, Ast.AliasTypeInfo )
   end
   
   local exportInfo = ast:get_exportInfo()
   return frontInterface.ModuleInfo.new(mod, exportInfo:get_moduleTypeInfo():get_rawTxt(), {}, moduleId, exportInfo, importedAliasMap)
end
...
function ModuleMgr:add( ast, moduleInfo )
   ...
   if self.loadedMetaMap[mod] then
      if mod == "lune.base.Testing" then
         
      else
       
         Util.err( string.format( "the meta is already loaded -- %s", mod) )
      end
      
   end
   
   self.mod2info:add( mod, _lune.newAlge( UptodateInfo.Update, {moduleInfo,ast}) )
   self.loadedMetaMap[mod] = frontInterface.ModuleMeta.new(ast:get_streamName(), _lune.newAlge( frontInterface.MetaOrModule.Module, {moduleInfo}))
end
function ModuleMgr:addMeta( mod, meta )

   if not self:get( mod ) then
      self.mod2info:add( mod, _lune.newAlge( UptodateInfo.Uptodate, {meta}) )
   end
...
local AstCreater = {}
function AstCreater:createAst( importModuleInfo, parserSrc, analyzeModule, analyzeMode, pos )

   local transUnit = TransUnit.TransUnit.new(self.moduleId, importModuleInfo, convLua.MacroEvalImp.new(self.builtinFunc), analyzeModule, analyzeMode, pos, self.option.targetLuaVer, self.option.transCtrlInfo, self.builtinFunc)
   
   return transUnit:createAST( parserSrc, false, self.mod )
end
function AstCreater.new( importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos, builtinFunc, option )
   local obj = {}
   AstCreater.setmeta( obj )
   if obj.__init then obj:__init( importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos, builtinFunc, option ); end
   return obj
end
function AstCreater:__init(importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos, builtinFunc, option) 
   self.option = option
   ...
   self.converter = function (  )
      ...
      local ast = self:createAst( importModuleInfo, parserSrc, analyzeModule, analyzeMode, pos )
      self.ast = ast
...
function AstCreater:getAst(  )
   ...
   return _lune.unwrap( self.ast), _lune.unwrap( self.moduleInfo)
end
...
function Front:createAstSub( importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos )
   ...
   local astCreater = AstCreater.new(importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos, self.builtinFunc, self.option)
   self.mod2astCreate[mod] = astCreater
   return _lune.newAlge( CreateAstResult.Creater, {astCreater})
end


function Front:processAstCreater( astCreater )

   local ast, moduleInfo = astCreater:getAst(  )
...
function Front:createAst( importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos )

   do
      local _matchExp = self:createAstSub( importModuleInfo, parserSrc, mod, moduleId, analyzeModule, analyzeMode, pos )
      ...
      elseif _matchExp[1] == CreateAstResult.Creater[1] then
         ...
         return self:processAstCreater( astCreater )
      end
   end
...
local function ast2LuaMain( ast, streamName, stream, metaStream, convMode, inMacro, option )
   ...
   local exportInfo = ast:get_exportInfo()
   local conv = convLua.createFilter( streamName, stream, metaStream, convMode, inMacro, exportInfo:get_moduleTypeInfo(), exportInfo:get_processInfo(), exportInfo:get_provideInfo():get_symbolKind(), ast:get_builtinFunc(), option.useLuneModule, option.targetLuaVer, option.testing, option.useIpairs )
   return conv
end
...
function Front:loadFromLnsTxt( importModuleInfo, name, txt )
   ...
   local _
   local transUnit = TransUnit.TransUnit.new(frontInterface.ModuleId.tempId, importModuleInfo, convLua.MacroEvalImp.new(self.builtinFunc), nil, nil, nil, self.option.targetLuaVer, self.option.transCtrlInfo, self.builtinFunc)
   
   local ast = transUnit:createAST( _lune.newAlge( Parser.ParserSrc.LnsCode, {txt,name}), false, string.format( "$load%d", self.loadCount) )
   self.loadCount = self.loadCount + 1
   
   local _357, luaTxt = self:convertFromAst( ast, name, convLua.ConvMode.ConvMeta )
   ...
   if  nil == foundPath then
      ...
      do
         ...
         if _exp ~= nil then
            ...
            Log.log( Log.Level.Err, __func__, 576, function (  )
...
function Front:getModuleIdAndCheckUptodate( lnsPath, mod )
   ...
   local function checkDependUptodate( metaTime, metaInfo, metaCode )
      ...
      for depMod, dependItem in pairs( metaInfo.__dependModuleMap ) do
         ...
         if  nil == modMetaPath then
            ...
            Log.log( Log.Level.Debug, __func__, 672, function (  )
         ...
         if  nil == time then
            ...
            Log.log( Log.Level.Debug, __func__, 677, function (  )
         ...
         if time > metaTime then
            ...
            if  nil == dependMeta then
               ...
               Log.log( Log.Level.Debug, __func__, 685, function (  )
            ...
            if metaModuleId:get_buildCount() ~= 0 and metaModuleId:get_buildCount() ~= orgMetaModuleId:get_buildCount() then
               ...
               Log.log( Log.Level.Debug, __func__, 695, function (  )
   ...
   if metaInfo ~= nil then
      ...
      Log.log( Log.Level.Debug, __func__, 733, function (  )
...
function Front:convertLns2LuaCode( importModuleInfo, parserSrc, stream, streamName )

   local _
   local mod = self:scriptPath2Module( streamName )
   local ast = self:createAst( importModuleInfo, parserSrc, mod, frontInterface.ModuleId.createId( 0.0, 0 ), nil, TransUnit.AnalyzeMode.Compile )
   
   local _523, luaTxt = self:convertFromAst( ast, streamName, convLua.ConvMode.ConvMeta )
   
   return luaTxt
end


function Front:loadParserToLuaCode( importModuleInfo, parserSrc, path, mod )
   local __func__ = '@lune.@base.@front.Front.loadParserToLuaCode'

   
   local moduleId = getModuleId( path, mod )
   local ast = self:createAst( importModuleInfo, parserSrc, mod, moduleId, nil, TransUnit.AnalyzeMode.Compile, nil )
   
   local metaTxt, luaTxt = self:convertFromAst( ast, path, convLua.ConvMode.ConvMeta )
   Log.log( Log.Level.Trace, __func__, 778, function (  )
   ...
   local meta = frontInterface.ModuleMeta.new(path, _lune.newAlge( frontInterface.MetaOrModule.Module, {createModuleInfo( ast, mod, moduleId )}))
   return meta, luaTxt
end
...
function Front:loadFile( importModuleInfo, path, mod )
   ...
   Log.log( Log.Level.Info, __func__, 832, function (  )
   ...
   local meta, luaTxt = self:loadParserToLuaCode( importModuleInfo, _lune.newAlge( Parser.ParserSrc.LnsPath, {path,mod}), path, mod )
...
function Front:checkUptodateMeta( lnsPath, metaPath, addSearchPath )
   ...
   if  nil == metaObj then
      ...
      Log.log( Log.Level.Warn, __func__, 863, function (  )
   ...
   if meta.__formatVersion ~= Ver.metaVersion then
      Log.log( Log.Level.Warn, __func__, 868, function (  )
   ...
   if meta.__hasTest then
      ...
      if meta.__enableTest ~= self.option.testing then
         Log.log( Log.Level.Warn, __func__, 874, function (  )
   ...
   for moduleFullName, _606 in pairs( meta.__dependModuleMap ) do
      do
         ...
         if moduleLnsPath ~= nil then
            do
               ...
               if moduleLuaPath ~= nil then
                  if not Util.getReadyCode( moduleLnsPath, metaPath ) then
                     ...
                     Log.log( Log.Level.Warn, __func__, 885, function (  )
                  ...
                  if Depend.existFile( moduleMetaPath ) and not Util.getReadyCode( moduleMetaPath, metaPath ) then
                     Log.log( Log.Level.Warn, __func__, 893, function (  )
                  ...
                  Log.log( Log.Level.Warn, __func__, 898, function (  )
            ...
            Log.log( Log.Level.Warn, __func__, 903, function (  )
   ...
   return frontInterface.ModuleMeta.new(lnsPath, _lune.newAlge( frontInterface.MetaOrModule.Meta, {meta}))
end
            ...
            do
               ...
               if lnsPath ~= nil then
                  ...
                  if _lune._Set_has(self.bindModuleSet, mod ) then
                     Log.log( Log.Level.Warn, __func__, 970, function (  )
                     
                        return string.format( "load from the binding -- %s", mod)
                     end )
                     
                     local workMod = require( mod )
                     local meta = frontInterface.ModuleMeta.new(mod:gsub( "%.", "/" ) .. ".lns", _lune.newAlge( frontInterface.MetaOrModule.Meta, {_lune.unwrap( loadFromLuaTxt( "return {}" ))}))
...
function Front:loadMeta( importModuleInfo, mod )
   ...
   do
      local creater = self.mod2astCreate[mod]
      if creater ~= nil then
         local ast, moduleInfo = creater:getAst(  )
         return frontInterface.ModuleMeta.new(ast:get_streamName(), _lune.newAlge( frontInterface.MetaOrModule.Module, {moduleInfo}))
      end
   end
   
   
   if not self.moduleMgr:getMeta( mod ) then
      do
         local _exp = self:getLoadInfo( mod )
         if _exp ~= nil then
            self.moduleMgr:addMeta( mod, _exp.meta )
         else
            do
               local lnsPath = self:searchModule( orgMod )
               if lnsPath ~= nil then
                  ...
                  if not _lune._Set_has(self.targetSet, mod ) then
                     local luaPath = string.gsub( lnsPath, "%.lns$", ".lua" )
                     do
                        local dir = self.option.outputDir
                        if dir ~= nil then
                     ...
                     if luaPath ~= nil then
                        ...
                        if not forceFlag then
                           if Util.getReadyCode( lnsPath, luaPath ) then
                              ...
                              Log.log( Log.Level.Warn, __func__, 1050, function (  )
                           ...
                           Log.log( Log.Level.Warn, __func__, 1054, function (  )
                        ...
                        Log.log( Log.Level.Warn, __func__, 1058, function (  )
                  ...
                  if meta ~= nil then
                     ...
                     local metawork, luaTxt = self:loadParserToLuaCode( importModuleInfo, _lune.newAlge( Parser.ParserSrc.LnsPath, {lnsPath,orgMod}), lnsPath, orgMod )
                     self.moduleMgr:addMeta( mod, metawork )
                  ...
                  do
                     ...
                     if lnsCode ~= nil then
                        ...
                        local meta, luaTxt = self:loadParserToLuaCode( importModuleInfo, _lune.newAlge( Parser.ParserSrc.LnsCode, {lnsCode,mod}), path, mod )
                        self.moduleMgr:addMeta( mod, meta )
                        self.convertedMap[mod] = luaTxt
                     end
                  end
...
function Front:dumpAst( scriptPath )
   ...
   Depend.profile( self.option.validProf, function (  )
      ...
      local ast = self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, getModuleId( scriptPath, mod ), nil, TransUnit.AnalyzeMode.Compile )
      ast:get_node():processFilter( dumpNode.createFilter( ast:get_exportInfo():get_moduleTypeInfo(), ast:get_exportInfo():get_processInfo(), io.stdout ), dumpNode.Opt.new("", 0) )
...
function Front:format( scriptPath )
   ...
   local ast = self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, getModuleId( scriptPath, mod ), nil, TransUnit.AnalyzeMode.Compile )
   ast:get_node():processFilter( Formatter.createFilter( ast:get_exportInfo():get_moduleTypeInfo(), io.stdout ), Formatter.Opt.new(ast:get_node()) )
end
...
function Front:checkDiag( scriptPath )
   ...
   local mod = self:scriptPath2Module( scriptPath )
   Util.setErrorCode( 0 )
   self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, getModuleId( scriptPath, mod ), nil, TransUnit.AnalyzeMode.Diag )
end
...
function Front:complete( scriptPath )
   ...
   local mod = self:scriptPath2Module( scriptPath )
   self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, getModuleId( scriptPath, mod ), self.option.analyzeModule, TransUnit.AnalyzeMode.Complete, self.option.analyzePos )
end
...
function Front:inquire( scriptPath )
   ...
   local mod = self:scriptPath2Module( scriptPath )
   self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, getModuleId( scriptPath, mod ), self.option.analyzeModule, TransUnit.AnalyzeMode.Inquire, self.option.analyzePos )
end
...
function LuaConverter.new( luaPath, metaPath, dependsPath, ast, convMode, path, byteCompile, stripDebugInfo, option )
   local obj = {}
   LuaConverter.setmeta( obj )
   if obj.__init then obj:__init( luaPath, metaPath, dependsPath, ast, convMode, path, byteCompile, stripDebugInfo, option ); end
   return obj
end
function LuaConverter:__init(luaPath, metaPath, dependsPath, ast, convMode, path, byteCompile, stripDebugInfo, option) 
   ...
   self.rootNode = _lune.unwrap( _lune.__Cast( ast:get_node(), 3, Nodes.RootNode ))
   ...
   self.converterFunc = function (  )
      ...
      self.filterInfo:outputLua( self.rootNode )
      return nil
   end
   self:start(  )
end
...
function LuaConverter:saveLua(  )
   ...
   self:join(  )
   self.filterInfo:outputMeta( self.rootNode )
...
local function outputDependInfo( stream, metaInfo, mod )
   ...
   if stream ~= nil then
      ...
      for dependMod, _937 in pairs( metaInfo.__dependModuleMap ) do
...
function Front:convertToLua( scriptPath, convMode, streamLua, streamMeta )
   ...
   local ast = self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, moduleId, nil, TransUnit.AnalyzeMode.Compile, nil )
...
function GoConverter.new( scriptPath, ast, option, goOpt )
   local obj = {}
   GoConverter.setmeta( obj )
   if obj.__init then obj:__init( scriptPath, ast, option, goOpt ); end
   return obj
end
function GoConverter:__init(scriptPath, ast, option, goOpt) 
   Runner.Runner.__init( self)
...
function Front:saveToGo( scriptPath, ast )
   ...
   return GoConverter.new(scriptPath, ast, self.option, self:createGoOption( scriptPath ))
end
...
function Front:outputBuiltin( scriptPath )
   ...
   local ast = self:createAst( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsCode, {"",mod}), mod, frontInterface.ModuleId.createId( 0.0, 0 ), nil, TransUnit.AnalyzeMode.Compile )
   ...
   do
      ...
      if _matchExp[1] == ModuleUptodate.NeedUpdate[1] then
         ...
         local function process( ast )
            ...
            local luaConv = LuaConverter.new(luaPath, metaPath, dependsPath, ast, convMode, scriptPath, self.option.byteCompile, self.option.stripDebugInfo, self.option)
            ...
            do
               ...
               elseif _switchExp == Types.Lang.Go then
                  goConv = self:saveToGo( scriptPath, ast )
               end
            end
         ...
         local result = self:createAstSub( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsPath, {scriptPath,mod}), mod, moduleId, nil, TransUnit.AnalyzeMode.Compile )
         ...
         do
            ...
            elseif _matchExp[1] == CreateAstResult.Creater[1] then
               ...
               return function (  )
                  ...
                  return process( self:processAstCreater( astCreater ) )
               end
            end
         end
...
local function convertLnsCode2LuaCode( lnsCode, path )
   ...
   return front:convertLns2LuaCode( frontInterface.ImportModuleInfo.new(), _lune.newAlge( Parser.ParserSrc.LnsCode, {lnsCode,path}), Parser.TxtStream.new(lnsCode), path )
end
   ...
   local function process( oneShot, updateInfo )
      ...
      do
         ...
         elseif _matchExp[1] == BuildMode.CreateAst[1] then
            ...
            if not self.moduleMgr:getAst( mod ) then
               self:convertToLua( updateInfo:get_scriptPath(), convLua.ConvMode.ConvMeta, Util.NullOStream.new(), Util.NullOStream.new() )
            end
   ...
   Depend.profile( self.option.validProf, function (  )
      ...
      if self.option.scriptPath == "@-" then
         ...
         for __index, path in ipairs( self.option.batchList ) do
            ...
            print( string.format( "%s: done %g", updateInfo:get_scriptPath(), os.clock(  ) - prev) )
         end
         
         local subProcessList = {}
         for __index, postProcess in ipairs( postProcessList ) do
            do
               local subProcess = postProcess(  )
               if subProcess ~= nil then
                  table.insert( subProcessList, subProcess )
               end
            end
            
         end
         
         for __index, subProcess in ipairs( subProcessList ) do
            subProcess(  )
         end
      ...
      if astCallback ~= nil then
         for __index, key in ipairs( self.moduleMgr:getModList(  ) ) do
            do
               local _exp = self.moduleMgr:getAst( key )
               if _exp ~= nil then
                  astCallback( _exp )
               end
            end
...
function Front:exec(  )
   ...
   Log.log( Log.Level.Trace, __func__, 1854, function (  )
