...
function Front:loadFromLnsTxt( importModuleInfo, name, txt )
   ...
   local _5679, luaTxt = self:convertFromAst( ast, name, convLua.ConvMode.Exec )
...
function Front:checkUptodateMeta( metaPath, addSearchPath )
   ...
   for moduleFullName, _5842 in pairs( meta.__dependModuleMap ) do
      do
   ...
   local function outputDependInfo( stream, metaInfo )
      ...
      if stream ~= nil then
         if metaInfo ~= nil then
            ...
            for dependMod, _5939 in pairs( metaInfo.__dependModuleMap ) do
