...
function Front:loadFromLnsTxt( importModuleInfo, name, txt )
   ...
   local _5520, luaTxt = self:convertFromAst( ast, name, convLua.ConvMode.Exec )
      ...
      for depMod, dependItem in pairs( metaInfo.__dependModuleMap ) do
         ...
         if time > metaTime then
            ...
            if  nil == dependMeta then
               ...
               Log.log( Log.Level.Debug, __func__, 410, function (  )
...
function Front:convertLns2LuaCode( importModuleInfo, stream, streamName )
   ...
   local _5627, luaTxt = self:convertFromAst( ast, streamName, convLua.ConvMode.Exec )
...
function Front:checkUptodateMeta( metaPath, addSearchPath )
   ...
   for moduleFullName, _5683 in pairs( meta.__dependModuleMap ) do
      do
   ...
   if self.loadedMetaMap[mod] == nil then
      do
         ...
         if _exp ~= nil then
            ...
            Log.log( Log.Level.Info, __func__, 680, function (  )
   ...
   local function outputDependInfo( stream, metaInfo )
      ...
      if stream ~= nil then
         if metaInfo ~= nil then
            ...
            for dependMod, _5780 in pairs( metaInfo.__dependModuleMap ) do
   ...
   local function checkDiff( oldStream, newStream )
      ...
      while true do
         ...
         if newLine ~= oldLine then
            ...
            if not cont then
               Log.log( Log.Level.Debug, __func__, 1064, function (  )
...
function Front:exec(  )
   ...
   Log.log( Log.Level.Trace, __func__, 1278, function (  )
