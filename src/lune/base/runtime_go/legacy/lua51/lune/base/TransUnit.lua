...
function TransUnit.new( moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info, builtinFunc )
   local obj = {}
   TransUnit.setmeta( obj )
   if obj.__init then obj:__init( moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info, builtinFunc ); end
   return obj
end
function TransUnit:__init(moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info, builtinFunc) 
...
function TransUnit:getNSInfo( typeInfo )
...
function TransUnit:getNSType( typeInfo )

   return self:getNSInfo( typeInfo ):get_typeInfo()
end
...
function TransUnit:getCurrentNSInfo(  )
...
function TransUnit:analyzeBlock( blockKind, tentativeMode, scope, refAccessSymPosList )
   ...
   do
      ...
      if _switchExp == Nodes.BlockKind.For or _switchExp == Nodes.BlockKind.Apply or _switchExp == Nodes.BlockKind.While or _switchExp == Nodes.BlockKind.Repeat or _switchExp == Nodes.BlockKind.Foreach then
         loopFlag = true
         table.insert( nsInfo:get_loopScopeQueue(), self.scope )
      end
   end
...
function TransUnit:analyzeAsyncLock( asyncToken )
   ...
   return Nodes.AsyncLockNode.create( self.nodeManager, asyncToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, block )
end
   ...
   do
      ...
      if aliasType ~= nil then
         ...
         if not _lune._Set_has(self.importModuleSet, aliasSrc:getModule(  ) ) and self.moduleType:get_parentInfo() ~= aliasSrc:getModule(  ):get_parentInfo() then
            self:addErrMess( symbolNode:get_pos(), string.format( "must import '%s' for this alias -- %s (%s,%s)", aliasSrc:getModule(  ):getFullName( self.typeNameCtrl, self.scope, false ), symbolNode:getSymbolInfo(  )[1]:get_name(), tostring( self.moduleType:get_typeId().id), tostring( aliasSrc:getModule(  ):get_typeId().id)) )
         end
...
function TransUnit:createAST( parserSrc, asyncParse, baseDir, stdinFile, macroFlag, moduleName )
   ...
   Log.log( Log.Level.Log, __func__, 543, function (  )
      ...
      return string.format( "%s start -- %s on %s", __func__, parser:getStreamName(  ), tostring( baseDir))
   end )
...
function TransUnit:analyzeDeclMacroSub( accessMode, firstToken, nameToken, macroScope, parentType, workArgList )
   ...
   local typeInfo = self.processInfo:createFuncAsync( false, false, macroScope, Ast.TypeInfoKind.Macro, parentType, false, false, true, accessMode, nameToken.txt, Ast.Async.Async, nil, argTypeList, retTypeList )
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _1, existSym = self.scope:addMacro( self.processInfo, nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _1, shadowing = self.scope:addEnum( self.processInfo, accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclForm( accessMode, firstToken )
   ...
   local formType = self.processInfo:createFuncAsync( false, false, nil, Ast.TypeInfoKind.FormFunc, self:getCurrentNamespaceTypeInfoMut(  ), false, false, true, accessMode, name.txt, self:getDefaultAsync( Ast.TypeInfoKind.FormFunc, self:getCurrentClass(  ), asyncMode ), nil, argTypeInfoList, retTypeList, false )
   
   local formSymbol, shadowing = self.scope:addForm( self.processInfo, name.pos, formType, accessMode )
   self:errorShadowing( name.pos, shadowing )
   
   local declFuncInfo = Nodes.DeclFuncInfo.new(Nodes.FuncKind.Form, nil, nil, false, name, formSymbol or shadowing, argList, false, accessMode, asyncMode, nil, retTypeList, retNodeList, false, false)
...
function TransUnit:addDefaultConstructor( pos, classTypeInfo, classScope, memberNodeList, methodNameSet, oldFlag )
   ...
   local initTypeInfo = self.processInfo:createFuncAsync( false, false, ctorScope, Ast.TypeInfoKind.Method, classTypeInfo, true, false, false, Ast.AccessMode.Pub, "__init", Ast.Async.Async, nil, argTypeList, {} )
   if oldFlag then
...
function TransUnit:addAccessor( memberNode, methodNameSet, classScope, classTypeInfo )
   ...
   if accessMode ~= Ast.AccessMode.None then
      if classScope:getTypeInfoChild( getterName ) then
         ...
         local retTypeInfo = self.processInfo:createFuncAsync( false, false, self:pushScope( false ), typeKind, classTypeInfo, false, false, memberNode:get_staticFlag(), accessMode, getterName, asyncMode, nil, {}, {getterMemberType} )
         self:popScope(  )
   ...
   if memberNode:get_setterMode() ~= Ast.AccessMode.None then
      if classScope:getTypeInfoChild( setterName ) then
         ...
         classScope:addMethod( self.processInfo, memberName.pos, self.processInfo:createFuncAsync( false, false, self:pushScope( false ), typeKind, classTypeInfo, false, false, memberNode:get_staticFlag(), accessMode, setterName, asyncMode, nil, {memberType}, nil, mutable ), accessMode, memberNode:get_staticFlag(), true )
         self:popScope(  )
...
function TransUnit:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleLang, moduleName, lazyLoad, nextToken, inheritInfo )
   ...
   local function processInitBlock( token )
      ...
      local funcTypeInfo = self.processInfo:createFuncAsync( false, false, initBlockScope, Ast.TypeInfoKind.Func, classTypeInfo, false, false, true, Ast.AccessMode.Pri, ininame, Ast.Async.Noasync, nil, nil, nil, false )
      ...
      local info = Nodes.DeclFuncInfo.new(Nodes.FuncKind.InitBlock, classTypeInfo, node, true, token, _lune.unwrap( (funcSym or shadowing )), {}, true, Ast.AccessMode.Pri, nil, block, {}, {}, false, false)
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   if classTypeInfo:isInheritFrom( self.processInfo, Ast.builtinTypeMapping, nil ) then
      ...
      local toMapFuncTypeInfo = self.processInfo:createFuncAsync( false, false, nil, Ast.TypeInfoKind.Method, classTypeInfo, true, false, false, Ast.AccessMode.Pub, "_toMap", Ast.Async.Async, nil, {}, {mapType}, false )
   ...
   local node, _1, methodNameSet = self:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleLang, moduleName, lazyLoad, nextToken, inheritInfo )
   ...
   if classTypeInfo:isInheritFrom( self.processInfo, Ast.builtinTypeMapping, nil ) then
      ...
      local fromMapFuncTypeInfo = self.processInfo:createFuncAsync( false, false, nil, Ast.TypeInfoKind.Func, classTypeInfo, true, false, true, Ast.AccessMode.Pub, "_fromMap", Ast.Async.Async, nil, {mapType:get_nilableTypeInfo()}, {classTypeInfo:get_nilableTypeInfo(), Ast.builtinTypeString:get_nilableTypeInfo()}, true )
      classScope:addMethod( self.processInfo, nil, fromMapFuncTypeInfo, ctorAccessMode, true, false )
      local fromStemFuncTypeInfo = self.processInfo:createFuncAsync( false, false, nil, Ast.TypeInfoKind.Func, classTypeInfo, true, false, true, Ast.AccessMode.Pub, "_fromStem", Ast.Async.Async, nil, {Ast.builtinTypeStem_}, {classTypeInfo:get_nilableTypeInfo(), Ast.builtinTypeString:get_nilableTypeInfo()}, true )
...
function TransUnit:analyzeDeclFunc( declFuncMode, asyncLocked, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   do
      local workTypeInfo = self.processInfo:createFuncAsync( abstractFlag, false, funcBodyScope, typeKind, namespaceInfo, false, false, staticFlag, accessMode, funcName, self:getDefaultAsync( typeKind, classTypeInfo or self:getCurrentClass(  ), asyncMode ), altTypeList, argTypeList, retTypeInfoList, mutable )
   ...
   local function createDeclFuncInfo( funcKind )
      ...
      return Nodes.DeclFuncInfo.new(funcKind, classTypeInfo, classDeclNode, outsizeOfClass, name, funcSym, argList, orgStaticFlag, accessMode, asyncMode, body, retTypeInfoList, retTypeNodeList, _lune._Set_has(self.has__func__Symbol, typeInfo ), overrideFlag)
   end
...
function TransUnit:analyzeDeclVar( mode, accessMode, firstToken )
            ...
            do
               ...
               if declNode ~= nil then
                  if not declNode:get_declInfo():get_name() then
                     ...
                     local newTypeInfo = self.processInfo:createFuncAsync( typeInfo:get_abstractFlag(), false, _lune.unwrap( self.namespace2Scope[typeInfo]), typeInfo:get_kind(), self:getNSType( typeInfo:get_parentInfo() ), false, false, typeInfo:get_staticFlag(), accessMode, letVarInfo.varName.txt, typeInfo:get_asyncMode(), typeInfo:get_itemTypeInfoList(), typeInfo:get_argTypeInfoList(), typeInfo:get_retTypeInfoList(), Ast.TypeInfo.isMut( typeInfo ) )
   ...
   for index, letVarInfo in ipairs( letVarList ) do
      ...
      table.insert( symbolInfoList, nsInfo:registerSym( _lune.unwrap( self.scope:getSymbolInfo( varName.txt, self.scope, true, self.scopeAccess )) ) )
   end
...
function TransUnit:checkMatchValType( pos, funcTypeInfo, expList, genericTypeList, genericsClass )
   ...
   do
      ...
      elseif _switchExp == self.builtinFunc.list_sort then
         ...
         local callback = self.processInfo:createFuncAsync( false, false, nil, Ast.TypeInfoKind.Func, self.processInfo:get_dummyParentType(), false, false, true, Ast.AccessMode.Pri, "sort", Ast.Async.Async, nil, {genericTypeList[1], genericTypeList[1]}, {Ast.builtinTypeBool}, false )
   ...
   local matchResult, _1, newExpNodeList = self:checkMatchType( funcTypeInfo:getTxt(  ), pos, argTypeList, expList, false, warnForFollow, alt2typeMap )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
      ...
      local _2, _3, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:checkAsyncField( symbolInfo, pos )
   ...
   if warn then
      if ((symbolInfo:get_staticFlag() and symbolInfo:get_kind() == Ast.SymbolKind.Mbr ) or symbolInfo:get_kind() == Ast.SymbolKind.Var ) and not _lune._Set_has(self.builtinFunc:get_allSymbolSet(), symbolInfo:getOrg(  ) ) and not self:canBeAsyncParam( symbolInfo:get_typeInfo() ) then
         self:addErrMess( pos, string.format( "can't access the mutable symbol(%s) from async (%s).", symbolInfo:get_name(), symbolInfo:get_typeInfo():getTxt(  )) )
      end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _1, _2, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _1, _2, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
   ...
   while true do
      ...
      elseif opToken.kind == Parser.TokenKind.Ope then
         if Parser.isOp2( opTxt ) then
            ...
            exp = Nodes.ExpOp2Node.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {retType}, opToken, self.nodeManager:MultiTo1( exp ), self.nodeManager:MultiTo1( exp2 ) )
         else
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _1, _2, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _1, _2, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
