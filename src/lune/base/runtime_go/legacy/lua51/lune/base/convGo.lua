...
function convFilter:processCondRet( node, opt )
   ...
   self:writeRaw( string.format( "_cond%d", node:get_order()) )
end
   ...
   for index, varSym in ipairs( node:get_varSymList() ) do
      if #node:get_expList():get_expTypeList() >= index then
         if varSym:get_name() == "_" then
            table.insert( needStrTempList, false )
         else
   ...
   if node:get_unwrapFlag() then
      do
         ...
         if expList ~= nil and  unwrapBlock ~= nil then
            ...
            local hasCond = false
            for index, varInfo in ipairs( node:get_varList() ) do
               if varInfo:get_name().txt ~= "_" and expList:getExpTypeNoDDDAt( index ):get_nilable() then
                  if hasCond then
                     self:writeRaw( " || " )
                  end
...
function convFilter:processDeclVar( node, opt )
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         ...
         local hasCond = false
         for index, varSym in ipairs( node:get_symbolInfoList() ) do
            if varSym:get_name() ~= "_" and expList:getExpTypeNoDDDAt( index ):get_nilable() then
               if hasCond then
                  self:writeRaw( " || " )
               end
...
function convFilter:processIf( node, opt )
   ...
   for __index, stmt in ipairs( node:get_stmtList() ) do
      do
         local _exp = stmt:get_condRetInfo()
         if _exp ~= nil then
...
function convFilter:processMatch( node, opt )
   ...
   for __index, caseInfo in ipairs( node:get_caseList() ) do
      ...
      for index, symbol in ipairs( caseInfo:get_valParamNameList() ) do
         if symbol:get_posForModToRef() then
            self:write( string.format( "%s := %s.Val%d", self:getSymbolSym( symbol ), val, index) )
            local orgParamType = valInfo:get_typeList()[index]:get_nonnilableType():get_srcTypeInfo()
            ...
            do
               ...
               if runtime ~= nil then
                  ...
                  do
                     ...
                     elseif _switchExp == self.builtinFuncs.lns___run or _switchExp == self.builtinFuncs.lns___join or _switchExp == self.builtinFuncs.lns___serr then
                        filter( node:get_func(), self, node )
                     else 
            ...
            elseif _switchExp == Ast.SymbolKind.Fun then
               if Ast.isBuiltin( node:get_expType():get_typeId().id ) then
                  ...
                  do
                     ...
                     elseif _switchExp == builtinFunc.lns___serr then
                        self:writeRaw( "LnsErr_create" )
                     else 
