...
function Front:createAstSub( importModuleInfo, parserSrc, baseDir, mod, moduleId, analyzeModule, analyzeMode, pos )
   ...
   local astCreater = Converter.AstCreater._new(importModuleInfo, parserSrc, mod, baseDir, moduleId, analyzeModule, analyzeMode, pos, self.builtinFunc, self.option)
...
function Front:applyAstResult( result )
   ...
   do
      ...
      elseif _matchExp[1] == Converter.CreateAstResult.Creater[1] then
         ...
         Log.log( Log.Level.Log, __func__, 344, function (  )
   ...
   if  nil == foundPath then
      ...
      do
         ...
         if _exp ~= nil then
            ...
            Log.log( Log.Level.Err, __func__, 532, function (  )
...
function Front:getModuleIdAndCheckUptodate( lnsPath, mod )
   ...
   local function checkDependUptodate( metaTime, metaInfo, metaCode )
      ...
      for depMod, dependItem in pairs( metaInfo.__dependModuleMap ) do
         ...
         if  nil == modMetaPath then
            ...
            Log.log( Log.Level.Debug, __func__, 629, function (  )
         ...
         if time > metaTime then
            ...
            if metaModuleId:get_buildCount() ~= 0 and metaModuleId:get_buildCount() ~= orgMetaModuleId:get_buildCount() then
               ...
               Log.log( Log.Level.Debug, __func__, 652, function (  )
   ...
   if metaInfo ~= nil then
      ...
      Log.log( Log.Level.Debug, __func__, 690, function (  )
...
function Front:loadParserToLuaCode( importModuleInfo, parserSrc, path, mod, baseDir )
   ...
   Log.log( Log.Level.Trace, __func__, 814, function (  )
...
function Front:loadFile( importModuleInfo, baseDir, path, mod )
   ...
   Log.log( Log.Level.Info, __func__, 896, function (  )
...
function Front:checkUptodateMeta( lnsPath, metaPath, baseDir, addSearchPath )
   ...
   do
      ...
      if meta.__formatVersion ~= Ver.metaVersion then
         Log.log( Log.Level.Warn, __func__, 940, function (  )
                  ...
                  do
                     ...
                     if moduleLuaPath ~= nil then
                        if not Util.getReadyCode( moduleLnsPath, metaPath ) then
                           ...
                           Log.log( Log.Level.Warn, __func__, 960, function (  )
                        ...
                        if Depend.existFile( moduleMetaPath ) and not Util.getReadyCode( moduleMetaPath, metaPath ) then
                           Log.log( Log.Level.Warn, __func__, 969, function (  )
...
function Front:loadModuleWithBaseDir( orgMod, baseDir )
   ...
   local _
   Log.log( Log.Level.Info, __func__, 1006, function (  )
   
      return string.format( "loadModule  -- %s:%s", orgMod, tostring( baseDir))
   end )
               ...
               if lnsPath ~= nil then
                  ...
                  if not _lune._Set_has(self.targetSet, orgMod ) then
                     ...
                     if luaPath ~= nil then
                        ...
                        if not forceFlag then
                           if Util.getReadyCode( lnsPath, luaPath ) then
                              ...
                              if Util.getReadyCode( lnsPath, metaPath ) then
                                 ...
                                 Log.log( Log.Level.Warn, __func__, 1178, function (  )
                              ...
                              Log.log( Log.Level.Warn, __func__, 1182, function (  )
                           ...
                           Log.log( Log.Level.Warn, __func__, 1186, function (  )
                        ...
                        Log.log( Log.Level.Warn, __func__, 1190, function (  )
         ...
         for __index, mod in ipairs( self.moduleMgr:getModList(  ) ) do
            do
               ...
               if _exp ~= nil then
                  ...
                  Log.log( Log.Level.Err, __func__, 1683, function (  )
...
function Front:executeLns( path, baseDir )
   ...
   Log.log( Log.Level.Debug, __func__, 1762, function (  )
...
function Front:exec(  )
   ...
   Log.log( Log.Level.Trace, __func__, 1797, function (  )
