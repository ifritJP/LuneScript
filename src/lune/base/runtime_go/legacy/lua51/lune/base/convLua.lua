...
function convFilter:outputMeta( node )
   ...
   do
      ...
      for __index, declClass in ipairs( node:get_nodeManager():getDeclClassNodeList(  ) ) do
         if declClass:get_lazyLoad() ~= Nodes.LazyLoad.Off and Ast.isPubToExternal( declClass:get_accessMode() ) then
            ...
            self:write( string.format( "%d", declClass:get_expType():get_typeId().id) )
         end
         ...
         if not typeInfo:get_externalFlag() then
            do
               ...
               if _switchExp == Ast.TypeInfoKind.IF or _switchExp == Ast.TypeInfoKind.Class or _switchExp == Ast.TypeInfoKind.Form or _switchExp == Ast.TypeInfoKind.FormFunc or _switchExp == Ast.TypeInfoKind.Alge or _switchExp == Ast.TypeInfoKind.Enum or _switchExp == Ast.TypeInfoKind.Map or _switchExp == Ast.TypeInfoKind.Set or _switchExp == Ast.TypeInfoKind.List or _switchExp == Ast.TypeInfoKind.Array or _switchExp == Ast.TypeInfoKind.Alternate or _switchExp == Ast.TypeInfoKind.Box then
                  ...
                  local imutType = self.processInfo:createModifier( typeInfo, Ast.MutMode.IMut )
            ...
            if classTypeInfo:get_accessMode() == Ast.AccessMode.Pub then
               ...
               for __index, memberNode in ipairs( _lune.unwrap( self.classId2MemberList[classTypeInfo:get_typeId()]) ) do
                  if Ast.isPubToExternal( memberNode:get_accessMode() ) then
                     ...
                     self:writeln( string.format( "  name='%s', staticFlag = %s, mutMode = %d,", memberName, tostring( memberNode:get_staticFlag()), memberNode:get_symbolInfo():get_mutMode()) .. string.format( "accessMode = %d, typeId = %s }", memberNode:get_accessMode(), serializeInfo:serializeId( memberTypeInfo:get_typeId() )) )
                           ...
                           do
                              ...
                              if symbolInfo:get_kind() == Ast.SymbolKind.Mbr or symbolInfo:get_kind() == Ast.SymbolKind.Var then
                                 if symbolInfo:get_accessMode() == Ast.AccessMode.Pub then
                                    ...
                                    self:writeln( string.format( "  name='%s', staticFlag = %s, ", fieldName, tostring( symbolInfo:get_staticFlag())) .. string.format( "accessMode = %d, typeId = %d }", symbolInfo:get_accessMode(), typeInfo:get_typeId().id) )
   ...
   for __index, macroDeclNode in ipairs( node:get_nodeManager():getDeclMacroNodeList(  ) ) do
      ...
      if declInfo:get_pubFlag() then
         ...
         for index, argNode in ipairs( declInfo:get_argList() ) do
            ...
            self:write( string.format( "{name=%q,typeId=%d}", argNode:get_name().txt, argNode:get_expType():get_typeId().id) )
         end
         ...
         do
            ...
            for __index, name in ipairs( __sorted ) do
               ...
               do
                  ...
                  self:write( string.format( "{name=%q,typeId=%d}", name, symInfo.typeInfo:get_typeId().id) )
         ...
         do
            ...
            if stmtBlock ~= nil then
               ...
               local workFilter = convFilter.new(declInfo:get_name().txt, memStream, memStream, ConvMode.Convert, false, Ast.headTypeInfo, self.processInfo, Ast.SymbolKind.Typ, self.useLuneRuntime, self.targetLuaVer, self.enableTest, self.useIpairs)
   ...
   do
      ...
      for __index, varName in ipairs( __sorted ) do
         ...
         do
            ...
            self:writeln( string.format( "  name='%s', accessMode = %d, typeId = %s, mutable = %s }", varName, varInfo.accessMode, serializeInfo:serializeId( varInfo.typeInfo:get_typeId() ), tostring( true)) )
   ...
   self:writeln( string.format( "_moduleObj.__moduleMutable = %s", tostring( Ast.TypeInfo.isMut( moduleTypeInfo ))) )
...
function convFilter:processProtoClass( node, opt )
   ...
   self:write( string.format( "local %s = {}", node:get_name().txt) )
end
      ...
      if node:get_nilAccess() then
         ...
         do
            ...
            elseif _switchExp == Ast.TypeInfoKind.Class then
               if prefixType:isInheritFrom( self.processInfo, Ast.builtinTypeMapping, nil ) and isGenericType( prefixType ) and (fieldNode:get_field().txt == "_fromMap" or fieldNode:get_field().txt == "_fromStem" ) then
                  wroteFuncFlag = true
                  setArgFlag = true
...
local function createFilter( streamName, stream, metaStream, convMode, inMacro, moduleTypeInfo, processInfo, moduleSymbolKind, useLuneRuntime, targetLuaVer, enableTest, useIpairs )
   ...
   return convFilter.new(streamName, stream, metaStream, convMode, inMacro, moduleTypeInfo, processInfo, moduleSymbolKind, useLuneRuntime, targetLuaVer, enableTest, useIpairs)
end
