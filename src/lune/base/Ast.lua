   ...
   if #expTypeList > 0 then
      for index, expType in ipairs( expTypeList ) do
         ...
         elseif #expTypeList == index then
            ...
            if not canEval then
               return MatchType.Error, string.format( "exp(%d) type mismatch %s(%d) <- %s(%d)%s", index, dstType:getTxt( _moduleObj.defaultTypeNameCtrl ), dstType:get_typeId(), expType:getTxt( _moduleObj.defaultTypeNameCtrl ), expType:get_typeId(), (evalMess and string.format( " -- %s", evalMess) or "" )
               )
            end
