...
function NormalTypeInfo:serialize( stream, serializeInfo )
  ...
  asyncMode = %d,     
]==], SerializeKind.Normal, parentId.id, self.typeId.id, serializeInfo:serializeId( self:get_baseId() ), self.rawTxt, self.finalFlag, self.abstractFlag, self.staticFlag, self.accessMode, self.kind, self.mutMode, self.asyncMode)
   do
