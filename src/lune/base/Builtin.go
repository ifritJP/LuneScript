...
// for 635
func Builtin_convExp0_15974(arg1 []LnsAny) LnsAny {
    return Lns_getFromMulti( arg1, 0 )
}
// for 627
func Builtin_convExp0_15855(arg1 []LnsAny) string {
    return Lns_getFromMulti( arg1, 0 ).(string)
}
// for 655
func Builtin_convExp0_16040(arg1 []LnsAny) string {
    return Lns_getFromMulti( arg1, 0 ).(string)
}
// for 197
func Builtin_convExp0_7926(arg1 []LnsAny) string {
    return Lns_getFromMulti( arg1, 0 ).(string)
}
// for 75
...
// for 630
func Builtin_convExp0_15878(arg1 []LnsAny) (LnsAny, LnsAny) {
    return Lns_getFromMulti( arg1, 0 ), Lns_getFromMulti( arg1, 1 )
}
// for 1069
func Builtin_convExp0_18085(arg1 []LnsAny) string {
    return Lns_getFromMulti( arg1, 0 ).(string)
}
// for 801
func Builtin_convExp0_16771(arg1 []LnsAny) []LnsAny {
    return Lns_2DDD( arg1[0:])
}
// for 929
func Builtin_convExp0_17458(arg1 []LnsAny) []LnsAny {
    return Lns_2DDD( arg1[0:])
}



// 978: decl @lune.@base.@Builtin.Builtin.registBuiltInScope.processCopyAlterList
...
// 522: decl @lune.@base.@Builtin.Builtin.getTypeInfo
func (self *Builtin_Builtin) getTypeInfo(_env *LnsEnv, typeName string,targetScope LnsAny) *Ast_TypeInfo {
        ...
        Builtin_getTypeInfo = func(_env *LnsEnv, workScope *Ast_Scope) LnsAny {
            ...
            if nameList.Len() <= 1{
                ...
                for _index, _name := range( nameList.Items ) {
                    ...
                    if workTypeInfo != nil{
                        workTypeInfo_709 := workTypeInfo.(*Ast_TypeInfo)
                        if index == nameList.Len(){
                            return workTypeInfo_709
                        }
                        {
                            __exp := workTypeInfo_709.FP.Get_scope(_env)
                            if !Lns_IsNil( __exp ) {
    ...
    if Lns_isCondTrue( Lns_car(_env.GetVM().String_find(typeName,"^&", nil, nil))){
        mutable = false
        typeName = Builtin_convExp0_15855(Lns_2DDD(_env.GetVM().String_gsub(typeName,"^&", "")))
    }
    var genTypeList *LnsList2_[*Ast_TypeInfo]
    genTypeList = NewLnsList2_[*Ast_TypeInfo]([]*Ast_TypeInfo{})
    var endIndex LnsAny
    _,endIndex = Builtin_convExp0_15878(Lns_2DDD(_env.GetVM().String_find(typeName,"[%w%.]+<", nil, nil)))
    var suffix string
    suffix = ""
    if endIndex != nil{
        endIndex_745 := endIndex.(LnsInt)
        var genTypeName string
        genTypeName = _env.GetVM().String_sub(typeName,endIndex_745 + 1, nil)
        for  {
            {
                _tailIndex := Builtin_convExp0_15974(Lns_2DDD(_env.GetVM().String_find(genTypeName,"[,>]", nil, nil)))
        ...
        typeName = _env.GetVM().String_sub(typeName,1, endIndex_745 - 1) + suffix
    }
    var typeInfo *Ast_TypeInfo
    ...
    if Lns_isCondTrue( Lns_car(_env.GetVM().String_find(typeName,"!$", nil, nil))){
        nilable = true
        orgTypeName = Builtin_convExp0_16040(Lns_2DDD(_env.GetVM().String_gsub(typeName,"!$", "")))
    } else { 
        nilable = false
    ...
    }
    {
        ...
        if !Lns_IsNil( __exp ) {
            ...
            if targetScope != nil{
                targetScope_761 := targetScope.(*Ast_Scope)
                {
                    __exp := Builtin_getTypeInfoFromScope(_env, targetScope_761, orgTypeName, genTypeList)
                    if !Lns_IsNil( __exp ) {
                        _exp := __exp.(*Ast_TypeInfo)
                        typeInfo = _exp
                    }
...
// 683: decl @lune.@base.@Builtin.Builtin.createGenType
...
// 708: decl @lune.@base.@Builtin.Builtin.processField
...
// 840: decl @lune.@base.@Builtin.Builtin.registClass
...
// 910: decl @lune.@base.@Builtin.Builtin.createBuiltinAlge
...
// 956: decl @lune.@base.@Builtin.Builtin.registBuiltInScope
            ...
            if Lns_op_not(builtinModuleName2Scope.Get(name)){
                ...
                }
                {
                    ...
                    for _, _orgFieldName := range( __forsortSorted0.Items ) {
                        ...
                        fieldName = Builtin_convExp0_18085(Lns_2DDD(_env.GetVM().String_gsub(orgFieldName,"<.*", "")))
...
// 1115: decl @lune.@base.@Builtin.Builtin.getBuiltInFuncType
...
func (self *Builtin_BuiltinFuncType) registerClass(_env *LnsEnv, classInfo *Ast_TypeInfo) {
    ...
    if classInfo.FP.Get_kind(_env) == Ast_TypeInfoKind__Box{
        ...
        if classInfo.FP.Get_parentInfo(_env) != Ast_headTypeInfo{
            fullname = Builtin_convExp0_7926(Lns_2DDD(_env.GetVM().String_gsub(Lns_car(_env.GetVM().String_gsub(classInfo.FP.GetParentFullName(_env, Ast_defaultTypeNameCtrl, nil, nil),"@", "")).(string),"%.", "_")))
        } else { 
            fullname = ""
        }
...
// 515: decl @lune.@base.@Builtin.BuiltinFuncType.isStrFormFunc
