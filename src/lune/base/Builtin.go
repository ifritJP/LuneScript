...
func (self *Builtin_Builtin) getTypeInfo(typeName string) *Ast_TypeInfo {
    ...
    suffix = ""
    if endIndex != nil{
        endIndex_3824 := endIndex.(LnsInt)
        var genTypeName string
        genTypeName = Lns_getVM().String_sub(typeName,endIndex_3824 + 1, nil)
        for  {
            {
        ...
        typeName = Lns_getVM().String_sub(typeName,1, endIndex_3824 - 1) + suffix
