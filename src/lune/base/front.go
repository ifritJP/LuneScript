...
func front_loadFromChunk_1114_(chunk LnsAny,err LnsAny) LnsAny {
    if err != nil{
        err_5740 := err.(string)
        Util_errorLog(err_5740)
    }
    if chunk != nil{
        chunk_5742 := chunk.(*Lns_luaValue)
        {
            _work := front_convExp773(Lns_2DDD(Lns_getVM().RunLoadedfunc(chunk_5742,Lns_2DDD([]LnsAny{}))[0]))
            if _work != nil {
                work := _work
                return work
            }
...
func front_byteCompileFromLuaTxt_1120_(txt string,stripDebugInfo bool) string {
    ...
    if chunk != nil{
        ...
        return Lns_getVM().String_dump(chunk_5757, stripDebugInfo)
    }
...
func front_getMetaInfo_1186_(lnsPath string,mod string,outdir LnsAny)(LnsAny, string, string) {
    ...
    if outdir != nil{
        outdir_5818 := outdir.(string)
        moduleMetaPath = Lns_getVM().String_format("%s/%s", []LnsAny{outdir_5818, Lns_car(Lns_getVM().String_gsub(mod,"%.", "/")).(string)})
...
func front_getModuleId_1192_(lnsPath string,mod string,outdir LnsAny,metaInfo LnsAny) *FrontInterface_ModuleId {
    ...
    if metaInfo != nil{
        metaInfo_5848 := metaInfo.(*front_MetaForBuildId)
        var buildId *FrontInterface_ModuleId
        buildId = metaInfo_5848.FP.CreateModuleId()
...
func Front_convertToLua___anonymous_1343_(stream LnsAny,metaStream LnsAny,dependStream LnsAny) {
    if dependStream != nil{
        dependStream_6250 := dependStream.(Lns_oStream)
        dependStream_6250.Close()
    }
}
...
func (self *front_Front) loadLua(path string) LnsAny {
    ...
    if chunk != nil{
        chunk_5692 := chunk.(*Lns_luaValue)
        return Lns_unwrap( Lns_car(Lns_getVM().RunLoadedfunc(chunk_5692,Lns_2DDD([]LnsAny{}))[0]))
    }
...
func (self *front_Front) searchModuleFile(mod string,suffix string,addPath LnsAny) LnsAny {
    ...
    if addPath != nil{
        addPath_5831 := addPath.(string)
        lnsSearchPath = Lns_getVM().String_format("%s/?%s;%s", []LnsAny{addPath_5831, suffix, Lns_package_path})
...
func (self *front_Front) getModuleIdAndCheckUptodate(lnsPath string,mod string)(*FrontInterface_ModuleId, LnsAny) {
    ...
    if metaInfo != nil{
        metaInfo_5898 := metaInfo.(*front_MetaForBuildId)
        if metaInfo_5898.__enableTest == self.option.Testing{
            var buildId *FrontInterface_ModuleId
            buildId = FrontInterface_ModuleId_createIdFromTxt(metaInfo_5898.__buildId)
            if buildId != FrontInterface_ModuleId__tempId{
                ...
                if lnsTime != nil && metaTime != nil{
                    lnsTime_5905 := lnsTime.(LnsReal)
                    metaTime_5906 := metaTime.(LnsReal)
                    if lnsTime_5905 == buildId.FP.Get_modTime(){
                        uptodate = checkDependUptodate(metaTime_5906, metaInfo_5898, metaCode)
            ...
            if _luaTxt != nil {
            ...
            } else {
                {
                    ...
                    if _lnsPath != nil {
                        ...
                        loadVal = nil
                        if luaPath != nil{
                            luaPath_6040 := luaPath.(string)
                            if Util_getReadyCode(lnsPath, luaPath_6040){
                                var metaPath string
                                metaPath = front_convExp3258(Lns_2DDD(Lns_getVM().String_gsub(luaPath_6040, "%.lua$", ".meta")))
                                if Util_getReadyCode(lnsPath, metaPath){
                                    {
                                        ...
                                        if _preLoadInfo != nil {
                                            ...
                                            loadVal = self.FP.loadLua(luaPath_6040)
            ...
            if __exp != nil {
            ...
            } else {
                {
                    ...
                    if _lnsPath != nil {
                        ...
                        meta = nil
                        if luaPath != nil{
                            luaPath_6074 := luaPath.(string)
                            if Util_getReadyCode(lnsPath, luaPath_6074){
                                var metaPath string
                                metaPath = front_convExp3556(Lns_2DDD(Lns_getVM().String_gsub(luaPath_6074, "%.lua$", ".meta")))
                                if Util_getReadyCode(lnsPath, metaPath){
                                    meta = self.FP.checkUptodateMeta(metaPath, self.option.OutputDir)
                                    
                                    if meta != nil{
                                        meta_6079 := meta
                                        self.loadedMetaMap.Set(mod,meta_6079)
                                    }
                                } else { 
                                ...
                                Log_log(Log_Level__Warn, __func__, 704, Log_CreateMessage(func() string {
                                    return Lns_getVM().String_format("%s not ready lua %s, %s", []LnsAny{mod, lnsPath, luaPath_6074})
                                }))
...
func (self *front_Front) ConvertLuaToStreamFromScript(parser LnsAny,moduleId *FrontInterface_ModuleId,uptodate LnsAny,convMode LnsInt,path string,mod string,byteCompile bool,stripDebugInfo bool,openOStream front_OpenOStreamForConvert_1317_,closeOStream LnsAny) LnsAny {
    ...
    outputDependInfo = func(stream LnsAny,metaInfo LnsAny) {
        if stream != nil{
            stream_6170 := stream.(Lns_oStream)
            if metaInfo != nil{
                metaInfo_6172 := metaInfo.(*front_MetaForBuildId)
                var dependInfo *OutputDepend_DependInfo
                dependInfo = NewOutputDepend_DependInfo(mod)
                for _dependMod, _ := range( metaInfo_6172.__dependModuleMap.Items ) {
                    dependMod := _dependMod.(string)
                    dependInfo.FP.AddImpotModule(dependMod)
                }
                for _, _subMod := range( metaInfo_6172.__subModuleMap.Items ) {
                    subMod := _subMod.(string)
                    dependInfo.FP.AddSubMod(subMod)
                }
                dependInfo.FP.Output(stream_6170)
            } else {
    ...
    case *front_ModuleUptodate__NeedUpdate:
        if stream != nil && metaStream != nil{
            stream_6196 := stream.(Lns_oStream)
            metaStream_6197 := metaStream.(Lns_oStream)
            var ast *TransUnit_ASTInfo
            ...
            if dependsStream != nil{
                dependsStream_6200 := dependsStream.(Lns_oStream)
                ast.FP.Get_node().FP.ProcessFilter(OutputDepend_createFilter(dependsStream_6200), 1)
            }
            var outStream Lns_oStream
            outStream = stream_6196
            var oMetaStream Lns_oStream
            oMetaStream = metaStream_6197
            ...
            if byteCompile{
                stream_6196.Write(front_byteCompileFromLuaTxt_1120_(byteStream.FP.Get_txt(), stripDebugInfo))
                if metaStream_6197 != stream_6196{
                    metaStream_6197.Write(front_byteCompileFromLuaTxt_1120_(byteMetaStream.FP.Get_txt(), true))
                }
            }
        } else {
    ...
    metaInfo := _exp4820.Val2
        ...
        if self.option.Mode == Option_ModeKind__SaveMeta{
            if metaStream != nil{
                metaStream_6214 := metaStream.(Lns_oStream)
                metaStream_6214.Write(metaCode)
            } else {
    ...
    if Str_isValidStrBuilder(){
        if streamDst != nil{
            streamDst_6225 := streamDst.(Lns_oStream)
            streamDst_6225.Write(streamMem.FP.Get_txt())
        }
        
        if metaStreamDst != nil{
            metaStreamDst_6227 := metaStreamDst.(Lns_oStream)
            metaStreamDst_6227.Write(metaStreamMem.FP.Get_txt())
        }
        
        if dependsStreamDst != nil{
            dependsStreamDst_6229 := dependsStreamDst.(Lns_oStream)
            dependsStreamDst_6229.Write(dependsStreamMem.FP.Get_txt())
        }
        
    }
    if closeOStream != nil{
        closeOStream_6231 := closeOStream.(front_CloseOStreamForConvert_1320_)
        closeOStream_6231(stream, metaStream, dependsStream)
    }
    return retAst
}
...
func (self *front_Front) convertToLua(scriptPath string) {
    ...
    if ast != nil{
        ast_6253 := ast.(*TransUnit_ASTInfo)
        if _switch5124 := self.option.ConvTo; _switch5124 == Types_Conv__Go {
            var conv *Nodes_Filter
            conv = ConvGo_createFilter(self.option.Testing, "stdout", Lns_io_stdout, ast_6253)
            ast_6253.FP.Get_node().FP.ProcessFilter(conv, ConvGo_Opt2Stem(NewConvGo_Opt(ast_6253.FP.Get_node())))
        }
    }
}
...
func (self *front_Front) SaveToLua(updateInfo *front_UpdateInfo) bool {
    ...
    checkDiff = func(oldStream *Parser_TxtStream,newStream *Parser_TxtStream)(bool, string) {
        ...
        for  {
            ...
            if oldLine != nil{
                oldLine_6340 := oldLine.(string)
                if len(oldBuildIdLine) == 0{
                    if Lns_isCondTrue( Lns_car(Lns_getVM().String_find(oldLine_6340,"^_moduleObj.__buildId", nil, nil))){
                        oldBuildIdLine = oldLine_6340
            ...
            if newLine != nil{
                newLine_6344 := newLine.(string)
                if len(newBuildIdLine) == 0{
                    if Lns_isCondTrue( Lns_car(Lns_getVM().String_find(newLine_6344,"^_moduleObj.__buildId", nil, nil))){
                        newBuildIdLine = newLine_6344
            ...
            if newLine != oldLine{
                ...
                if newLine != nil && oldLine != nil{
                    newLine_6350 := newLine.(string)
                    oldLine_6351 := oldLine.(string)
                    if Lns_isCondTrue( Lns_car(Lns_getVM().String_find(oldLine_6351,"^_moduleObj.__buildId", nil, nil))){
                        if Lns_isCondTrue( Lns_car(Lns_getVM().String_find(newLine_6350,"^_moduleObj.__buildId", nil, nil))){
                        ...
                        Lns_GetEnv().SetStackVal( Lns_car(Lns_getVM().String_find(oldLine_6351,"^__dependModuleMap.*buildId =", nil, nil))) &&
                        Lns_GetEnv().SetStackVal( Lns_car(Lns_getVM().String_find(newLine_6350,"^__dependModuleMap.*buildId =", nil, nil))) )){
                        var oldSub string
                        oldSub = front_convExp5811(Lns_2DDD(Lns_getVM().String_gsub(oldLine_6351,"buildId =.*", "")))
                        var newSub string
                        newSub = front_convExp5824(Lns_2DDD(Lns_getVM().String_gsub(newLine_6350,"buildId =.*", "")))
                        if oldSub == newSub{
                            cont = true
    ...
    if luaPath == scriptPath{
        ...
        ast = self.FP.ConvertLuaToStreamFromScript(parser, moduleId, uptodate, convMode, scriptPath, mod, self.option.ByteCompile, self.option.StripDebugInfo, front_OpenOStreamForConvert_1317_(func(mode LnsAny)(LnsAny, LnsAny, LnsAny) {
            ...
            if stream != nil{
                stream_6417 := stream.(Lns_oStream)
                stream_6417.Close()
            }
            if dependStream != nil{
                dependStream_6419 := dependStream.(Lns_oStream)
                dependStream_6419.Close()
            }
            if metaFileObj != nil{
                metaFileObj_6421 := metaFileObj.(Lns_luaStream)
                metaFileObj_6421.Flush()
                metaFileObj_6421.Seek("set", 0)
                var newMetaTxt string
                
                {
                    _newMetaTxt := metaFileObj_6421.Read("*a")
                    if _newMetaTxt == nil{
    ...
    if ast != nil{
        ast_6438 := ast.(*TransUnit_ASTInfo)
        if _switch6696 := self.option.ConvTo; _switch6696 == Types_Conv__C {
            self.FP.SaveToC(scriptPath, ast_6438)
        } else if _switch6696 == Types_Conv__Go {
            self.FP.SaveToGo(scriptPath, ast_6438)
        }
    }
    return updateFlag
}
...
func front_MetaForBuildId_LoadFromMeta_1183_(metaPath string)(LnsAny, LnsAny) {
    {
        ...
        if _fileObj != nil {
            ...
            fileObj.Close()
            if luaCode != nil{
                luaCode_5808 := luaCode.(string)
                var meta LnsAny
                meta = front_convExp1155(Lns_2DDD(front_MetaForBuildId__fromStem_1179_(Lns_getVM().ExpandLuavalMap(front_loadFromLuaTxt_1117_(luaCode_5808)),nil)))
                return meta, luaCode_5808
            }
        }
    }
    return nil, nil
