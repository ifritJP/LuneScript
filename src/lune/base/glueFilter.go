...
func glueFilter_getDeclFuncInfo_1022_(node *Nodes_Node) *Nodes_DeclFuncInfo {
    {
...
func glueFilter_getFuncName_1028_(name string) string {
    if name == "__free"{
        return "__gc"
    }
    return name
...
func (self *glueFilter_glueGenerator) outputPrototype(node *Nodes_Node) {
    var name string
    name = glueFilter_getFuncName_1028_(node.FP.Get_expType().FP.Get_rawTxt())
...
func (self *glueFilter_glueGenerator) outputUserPrototype(node *Nodes_Node,gluePrefix string) {
    ...
    self.FP.writeHeader(Lns_getVM().String_format("extern int %s%s( lua_State * pLua", []LnsAny{gluePrefix, glueFilter_getFuncName_1028_(expType.FP.Get_rawTxt())}))
    var declInfo *Nodes_DeclFuncInfo
    declInfo = glueFilter_getDeclFuncInfo_1022_(node)
...
func (self *glueFilter_glueGenerator) outputFuncReg(symbolName string,methodNodeList *LnsList) {
    ...
    for _, _node := range( methodNodeList.Items ) {
        node := _node.(Nodes_NodeDownCast).ToNodes_Node()
        {
            _nameToken := glueFilter_getDeclFuncInfo_1022_(node).FP.Get_name()
            if !Lns_IsNil( _nameToken ) {
                nameToken := _nameToken.(*Types_Token)
                var name string
                name = glueFilter_getFuncName_1028_(nameToken.Txt)
...
func (self *glueFilter_glueGenerator) outputMethod(node *Nodes_Node,gluePrefix string) {
    ...
    var name string
    {
        ...
        if !Lns_IsNil( __exp ) {
            ...
            name = gluePrefix + glueFilter_getFuncName_1028_(_exp.Txt)
            ...
            if !Lns_IsNil( _moduleName ) {
                ...
                moduleSymbolName = glueFilter_convExp1553(Lns_2DDD(Lns_getVM().String_gsub(moduleName.FP.GetExcludedDelimitTxt(),"%.", "_")))
                {
                    ...
                    if !Lns_IsNil( __exp ) {
                        ...
                        cFile = createFile(moduleSymbolName + "_glue.c")
                        var hFile Lns_oStream
                        hFile = createFile(moduleSymbolName + "_glue.h")
                        var glue *glueFilter_glueGenerator
                        glue = NewglueFilter_glueGenerator(cFile, hFile)
