...
function PythonConverter._new( scriptPath, astResult, option, pythonOpt )
   local obj = {}
   PythonConverter._setmeta( obj )
   if obj.__init then obj:__init( scriptPath, astResult, option, pythonOpt ); end
   return obj
end
function PythonConverter:__init(scriptPath, astResult, option, pythonOpt) 
   Runner.Runner.__init( self)
   
   self.validFlag = true
   
   local path = scriptPath:gsub( "%.lns$", ".py" )
