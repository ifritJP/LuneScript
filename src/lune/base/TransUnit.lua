...
function TransUnit:__init(macroEval, analyzeModule, mode, pos) 
   ...
   self.analyzeMode = _lune.unwrapDefault( mode, AnalyzeMode.Complete)
...
quotedChar2Code["'"] = 39
local _TypeInfo = {}
_moduleObj._TypeInfo = _TypeInfo
      ...
      do
         ...
         if _exp ~= nil then
            ...
            if atomInfo.kind == Ast.TypeInfoKind.Enum then
               local accessMode = _lune.unwrap( Ast.AccessMode:_from( atomInfo.accessMode ))
               ...
               local enumTypeInfo = Ast.NormalTypeInfo.createEnum( scope, _lune.unwrap( parentInfo), true, accessMode, atomInfo.txt, _lune.unwrap( typeId2TypeInfo[atomInfo.valTypeId]), name2EnumValInfo )
               ...
               if atomInfo.nilable then
                  ...
                  if newTypeInfo and (atomInfo.kind == Ast.TypeInfoKind.Class or atomInfo.kind == Ast.TypeInfoKind.IF ) then
                     ...
                     if atomInfo.kind == Ast.TypeInfoKind.Class or atomInfo.kind == Ast.TypeInfoKind.IF then
                        ...
                        local workTypeInfo = Ast.NormalTypeInfo.createClass( atomInfo.kind == Ast.TypeInfoKind.Class, atomInfo.abstructFlag, scope, baseInfo, interfaceList, parentInfo, true, Ast.AccessMode.Pub, atomInfo.txt )
                        newTypeInfo = workTypeInfo
                        typeId2Scope[atomInfo.typeId] = scope
                        typeId2TypeInfo[atomInfo.typeId] = workTypeInfo
                        parentScope:addClass( atomInfo.txt, workTypeInfo )
                        ...
                        if atomInfo.kind == Ast.TypeInfoKind.Func or atomInfo.kind == Ast.TypeInfoKind.Method then
                           ...
                           local accessMode = _lune.unwrap( Ast.AccessMode:_from( atomInfo.accessMode ))
                           local workParentScope = _lune.unwrap( typeId2Scope[atomInfo.parentId])
                           workParentScope:add( symbolKind, false, atomInfo.kind == Ast.TypeInfoKind.Func, atomInfo.txt, workTypeInfo, accessMode, atomInfo.staticFlag, false, true )
                           typeId2Scope[atomInfo.typeId] = scope
                        end
...
function TransUnit:analyzeImport( token )
   ...
   return Ast.ImportNode.create( self.nodeManager, token.pos, {Ast.builtinTypeNone}, modulePath, moduleTypeInfo )
end
...
function TransUnit:analyzeIf( token )
   ...
   self:pushback(  )
   local list = {}
   table.insert( list, Ast.IfStmtInfo.new("if", self:analyzeExp( false ), self:analyzeBlock( Ast.BlockKind.If )) )
   nextToken = self:getToken( true )
   if nextToken.txt == "elseif" then
      while nextToken.txt == "elseif" do
         table.insert( list, Ast.IfStmtInfo.new("elseif", self:analyzeExp( false ), self:analyzeBlock( Ast.BlockKind.Elseif )) )
         nextToken = self:getToken( true )
      end
   ...
   if nextToken.txt == "else" then
      table.insert( list, Ast.IfStmtInfo.new("else", self:createNoneNode( nextToken.pos ), self:analyzeBlock( Ast.BlockKind.Else )) )
   else
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if macroFlag then
      ...
      local token = self:getTokenNoErr(  )
      if token ~= Parser.getEofToken(  ) then
         Util.err( string.format( "%s:%d:%d:(%s) not eof -- %s", self.parser:getStreamName(  ), token.pos.lineNo, token.pos.column, Parser.TokenKind:_getTxt( token.kind)
         , token.txt) )
      end
...
function TransUnit:analyzeDecl( accessMode, staticFlag, firstToken, token )
   ...
   elseif token.txt == "fn" then
      return self:analyzeDeclFunc( DeclFuncMode.Func, abstructFlag, overrideFlag, accessMode, staticFlag, nil, firstToken, nil )
   elseif token.txt == "class" then
      return self:analyzeDeclClass( abstructFlag, accessMode, firstToken, DeclClassMode.Class )
...
function TransUnit:analyzeClassBody( classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
   ...
   local advertiseList = {}
   local trustList = {}
   local node = Ast.DeclClassNode.create( self.nodeManager, firstToken.pos, {classTypeInfo}, classAccessMode, name, gluePrefix, fieldList, moduleName, memberList, self.scope, initStmtList, advertiseList, trustList, {} )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   for __index, memberNode in pairs( node:get_memberList() ) do
      ...
      if accessMode ~= Ast.AccessMode.None and not self.scope:getTypeInfoChild( getterName ) then
      ...
      if memberNode:get_setterMode() ~= Ast.AccessMode.None and not self.scope:getTypeInfoChild( setterName ) then
   ...
   if not self.scope:getTypeInfoChild( "__init" ) then
      ...
      self:popScope(  )
      self.scope:addMethod( initTypeInfo, Ast.AccessMode.Pub, false, false )
            ...
            for __index, child in pairs( memberType:get_children() ) do
               if child:get_kind() == Ast.TypeInfoKind.Method and child:get_accessMode() ~= Ast.AccessMode.Pri and not child:get_staticFlag() then
                  ...
                  if not methodNameSet[childName] then
                     ...
                     self.scope:addMethod( impMtdType, child:get_accessMode(), child:get_staticFlag(), false )
                  end
   ...
   local function checkOverrideMethod( scope )
      scope:filterTypeInfoField( true, self.scope, function ( symbolInfo )
         if symbolInfo:get_kind() == Ast.SymbolKind.Mtd then
            local impMethodType = self.scope:getTypeInfoField( symbolInfo:get_name(), true, self.scope )
            local noImp = false
            do
               local _exp = impMethodType
               if _exp ~= nil then
                  if _exp:get_abstructFlag() then
                     noImp = true
                  end
...
function TransUnit:analyzeDeclFunc( declFuncMode, abstructFlag, overrideFlag, accessMode, staticFlag, classNameToken, firstToken, name )
   ...
   if token.txt == ";" then
      ...
      needNode = true
      if abstructFlag then
         self:addErrMess( token.pos, "abstruct method can't have body." )
      end
      
      self:pushback(  )
      body = self:analyzeBlock( Ast.BlockKind.Func, scope )
   end
   
   if needNode then
      local info = Ast.DeclFuncInfo.new(classNameToken, name, argList, orgStaticFlag, accessMode, body, retTypeInfoList)
      do
         ...
         elseif _switchExp == Ast.nodeKindDeclDestr then
...
function TransUnit:analyzeExpList( skipOp2Flag, expNode, expectTypeList, contExpect )
   ...
   repeat 
      ...
      do
         local _exp = expectTypeList
         if _exp ~= nil then
            if #_exp > 0 then
               local checkIndex = index
               if index > #_exp and contExpect then
                  checkIndex = #_exp
               end
               
               if checkIndex <= #_exp and _exp[checkIndex] ~= Ast.builtinTypeNone then
   ...
   if not skipFlag then
      repeat 
         ...
         if nextToken.txt == "(" or nextToken.txt == "$(" then
            ...
            if work.txt ~= ")" then
               self:pushback(  )
               expList = self:analyzeExpList( false, nil, funcTypeInfo:get_argTypeInfoList() )
               self:checkNextToken( ")" )
            end
            ...
            if macroFlag then
               ...
               exp = Ast.ExpCallNode.create( self.nodeManager, firstToken.pos, retTypeInfoList, exp, nilAccess, expList )
            end
...
function TransUnit:analyzeExp( skipOp2Flag, prevOpLevel, expectType )
   ...
   if token.kind == Parser.TokenKind.Dlmt then
      ...
      elseif token.txt == "(" then
         ...
         self:checkNextToken( ")" )
         exp = Ast.ExpParenNode.create( self.nodeManager, firstToken.pos, exp:get_expTypeList(), exp )
         exp = self:analyzeExpCont( firstToken, exp, false )
      end
   ...
   if exp:get_kind() == Ast.nodeKindNone then
