...
function TransUnit:__init(moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info) 
   ...
   self.processInfo = Ast.createProcessInfo( ctrl_info.validCheckingMutable, ctrl_info.validLuaval )
...
function TransUnit:pushModule( processInfo, externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _4186, existSym = parentScope:addClass( processInfo, modName, nil, typeInfo )
...
function TransUnit:analyzeImportFor( pos, modulePath, assignName, assigned, lazyLoad )
   ...
   if  nil == importObj then
      ...
      importObj = Import.Import.new(self, self.importModuleInfo, self.moduleType, builtinFunc, self.globalScope, self.macroCtrl, self.typeNameCtrl, self.helperInfo, self.importedAliasMap, self.validMutControl)
      self.importCtrl = importObj
   end
   
   local moduleInfo = importObj:processImport( self.processInfo, modulePath, 1 )
   
   self.processInfo:switchIdProvier( Ast.IdType.Base )
   
   self.scope = backupScope
   
   local moduleTypeInfo = moduleInfo:get_moduleProvide():get_typeInfo()
   self.scope:addModule( moduleTypeInfo, moduleInfo:assign( assignName ) )
   
   local moduleSymbolKind = moduleInfo:get_moduleProvide():get_symbolKind()
   local moduleSymbolInfo, shadowing = self.scope:add( self.processInfo, moduleSymbolKind, false, false, assignName, pos, moduleTypeInfo, Ast.AccessMode.Local, true, moduleInfo:get_moduleProvide():get_mutable() and Ast.MutMode.Mut or Ast.MutMode.IMut, true, lazyLoad ~= Nodes.LazyLoad.Off )
...
function ASTInfo.new( node, moduleTypeInfo, provideInfo, processInfo, streamName )
   local obj = {}
   ASTInfo.setmeta( obj )
   if obj.__init then
      obj:__init( node, moduleTypeInfo, provideInfo, processInfo, streamName )
   end
   return obj
end
function ASTInfo:__init( node, moduleTypeInfo, provideInfo, processInfo, streamName )
...
function ASTInfo:get_provideInfo()
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   Log.log( Log.Level.Log, __func__, 521, function (  )
   ...
   if moduleName ~= nil then
      for _5452 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
   ...
   local function createId2proto( map )
      ...
      local id2proto = {}
      for protoType, _5459 in pairs( map ) do
   ...
   if moduleSymboInfo ~= nil then
      moduleSymbo = frontInterface.ModuleProvideInfo.new(moduleSymboInfo:get_typeInfo(), moduleSymboInfo:get_kind(), moduleSymboInfo:get_mutable())
   else
      moduleSymbo = frontInterface.ModuleProvideInfo.new(moduleTypeInfo, Ast.SymbolKind.Typ, false)
   end
   
   return ASTInfo.new(ast, moduleTypeInfo, moduleSymbo, self.processInfo, parser:getStreamName(  ))
end
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _5531, existSym = self.scope:addMacro( self.processInfo, nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _5705, shadowing = self.scope:addEnum( self.processInfo, accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _5717, shadowing = scope:addAlge( self.processInfo, accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeDeclForm( accessMode, firstToken )
   ...
   local declFuncInfo = Nodes.DeclFuncInfo.new(Nodes.FuncKind.Form, nil, nil, name, formSymbol or shadowing, argList, false, accessMode, nil, retTypeList, retNodeList, false, false)
...
function TransUnit:analyzeDeclMember( classTypeInfo, accessMode, staticFlag, firstToken )
   ...
   if token.txt == "{" then
      ...
      if nextToken.txt == "," then
         ...
         Log.log( Log.Level.Debug, __func__, 1741, function (  )
   ...
   if Ast.TypeInfo.isMut( typeInfo ) and typeInfo:get_mutMode() ~= mutMode and self.ctrl_info.legacyMutableControl then
   ...
   return Nodes.DeclMemberNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {typeInfo}, varName, refType, workSym, classTypeInfo, staticFlag, accessMode, getterMutable, getterMode, getterToken, getterRetType, setterMode, setterToken )
end
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _6229, methodNameSet = self:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleLang, moduleName, lazyLoad, nextToken, inheritInfo )
...
function TransUnit:analyzeDeclFunc( declFuncMode, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   local function createDeclFuncInfo( funcKind )
      ...
      return Nodes.DeclFuncInfo.new(funcKind, classTypeInfo, classDeclNode, name, funcSym, argList, orgStaticFlag, accessMode, body, retTypeInfoList, retTypeNodeList, _lune._Set_has(self.has__func__Symbol, typeInfo ), overrideFlag)
   end
...
function TransUnit:analyzeInitExp( firstPos, accessMode, unwrapFlag, letVarList, typeInfoList )
   ...
   if expList ~= nil then
      ...
      if unwrapFlag then
         ...
         for index, _6537 in ipairs( letVarList ) do
...
function TransUnit:checkMatchValType( pos, funcTypeInfo, expList, genericTypeList, genericsClass )
   ...
   local matchResult, _7126, newExpNodeList = self:checkMatchType( funcTypeInfo:getTxt(  ), pos, argTypeList, expList, false, warnForFollow, alt2typeMap )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _7161 = 1, #expNode:get_expTypeList() do
      ...
      local _7164, _7165, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _7895, _7896, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _7995, _7996, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _8271, _8272, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _8340, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _8444, _8445, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
