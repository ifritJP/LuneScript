...
function TransUnit:__init(moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info) 
   ...
   self.helperInfo = Ast.LuneHelperInfo.new(false, false, false, false, false, false, false)
      ...
      if newTypeInfo and (self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF ) then
         ...
         if self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF then
            ...
            for __index, itemType in pairs( itemTypeInfo ) do
               table.insert( altTypeList, _lune.unwrap( (_lune.__Cast( itemType, Ast.AlternateTypeInfo ) )) )
            end
...
function _TypeInfoEnum:createTypeInfo( param )
   ...
   local scope = Ast.Scope.new(parentScope, true, nil)
   param.typeId2Scope[self.typeId] = scope
   local enumTypeInfo = Ast.NormalTypeInfo.createEnum( scope, parentInfo, true, accessMode, self.txt, _lune.unwrap( param:getTypeInfo( self.valTypeId )), name2EnumValInfo )
   ...
   do
      ...
      if _exp ~= nil then
         do
            ...
            elseif _switchExp == "string" then
               table.insert( tokenList, Parser.Token.new(Parser.TokenKind.Str, string.format( '[[%s]]', _exp), pos, false) )
...
local ExtMacroInfo = {}
setmetatable( ExtMacroInfo, { __index = Ast.MacroInfo } )
...
function TransUnit:processImport( modulePath )
   ...
   for varName, varInfo in pairs( metaInfo.__varName2InfoMap ) do
      do
         ...
         if typeId ~= nil then
            self.scope:addStaticVar( false, true, varName, _lune.unwrap( typeId2TypeInfo[math.floor(typeId)]), varInfo['mutable'] and true or false )
         else
   ...
   for orgTypeId, macroInfoStem in pairs( metaInfo.__macroName2InfoMap ) do
      ...
      if macroInfo ~= nil then
         ...
         for __index, argInfo in pairs( macroInfo.argList ) do
            ...
            table.insert( argList, Ast.MacroArgInfo.new(argInfo.name, argTypeInfo) )
         ...
         for __index, symInfo in pairs( macroInfo.symList ) do
            ...
            symbol2MacroValInfoMap[symInfo.name] = Ast.MacroValInfo.new(nil, symTypeInfo)
         end
...
function TransUnit:analyzeMatch( firstToken )
   ...
   local exp = self:analyzeExp( false )
   local algeTypeInfo = _lune.__Cast( exp:get_expType():get_srcTypeInfo(), Ast.AlgeTypeInfo )
...
function TransUnit:analyzeApply( token )
   ...
   do
      local callNode = _lune.__Cast( exp, Ast.ExpCallNode )
...
function TransUnit:analyzeForeach( token, sortFlag )
   ...
   self:popScope(  )
   if sortFlag then
      return Ast.ForsortNode.create( self.nodeManager, token.pos, {Ast.builtinTypeNone}, mainSymbol, subSymbol, exp, block, sortFlag )
   else
    
      return Ast.ForeachNode.create( self.nodeManager, token.pos, {Ast.builtinTypeNone}, valSymbol, subSymbol, exp, block )
   end
            ...
            elseif _switchExp == Ast.TypeInfoKind.Class or _switchExp == Ast.TypeInfoKind.IF then
               if checkAlternateTypeCount( #typeInfo:get_itemTypeInfoList() ) then
                  for index, itemType in pairs( genericList ) do
                     local altType = _lune.unwrap( _lune.__Cast( typeInfo:get_itemTypeInfoList()[index], Ast.AlternateTypeInfo ))
                     if itemType:get_nilable() then
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if macroFlag then
      ...
      local rootNode = Ast.RootNode.create( self.nodeManager, Parser.Position.new(0, 0), {Ast.builtinTypeNone}, children, self.useModuleMacroSet, self.moduleId, processInfo, moduleTypeInfo, nil, self.helperInfo, self.nodeManager, self.importModule2ModuleInfo, self.typeId2MacroInfo, self.typeId2ClassMap )
      ast = rootNode
      do
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   if nextToken.txt == "{" then
      ...
      self.macroScope = nil
      stmtBlock = Ast.BlockNode.create( self.nodeManager, firstToken.pos, {Ast.builtinTypeNone}, Ast.BlockKind.Macro, stmtList )
   else
   ...
   self.typeId2MacroInfo[typeInfo:get_typeId(  )] = Ast.DefMacroInfo.new(macroObj, declMacroInfo, self.symbol2ValueMapForMacro)
   self.symbol2ValueMapForMacro = {}
   return node
end
...
function TransUnit:analyzeDeclFunc( declFuncMode, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   if needNode then
      local info = Ast.DeclFuncInfo.new(classTypeInfo, name, argList, orgStaticFlag, accessMode, body, retTypeInfoList, _lune._Set_has(self.has__func__Symbol, typeInfo ))
      do
         local _switchExp = (kind )
...
function TransUnit:createExpListNode( pos, followOn, newExpList )
   ...
   return Ast.ExpListNode.create( self.nodeManager, pos, newExpTypeList, newExpList, followOn )
end
   ...
   if expList ~= nil then
      ...
      for index, exp in pairs( expList:get_expList() ) do
         ...
         if not exp:canBeRight(  ) then
            self:addErrMess( exp:get_pos(), string.format( "this node(%d) can not be r-value. -- %s", index, Ast.getNodeKindName( exp:get_kind() )) )
         end
      ...
      for index, expType in pairs( expList:get_expTypeList() ) do
         ...
         if index == #expList:get_expTypeList() and expType:get_kind() == Ast.TypeInfoKind.DDD then
            ...
            if index <= #typeInfoList then
               ...
               if not varType:equals( Ast.builtinTypeNone ) and not varType:canEvalWith( expTypeInfo, "=", alt2typeMap ) and not (unwrapFlag and expTypeInfo:equals( Ast.builtinTypeNil ) ) then
               ...
               if Ast.CanEvalCtrlTypeInfo.canAutoBoxing( varType, expTypeInfo ) then
                  ...
                  newExpList[index] = Ast.BoxingNode.create( self.nodeManager, exp:get_pos(), {varType}, exp )
...
function TransUnit:analyzeWhen( firstToken )
   ...
   for __index, expNode in pairs( symListNode:get_expList() ) do
      ...
      do
         local refNode = _lune.__Cast( expNode, Ast.ExpRefNode )
         if refNode ~= nil then
...
function TransUnit:analyzeSetConst( token )
   ...
   return Ast.LiteralSetNode.create( self.nodeManager, token.pos, typeInfoList, expList )
end
...
function TransUnit:checkMatchType( message, pos, dstTypeList, expNodeList, allowDstShort, warnForFollow, genericsClassType )
   ...
   if #expNodeList > 0 then
      if expNodeList[#expNodeList]:get_kind() == Ast.NodeKind.get_Abbr() then
         hasAbbr = true
         local workList = {}
      ...
      for index, expNode in pairs( expNodeList ) do
         ...
         if #dstTypeList >= index then
            ...
            if Ast.CanEvalCtrlTypeInfo.canAutoBoxing( dstType, expNode:get_expType() ) then
               ...
               newExpNodeList[index] = Ast.BoxingNode.create( self.nodeManager, expNode:get_pos(), {dstType}, expNode )
            end
...
local function isMatchStringFormatType( opKind, argType, luaVer )
   ...
   do
      local enumType = _lune.__Cast( argType:get_srcTypeInfo(), Ast.EnumTypeInfo )
...
function TransUnit:analyzeExpCall( firstToken, exp, nextToken )
   ...
   do
      local refField = _lune.__Cast( exp, Ast.RefFieldNode )
...
function TransUnit:analyzeExpCast( firstToken, opTxt, exp )
   ...
   return Ast.ExpCastNode.create( self.nodeManager, firstToken.pos, {castType}, exp, opTxt ~= "@@@" )
end
...
function TransUnit:analyzeExpField( firstToken, token, mode, prefixExp )
   ...
   if self.macroMode == Ast.MacroMode.Analyze then
      ...
      return Ast.RefFieldNode.create( self.nodeManager, firstToken.pos, {Ast.builtinTypeSymbol}, token, nil, accessNil, prefixExp )
   end
         ...
         do
            ...
            if expType ~= nil then
               if prefixExp:get_expType():isModule(  ) then
                  do
                     local algeType = _lune.__Cast( expType, Ast.AlgeTypeInfo )
   ...
   elseif mode == ExpSymbolMode.Symbol then
      if self.macroMode == Ast.MacroMode.Analyze then
         ...
         if symbolInfo:get_kind() == Ast.SymbolKind.Typ then
            do
               local algeType = _lune.__Cast( typeInfo, Ast.AlgeTypeInfo )
               if algeType ~= nil then
                  local nextToken = self:getToken(  )
                  if nextToken.txt == "." then
...
function TransUnit:analyzeExpOpSet( exp, opeToken, exp2NodeList )
   ...
   do
      local alt2typeMap, newExpNodeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), exp2NodeList, true, false, nil )
      if alt2typeMap ~= nil and newExpNodeList ~= nil then
      ...
      elseif nextToken.kind == Parser.TokenKind.Ope then
         if Parser.isOp2( opTxt ) then
            ...
            do
               ...
               do
                  local _exp = _lune.__Cast( prefixExpType:get_srcTypeInfo(), Ast.EnumTypeInfo )
                  if _exp ~= nil then
                     enumTypeInfo = _exp
                  end
               end
...
function TransUnit:analyzeExpMacroStat( firstToken )
   ...
   while true do
      ...
      if token.txt == ",," or token.txt == ",,," or token.txt == ",,,," then
         ...
         if token.txt == ",," then
            do
               local refNode = _lune.__Cast( exp, Ast.ExpRefNode )
               if refNode ~= nil then
   ...
   if currentFunc:get_kind() == Ast.TypeInfoKind.Method then
      ...
      if superType:equals( Ast.headTypeInfo ) then
         ...
         if currentFunc:get_rawTxt() == "__init" then
            ...
            self:checkMatchValType( firstToken.pos, superCtorType, expList, {}, nil )
            ...
            do
               ...
               if superFunc ~= nil then
                  ...
                  self:checkMatchValType( firstToken.pos, superFunc, expList, {}, nil )
      ...
      if token.txt == "." then
         do
            ...
            if _exp ~= nil then
               ...
               do
                  local enumTyepInfo = _lune.__Cast( orgExpectType:get_srcTypeInfo(), Ast.EnumTypeInfo )
