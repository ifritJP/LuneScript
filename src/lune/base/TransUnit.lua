...
function TransUnit:pushModule( externalFlag, name, mutable )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         local _5676, existSym = parentScope:addClass( modName, nil, typeInfo )
...
function TransUnit:createExtType( pos, typeInfo )
   ...
   do
      local _matchExp = Ast.NormalTypeInfo.createLuaval( typeInfo )
...
function _TypeInfoModule:createTypeInfo( param )
   ...
   do
      ...
      if _exp ~= nil then
         ...
         Log.log( Log.Level.Info, __func__, 1501, function (  )
   ...
   if self.parentId ~= Ast.rootTypeId or not Ast.getBuiltInTypeIdMap(  )[self.typeId] or self.kind == Ast.TypeInfoKind.List or self.kind == Ast.TypeInfoKind.Array or self.kind == Ast.TypeInfoKind.Map or self.kind == Ast.TypeInfoKind.Set then
      ...
      if newTypeInfo and (self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF ) then
         ...
         if self.kind == Ast.TypeInfoKind.Class or self.kind == Ast.TypeInfoKind.IF then
            Log.log( Log.Level.Info, __func__, 1604, function (  )
...
function TransUnit:registBuiltInScope(  )
   ...
   local function getTypeInfo( typeName )
      ...
      local genTypeList = {}
      local _6489, endIndex = typeName:find( "[%w%.]+<" )
      local suffix = ""
         ...
         if name ~= "" then
            ...
            do
               ...
               elseif _switchExp == DeclClassMode.Module then
                  ...
                  self.scope:get_parent():add( Ast.SymbolKind.Typ, false, false, name, nil, parentInfo, Ast.AccessMode.Local, true, Ast.MutMode.Mut, true )
               end
            end
...
function TransUnit:processImport( modulePath )
   ...
   Log.log( Log.Level.Info, __func__, 2951, function (  )
      ...
      if moduleInfo ~= nil then
         do
            ...
            if metaInfoStem ~= nil then
               Log.log( Log.Level.Info, __func__, 2963, function (  )
   ...
   local metaInfo = metaInfoStem
   Log.log( Log.Level.Info, __func__, 2983, function (  )
      ...
      for __index, dependName in ipairs( __sorted ) do
         ...
         do
            if dependInfo['use'] then
               local _6718, metaTypeId2TypeInfoMap = self:processImport( dependName )
   ...
   for __index, _6732 in ipairs( nameList ) do
      self:popModule(  )
   end
   ...
   for __index, atomInfo in ipairs( metaInfo.__typeInfoList ) do
      do
         ...
         if skind ~= nil then
            ...
            do
               ...
               elseif _switchExp == Ast.SerializeKind.Ext then
                  actInfo = _TypeInfoExt._fromMap( atomInfo )
               end
            end
   ...
   local function registMember( classTypeId )
      ...
      do
         ...
         elseif _switchExp == Ast.TypeInfoKind.Module then
            ...
            Log.log( Log.Level.Info, __func__, 3238, function (  )
   ...
   for __index, _6788 in ipairs( nameList ) do
      self:popModule(  )
   end
...
function TransUnit:analyzeImportFor( pos, modulePath, assignName )
   ...
   local moduleSymbolInfo, shadowing = self.scope:add( moduleSymbolKind, false, false, assignName, pos, moduleTypeInfo, Ast.AccessMode.Local, true, metaInfo.__moduleMutable and Ast.MutMode.Mut or Ast.MutMode.IMut, true )
...
function TransUnit:convToExtTypeList( pos, typeInfo, list )
   ...
   local newList, mess = Ast.convToExtTypeList( list )
   if newList ~= nil then
      return newList
   end
...
function TransUnit:analyzeForeach( token, sortFlag )
   ...
   local exp = self:analyzeExpOneRVal( false, false )
   if sortFlag and exp:get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      self:addErrMess( exp:get_pos(), string.format( "forsort doesn't support with %s.", exp:get_expType():getTxt(  )) )
   end
            ...
            elseif _switchExp == Ast.TypeInfoKind.Class or _switchExp == Ast.TypeInfoKind.IF then
               if checkAlternateTypeCount( #typeInfo:get_itemTypeInfoList() ) then
                  ...
                  for index, itemType in ipairs( typeInfo:get_itemTypeInfoList() ) do
                     if itemType:hasBase(  ) and not genericList[index]:isInheritFrom( itemType:get_baseTypeInfo() ) then
...
function TransUnit:checkOverrideMethod( overrideType, typeInfo )
   ...
   local matchFlag, err = overrideType:canEvalWith( typeInfo, Ast.CanEvalType.SetEq, alt2typeMap )
...
function TransUnit:createAST( parser, macroFlag, moduleName )
   ...
   if moduleName ~= nil then
      for _7158 in string.gmatch( moduleName, '[^%.]+' ) do
         self:popModule(  )
      end
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _7202, existSym = self.scope:addMacro( nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   while nextToken.txt ~= "}" do
      ...
      if not workEnumTypeInfo then
         workEnumTypeInfo = Ast.NormalTypeInfo.createEnum( scope, self:getCurrentNamespaceTypeInfo(  ), false, accessMode, name.txt, valTypeInfo )
      end
      
      
      if workEnumTypeInfo ~= nil then
         scope:addEnumVal( valName.txt, valName.pos, workEnumTypeInfo )
   ...
   local _7298, shadowing = self.scope:addEnum( accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _7309, shadowing = scope:addAlge( accessMode, name.txt, name.pos, algeTypeInfo )
   self:errorShadowing( name.pos, shadowing )
...
function TransUnit:analyzeClassBody( classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
   ...
   local node = Nodes.DeclClassNode.create( self.nodeManager, firstToken.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {classTypeInfo}, classAccessMode, name, gluePrefix, moduleName, false, allStmtList, declStmtList, fieldList, memberList, self.scope, initBlockInfo, advertiseList, trustList, uninitMemberList, {} )
   ...
   local function processInitBlock( token )
      ...
      local funcSym, shadowing = parentScope:addFunc( token.pos, funcTypeInfo, Ast.AccessMode.Pri, true, true )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _7563, methodNameSet = self:analyzeClassBody( classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleName, nextToken )
...
function TransUnit:processAddFunc( isFunc, parentScope, name, typeInfo, alt2typeMap )
   ...
   if isFunc then
      funcSym, shadowing = parentScope:addFunc( name.pos, typeInfo, accessMode, staticFlag, mutable )
      self:errorShadowing( name.pos, shadowing )
   else
    
      funcSym, shadowing = parentScope:addMethod( name.pos, typeInfo, accessMode, staticFlag, mutable )
   end
...
function TransUnit:analyzeDeclFunc( declFuncMode, abstractFlag, overrideFlag, accessMode, staticFlag, classTypeInfo, firstToken, name )
   ...
   do
      local workTypeInfo = Ast.NormalTypeInfo.createFunc( abstractFlag, false, funcBodyScope, typeKind, namespaceInfo, false, false, staticFlag, accessMode, funcName, altTypeList, argTypeList, retTypeInfoList, mutable )
         ...
         if not _lune._Set_has(CantOverrideMethods, name.txt ) then
            if self.scope:get_parent():getTypeInfoField( name.txt, false, funcBodyScope, Ast.ScopeAccess.Full ) then
               ...
               do
                  ...
                  if ifFunc ~= nil then
                     if not ifFunc:get_typeInfo():canEvalWith( typeInfo, Ast.CanEvalType.SetEq, alt2typeMap ) then
...
function TransUnit:analyzeLetAndInitExp( firstPos, initMutable, accessMode, unwrapFlag )
   ...
   if expList ~= nil then
      ...
      if unwrapFlag then
         ...
         for index, _7729 in ipairs( letVarList ) do
   ...
   if unwrapFlag then
      ...
      if mode == Nodes.DeclVarMode.Let or mode == Nodes.DeclVarMode.Sync then
         if unwrapBlock ~= nil then
            ...
            for __index, letVarInfo in ipairs( letVarList ) do
...
function TransUnit:analyzeIfUnwrap( firstToken )
   ...
   if nextToken.txt == "let" then
      ...
      if expType:get_nilable() then
         table.insert( typeInfoList, expType:get_nonnilableType() )
      else
       
         table.insert( typeInfoList, expType )
      end
   ...
   local function process( index, dstType, expNode, workNode )
      ...
      do
         ...
         if repNode ~= nil then
            ...
            if expNode:get_expType() ~= Ast.builtinTypeNil and dstType ~= Ast.builtinTypeEmpty and not dstType:get_srcTypeInfo():equals( expNode:get_expType():get_srcTypeInfo() ) and not dstType:get_nonnilableType():get_srcTypeInfo():equals( expNode:get_expType():get_srcTypeInfo() ) then
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _8031 = 1, #expNode:get_expTypeList() do
      ...
      local _8032, _8033, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:analyzeExpCast( firstToken, opTxt, exp )
   ...
   if exp:get_expType():get_kind() == Ast.TypeInfoKind.Ext and castType:get_kind() ~= Ast.TypeInfoKind.Ext then
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _8426, _8427, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpSymbol( firstToken, symbolToken, mode, prefixExp, skipFlag, canLeftExp )
   ...
   elseif mode == ExpSymbolMode.Symbol then
      if self.macroCtrl:get_analyzeInfo():isAnalyzingSymArg(  ) then
         ...
         if symbolToken.txt == "_" then
            self:addErrMess( symbolToken.pos, "can't access '_' symbol." )
         end
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _8477, _8478, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
...
function TransUnit:analyzeExpOpEquals( pos, opToken, exp1, exp2 )
   ...
   if exp1Type:equals( Ast.builtinTypeBool ) and exp2Type:equals( Ast.builtinTypeBool ) and (exp1:get_kind() == Nodes.NodeKind.get_LiteralBool() or exp2:get_kind() == Nodes.NodeKind.get_LiteralBool() ) then
   ...
   if nonNilType1 ~= nonNilType2 then
      if nonNilType1:get_kind() == Ast.TypeInfoKind.Class or nonNilType1:get_kind() == Ast.TypeInfoKind.IF then
         if nonNilType2:get_kind() == Ast.TypeInfoKind.Class or nonNilType2:get_kind() == Ast.TypeInfoKind.IF then
            if nonNilType1:isInheritFrom( nonNilType2 ) then
            ...
            do
               ...
               elseif _switchExp == "+" or _switchExp == "-" or _switchExp == "*" or _switchExp == "/" or _switchExp == "%" then
                  ...
                  if exp1Type:equals( Ast.builtinTypeReal ) or exp2Type:equals( Ast.builtinTypeReal ) then
                     ...
                     if exp1Type:equals( Ast.builtinTypeInt ) then
                     ...
                     elseif exp2Type:equals( Ast.builtinTypeInt ) then
...
function TransUnit:analyzeExpUnwrap( firstToken )
   ...
   if Ast.isExtType( expType:get_nonnilableType() ) then
      do
         local _matchExp = Ast.NormalTypeInfo.createLuaval( unwrapType )
         if _matchExp[1] == Ast.LuavalResult.OK[1] then
            local work = _matchExp[2][1]
            local _ = _matchExp[2][2]
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _8598, _8599, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _8633, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
   ...
   local function processOp1( token )
      ...
      do
         ...
         if _switchExp == "-" then
            ...
            typeInfo = expType
         elseif _switchExp == "#" then
            if expType:get_extedType():get_kind() ~= Ast.TypeInfoKind.List and expType:get_extedType():get_kind() ~= Ast.TypeInfoKind.Array and not Ast.builtinTypeString:canEvalWith( expType, Ast.CanEvalType.SetOp, {} ) then
               self:addErrMess( token.pos, string.format( 'unmatch type for "#" -- %s', expType:getTxt(  )) )
            end
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _8670, _8671, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
