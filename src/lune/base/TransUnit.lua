...
function TransUnit.getSuperParam( ctrl_info, macroEval )
   ...
   local scope = Ast.Scope.new(processInfo, processInfo:get_topScope(), true, nil)
   local parser = Parser.DefaultPushbackParser.new(Parser.DummyParser.new())
...
function TransUnit:__init(moduleId, importModuleInfo, macroEval, analyzeModule, mode, pos, targetLuaVer, ctrl_info, builtinFunc) 
   TransUnitIF.TransUnitBase.__init( self,TransUnit.getSuperParam( ctrl_info, macroEval ))
   ...
   self.subfileList = {}
   self.globalScope = self.processInfo:get_topScope()
...
function TransUnit:checkThreading( pos )
   ...
   if curClass ~= Ast.headTypeInfo then
      if curClass:isInheritFrom( self.processInfo, self.builtinFunc.lnsthread_, nil ) then
         return true
      end
...
function TransUnit:createAST( parserSrc, macroFlag, moduleName )
   local __func__ = '@lune.@base.@TransUnit.TransUnit.createAST'

   local parser = Parser.createParserFrom( parserSrc )
   
   Log.log( Log.Level.Log, __func__, 535, function (  )
   ...
   if moduleName ~= nil then
      for __index, _1705 in ipairs( Util.splitStr( moduleName, '[^%.]+' ) ) do
         self:popModule(  )
      end
...
function TransUnit:analyzeDeclMacro( accessMode, firstToken )
   ...
   local _1789, existSym = self.scope:addMacro( self.processInfo, nameToken.pos, node:get_expType(), accessMode )
   if existSym then
...
function TransUnit:analyzePushClass( mode, abstractFlag, firstToken, name, allowMultiple, requirePath, moduleLang, accessMode, altTypeList )
   ...
   if nextToken.txt == "extend" then
      ...
      if baseTypeInfo ~= nil then
         ...
         if baseTypeInfo:isInheritFrom( self.processInfo, self.builtinFunc.lnsthread_, nil ) and not _lune._Set_has(self.helperInfo.pragmaSet, _lune.newAlge( LuneControl.Pragma.use_async) ) then
            self:addErrMess( nextToken.pos, "must set '_lune_control use_async'" )
         end
...
function TransUnit:analyzeDeclEnum( accessMode, firstToken )
   ...
   local _1964, shadowing = self.scope:addEnum( self.processInfo, accessMode, name.txt, name.pos, enumTypeInfo )
...
function TransUnit:analyzeDeclAlge( accessMode, firstToken )
   ...
   local _1976, shadowing = scope:addAlge( self.processInfo, accessMode, name.txt, name.pos, algeTypeInfo )
...
function TransUnit:analyzeDeclMember( classTypeInfo, accessMode, staticFlag, firstToken )
   ...
   if token.txt == "{" then
      ...
      if nextToken.txt == "," then
         ...
         Log.log( Log.Level.Debug, __func__, 1836, function (  )
...
function TransUnit:analyzeDeclClass( classAbstructFlag, classAccessMode, firstToken, mode )
   ...
   local node, _2514, methodNameSet = self:analyzeClassBody( hasProto, classAccessMode, firstToken, mode, gluePrefix, classTypeInfo, name, moduleLang, moduleName, lazyLoad, nextToken, inheritInfo )
...
function TransUnit:analyzeInitExp( firstPos, accessMode, unwrapFlag, letVarList, typeInfoList )
   ...
   if expList ~= nil then
      ...
      if unwrapFlag then
         ...
         for index, _2841 in ipairs( letVarList ) do
...
function TransUnit:checkMatchValType( pos, funcTypeInfo, expList, genericTypeList, genericsClass )
   ...
   local matchResult, _3433, newExpNodeList = self:checkMatchType( funcTypeInfo:getTxt(  ), pos, argTypeList, expList, false, warnForFollow, alt2typeMap )
...
function TransUnit:analyzeListItems( firstPos, nextToken, termTxt, expectTypeList )
   ...
   if not expectTypeList then
         ...
         for index, expNode in ipairs( expList:get_expList() ) do
            if index == #expList:get_expList() then
               if expNode:get_expType():get_kind() == Ast.TypeInfoKind.DDD then
                  ...
                  for _3468 = 1, #expNode:get_expTypeList() do
      ...
      local _3471, _3472, workExpList = self:checkMatchType( "List constructor", firstPos, expTypeList, expList, false, false, nil )
      if workExpList ~= nil then
         expList = workExpList
      end
...
function TransUnit:processFunc( firstToken, nextToken, refFieldNode, funcExp, funcType, alt2typeMap, genericTypeList, genericsClass, argList )
   ...
   if nilAccess or funcType:get_kind() == Ast.TypeInfoKind.Ext or _lune._Set_has(self.builtinFunc:get_needThreadingTypes(), funcType:get_nonnilableType():get_srcTypeInfo() ) then
...
function TransUnit:checkAsyncField( symbolInfo, pos )
   ...
   if warn then
      if ((symbolInfo:get_staticFlag() and symbolInfo:get_kind() == Ast.SymbolKind.Mbr ) or symbolInfo:get_kind() == Ast.SymbolKind.Var ) and not _lune._Set_has(self.builtinFunc:get_allSymbolSet(), symbolInfo:getOrg(  ) ) and not Ast.TypeInfo.canBeAsyncParam( symbolInfo:get_typeInfo() ) then
...
function TransUnit:analyzeNewAlge( firstToken, algeTypeInfo, prefix )
   ...
   do
      ...
      if valInfo ~= nil then
         ...
         do
            local _4258, _4259, newExpNodeList = self:checkMatchType( "call", symbolToken.pos, valInfo:get_typeList(), argListNode, false, true, nil )
...
function TransUnit:analyzeExpOpSet( exp, opeToken, expectTypeList )
   ...
   local _4363, _4364, workList, expTypeList = self:checkMatchType( "= operator", opeToken.pos, exp:get_expTypeList(), expList, true, false, nil )
   if workList ~= nil then
      expList = workList
   end
...
function TransUnit:analyzeStrConst( firstToken, token )
   ...
   if nextToken.kind ~= Parser.TokenKind.Eof then
      ...
      if nextToken.txt == "(" then
         ...
         local _4639, _4640, workExpList = self:checkMatchType( "str constructor", firstToken.pos, {Ast.builtinTypeDDD}, argNodeList, false, false, nil )
         if workExpList ~= nil then
...
function TransUnit:analyzeExp( allowNoneType, skipOp2Flag, canLeftExp, prevOpLevel, expectType )
   ...
   local function processsNewExp( token )
      ...
      local _4708, alt2type, newArgList = self:checkMatchValType( exp:get_pos(), initTypeInfo, argList, classTypeInfo:get_itemTypeInfoList(), classTypeInfo )
...
function TransUnit:analyzeJoinRunner( token )

   local exp = self:analyzeExpOneRVal( false, true, nil, self.builtinFunc.__runner_ )
   
   if not exp:get_expType():isInheritFrom( self.processInfo, self.builtinFunc.__runner_, nil ) then
      self:addErrMess( exp:get_pos(), string.format( "this doesn't inherit the __Runner. -- %s", exp:get_expType():getTxt(  )) )
   end
   
   
   self:checkNextToken( ";" )
   
   return Nodes.JoinRunnerNode.create( self.nodeManager, token.pos, self.macroCtrl:isInAnalyzeArgMode(  ), {Ast.builtinTypeNone}, exp )
end
...
function TransUnit:analyzeReturn( token )
   ...
   do
      ...
      if workList ~= nil then
         do
            local _4823, _4824, newExpNodeList = self:checkMatchType( "return", token.pos, retTypeList, workList, false, not workList:get_followOn(), nil )
            if newExpNodeList ~= nil then
...
function TransUnit:analyzeStatement( termTxt )
   ...
   if not statement then
      ...
      elseif token.txt == "__join" then
         statement = self:analyzeJoinRunner( token )
