...
function _TypeInfoAlge:createTypeInfo( param )
   ...
   param.typeId2Scope[self.typeId] = scope
   local algeTypeInfo = param.processInfo:createAlge( scope, parentInfo, parentTypeDataAccessor, true, accessMode, self.txt, itemTypeInfo )
...
function _TypeInfoAlge._new( parentId, txt, accessMode, algeValList, itemTypeId )
   local obj = {}
   _TypeInfoAlge._setmeta( obj )
   if obj.__init then
      obj:__init( parentId, txt, accessMode, algeValList, itemTypeId )
   end
   return obj
end
function _TypeInfoAlge:__init( parentId, txt, accessMode, algeValList, itemTypeId )
...
function ModuleLoader:getExportInfo(  )
   ...
   if not self.result:get_exportInfo() then
      Log.log( Log.Level.Err, __func__, 1004, function (  )
...
function ModuleLoader:processImportFromFile( processInfo, lnsPath, metaInfoStem, fullModulePath, modulePath, nameList, baseDir, depth )
   ...
   do
      ...
      Log.log( Log.Level.Info, __func__, 1022, function (  )
         ...
         do
            ...
            do
               ...
               elseif _switchExp == Ast.TypeInfoKind.Module then
                  ...
                  Log.log( Log.Level.Debug, __func__, 1344, function (  )
...
function Import:createModuleLoader( baseDir, modulePath, moduleLoaderParam, depth )
   ...
   if exportInfo ~= nil then
      Log.log( Log.Level.Info, __func__, 1544, function (  )
...
function Import:loadModuleInfo( moduleLoader )
   ...
   Log.log( Log.Level.Info, __func__, 1595, function (  )
...
function ModuleLoader:processImportMain( processInfo, baseDir, modulePath, depth )
   ...
   Log.log( Log.Level.Info, __func__, 1608, function (  )
