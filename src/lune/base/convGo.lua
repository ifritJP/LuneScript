...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if mRetIndex ~= nil then
      ...
      if restIndex ~= nil then
         self:write( "Lns_2DDD( " )
         for index, _5694 in pairs( expList ) do
      ...
      for index, _5696 in pairs( dstTypeList ) do
         if index ~= 1 then
            self:write( ", " )
         end
...
function convFilter:outputConvExt( funcNode )
   ...
   self:writeln( ") {" )
   self:write( "    return " )
   for index, _5804 in pairs( funcNode:get_expType():get_retTypeInfoList() ) do
      if index > 1 then
         self:write( "," )
      end
...
function convFilter:processRoot( node, opt )
   ...
   do
      local function procNode( workNode )
         ...
         local symTypeList = {}
         for _5884 = 1, #workNode:get_varSymList() do
...
function convFilter:processIfUnwrap( node, opt )
   ...
   for index, varSym in pairs( node:get_varSymList() ) do
      ...
      if index == #node:get_varSymList() then
         for _6107 = index + 1, #node:get_expList():get_expTypeList() do
            self:write( ", _" )
         end
               ...
               if index == #node:get_varList() then
                  do
                     ...
                     if expListNode ~= nil then
                        for _6125 = index + 1, #expListNode:get_expTypeList() do
                           self:write( ", _" )
                        end
      ...
      do
         ...
         if expList ~= nil then
            ...
            if #expList:get_expTypeList() > #node:get_symbolInfoList() then
               for _6137 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
...
function convFilter:processDeclVar( node, opt )
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         ...
         for _6154 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
            self:write( ",_" )
         end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in pairs( node:get_caseList() ) do
         for _6248, symbol in pairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _6291 in pairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _6425 in pairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) and not symbol:get_staticFlag() then
                  ...
                  for index, _6515 in pairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
...
function convFilter:processExpSetVal( node, opt )
   ...
   for _6639 = #node:get_exp1():get_expTypeList() + 1, #node:get_exp2():get_expTypeList() do
      self:write( ",_" )
   end
