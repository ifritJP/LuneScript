      ...
      elseif _matchExp[1] == SymbolKind.Func[1] then
         ...
         if typeInfo:get_kind() == Ast.TypeInfoKind.Method then
            ...
            if isInnerDeclType( typeInfo ) then
               ...
               if not Ast.isBuiltin( typeInfo:get_typeId() ) then
...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   for index, dstType in pairs( dstTypeList ) do
      ...
      if index >= mRetIndex then
         local wrote = false
         if index <= #argList:get_expList() then
            local exp = argList:get_expList()[index]
            do
               local castNode = _lune.__Cast( exp, 3, Nodes.ExpCastNode )
               if castNode ~= nil then
   ...
   if restIndex ~= nil then
      self:write( "Lns_2DDD( " )
      for index, _5499 in pairs( argList:get_expList() ) do
...
function convFilter:processRoot( node, opt )
   ...
   for __index, workNode in pairs( node:get_nodeManager():getIfUnwrapNodeList(  ) ) do
      local symTypeList = {}
      for _5630 = 1, #workNode:get_varSymList() do
...
function convFilter:processIfUnwrap( node, opt )
   ...
   for index, varSym in pairs( node:get_varSymList() ) do
      ...
      if index == #node:get_varSymList() then
         for _5815 = index + 1, #node:get_expList():get_expTypeList() do
            self:write( ", _" )
         end
               ...
               if index == #node:get_varList() then
                  do
                     ...
                     if expListNode ~= nil then
                        for _5833 = index + 1, #expListNode:get_expTypeList() do
                           self:write( ", _" )
                        end
      ...
      do
         ...
         if expList ~= nil then
            ...
            if #expList:get_expTypeList() > #node:get_symbolInfoList() then
               for _5845 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in pairs( node:get_caseList() ) do
         for _5955, symbol in pairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _5998 in pairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _6129 in pairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
      ...
      do
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) then
                  ...
                  for index, _6219 in pairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
   ...
   do
      ...
      elseif _switchExp == Ast.TypeInfoKind.Map or _switchExp == Ast.TypeInfoKind.Stem then
         if node:get_nilAccess() then
            ...
            filter( node:get_val(), self, node )
            if prefixTpe:get_kind() == Ast.TypeInfoKind.Stem then
               self:write( ".(*LnsMap)" )
            end
