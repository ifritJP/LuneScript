...
local ignoreNodeInInnerBlockSet = {[Nodes.NodeKind.get_DeclAlge()] = true, [Nodes.NodeKind.get_DeclEnum()] = true, [Nodes.NodeKind.get_DeclMethod()] = true, [Nodes.NodeKind.get_DeclForm()] = true, [Nodes.NodeKind.get_DeclMacro()] = true, [Nodes.NodeKind.get_TestCase()] = true, [Nodes.NodeKind.get_TestBlock()] = true}
...
function convFilter:concatSymWithType( name, typeInfo )
   ...
   return concatGLSym( string.format( "%s_", getModuleName( typeInfo:getModule(  ) )) .. name, self:isPubType( typeInfo ) )
end
   ...
   do
      ...
      elseif _matchExp[1] == SymbolKind.Var[1] then
         ...
         if symbolInfo:getModule(  ) ~= self.moduleTypeInfo and symbolInfo:get_staticFlag() then
            symbolName = string.format( "%s_%s", getModuleName( symbolInfo:getModule(  ) ), symbolInfo:get_name())
         ...
         if typeInfo:get_kind() == Ast.TypeInfoKind.Method then
            ...
            do
               ...
               elseif _switchExp == Ast.TypeInfoKind.Enum or _switchExp == Ast.TypeInfoKind.Class then
                  ...
                  symbolName = string.format( "%s_%s", self:getSymbol( _lune.newAlge( SymbolKind.Type, {parentInfo}), parentInfo:get_rawTxt() ), symbolName)
                  if not self:isPubType( typeInfo ) then
                     ...
                     Util.err( string.format( "%s: not support -- %s", __func__, typeInfo:getFullName( self:get_typeNameCtrl(), self:get_moduleInfoManager(), true )) )
               end
            end
...
function convFilter:outputStem2Type( dstType )
   ...
   if dstType:get_kind() == Ast.TypeInfoKind.Class and dstType ~= Ast.builtinTypeString then
...
local function needConvFormFunc( node )
   ...
   for index, retType in pairs( castType:get_retTypeInfoList() ) do
...
local function getExpListKind( dstTypeList, node )
   ...
   if #dstTypeList > 1 and node:get_mRetExp() then
      for __index, exp in pairs( node:get_expList() ) do
         do
...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   for index, argExp in pairs( expList ) do
      do
   ...
   if mRetIndex ~= nil then
      ...
      if restIndex ~= nil then
         self:write( "Lns_2DDD( " )
         for index, _5711 in pairs( expList ) do
      ...
      for index, _5713 in pairs( dstTypeList ) do
         if index ~= 1 then
            self:write( ", " )
         end
...
function convFilter:outputDeclFunc( funcInfo )
   ...
   if extFlag then
      retTypeList = _lune.unwrap( Ast.convToExtTypeList( workType:get_retTypeInfoList() ))
   else
   ...
   do
      ...
      if _matchExp[1] == FuncInfo.DeclInfo[1] then
         ...
         for index, arg in pairs( declInfo:get_argList() ) do
            if index ~= 1 then
               self:write( "," )
            end
      ...
      elseif _matchExp[1] == FuncInfo.Type[1] then
         ...
         for index, argType in pairs( workType:get_argTypeInfoList() ) do
            if index ~= 1 then
               self:write( "," )
            end
...
function convFilter:outputTopScopeVar( node )
   ...
   for __index, symbolInfo in pairs( node:get_symbolInfoList() ) do
      if symbolInfo:get_scope() == self.moduleScope and (Ast.isPubToExternal( symbolInfo:get_accessMode() ) or symbolInfo:get_posForModToRef() ) then
...
function convFilter:outputConvExt( funcNode )
   ...
   for index, retType in pairs( funcNode:get_expType():get_retTypeInfoList() ) do
      if index > 1 then
         self:write( "," )
      end
   ...
   self:writeln( ") {" )
   self:write( "    return " )
   for index, _5821 in pairs( funcNode:get_expType():get_retTypeInfoList() ) do
      if index > 1 then
         self:write( "," )
      end
...
function convFilter:processRoot( node, opt )
   ...
   local function isUsingInTest( aNode )
      ...
      for __index, testBlock in pairs( node:get_nodeManager():getTestBlockNodeList(  ) ) do
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclEnumNodeList(  ) ) do
   ...
   for __index, workNode in pairs( node:get_nodeManager():getDeclVarNodeList(  ) ) do
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclAlgeNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclFormNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getExpCallNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getExpCastNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getTestCaseNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      local function procNode( workNode )
         ...
         local symTypeList = {}
         for _5900 = 1, #workNode:get_varSymList() do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getIfUnwrapNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getExpSetValNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      local function procNode( workNode )
         ...
         do
            ...
            if expList ~= nil then
               for index, symbolInfo in pairs( workNode:get_symbolInfoList() ) do
                  if workNode:get_mode() == Nodes.DeclVarMode.Let or workNode:get_mode() == Nodes.DeclVarMode.Unwrap then
                     if workNode:get_unwrapFlag() then
                        table.insert( typeList, expList:getExpTypeAt( index ) )
                     else
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclVarNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclFuncNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   do
      ...
      for __index, tmpNode in pairs( node:get_nodeManager():getDeclClassNodeList(  ) ) do
         if self.enableTest or not isUsingInTest( tmpNode ) then
            procNode( tmpNode )
         end
   ...
   if self.enableTest then
      for __index, workNode in pairs( node:get_nodeManager():getTestCaseNodeList(  ) ) do
...
function convFilter:expList2Slice( subList, toStem )
   ...
   local function processExp( exp )
      ...
      if toStem and Ast.isClass( exp:get_expType() ) then
         self:write( string.format( "%s2Stem(", self:getTypeSymbol( exp:get_expType():get_nonnilableType() )) )
...
function convFilter:processSetFromExpList( convArgFuncName, dstTypeList, expListNode )
   ...
   do
      ...
      elseif _switchExp == ExpListKind.Direct then
         local mRetIndex = _lune.nilacc( expListNode:get_mRetExp(), 'get_index', 'callmtd' )
         for index, funcArgType in pairs( dstTypeList ) do
...
function convFilter:processNewAlgeVal( node, opt )
   ...
   if #node:get_valInfo():get_typeList() == 0 then
      ...
      for index, param in pairs( node:get_paramList() ) do
         if index > 1 then
            self:write( ", " )
         end
...
function convFilter:outputDeclFuncInfo( node, declInfo )
   ...
   for __index, convArg in pairs( convFunc:get_argList() ) do
...
function convFilter:processIfUnwrap( node, opt )
   ...
   if getExpListKind( tempTypeList, node:get_expList() ) == ExpListKind.Direct then
      for _6137 = #node:get_varSymList() + 1, #node:get_expList():get_expTypeList() do
         self:write( ", _" )
      end
   ...
   self:write( "if " )
   local hasSym = false
   for __index, varSym in pairs( node:get_varSymList() ) do
      if varSym:get_name() ~= "_" then
         if hasSym then
            self:write( " && " )
         end
   ...
   if node:get_unwrapFlag() then
      do
         ...
         if expList ~= nil and  unwrapBlock ~= nil then
            ...
            self:writeln( "{" )
            self:pushIndent(  )
            for index, varInfo in pairs( node:get_varList() ) do
               if index ~= 1 then
                  self:write( ", " )
               end
            ...
            for index, _6155 in pairs( node:get_symbolInfoList() ) do
               table.insert( tmpVarTypeList, expList:getExpTypeAt( index ) )
            end
            
            
            if getExpListKind( tmpVarTypeList, expList ) == ExpListKind.Direct then
               for _6157 = #tmpVarTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
            ...
            self:write( " := " )
            self:processSetFromExpList( getConvExpName( node:get_id(), expList ), tmpVarTypeList, expList )
            self:writeln( "" )
            self:write( "if " )
            local hasCond = false
            for index, varInfo in pairs( node:get_varList() ) do
            ...
            do
               ...
               if thenBlock ~= nil then
                  self:writeln( "} else {" )
                  self:pushIndent(  )
                  for index, varInfo in pairs( node:get_symbolInfoList() ) do
      ...
      do
         ...
         if expList ~= nil then
            ...
            if getExpListKind( varTypeList, expList ) == ExpListKind.Direct then
               for _6170 = #varTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
            ...
            self:write( " = " )
            self:processSetFromExpList( getConvExpName( node:get_id(), expList ), varTypeList, expList )
            self:writeln( "" )
         end
      end
...
function convFilter:processDeclVar( node, opt )
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         self:writeln( "{" )
         self:pushIndent(  )
         for __index, varSym in pairs( node:get_symbolInfoList() ) do
         ...
         local function setVals(  )
            ...
            for index, varSym in pairs( node:get_symbolInfoList() ) do
               self:write( string.format( "%s = _%s", varSym:get_name(), varSym:get_name()) )
               if expList:getExpTypeAt( index ):get_nilable() then
                  self:outputAny2Type( varSym:get_typeInfo() )
               end
         ...
         if getExpListKind( typeList, expList ) == ExpListKind.Direct then
            for _6187 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
               self:write( ",_" )
            end
...
function convFilter:processSwitch( node, opt )
   ...
   for caseIndex, caseNode in pairs( node:get_caseList() ) do
      if caseIndex ~= 1 then
         self:write( "} else if " )
      end
      
      for index, exp in pairs( caseNode:get_expList():get_expList() ) do
         if index ~= 1 then
            self:write( " || " )
         end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in pairs( node:get_caseList() ) do
         for _6281, symbol in pairs( caseInfo:get_valParamNameList() ) do
   ...
   for __index, caseInfo in pairs( node:get_caseList() ) do
      ...
      for index, symbol in pairs( caseInfo:get_valParamNameList() ) do
         if symbol:get_posForModToRef() then
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _6324 in pairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputForeachLua( node, extType )
   ...
   do
      ...
      if _switchExp == Ast.TypeInfoKind.List or _switchExp == Ast.TypeInfoKind.Map then
         ...
         if node:get_val():hasAccess(  ) then
            tmpValName = "_val"
         else
         ...
         self:writeln( string.format( "_key, %s := _exp.Get1stFromMap()", tmpValName) )
         ...
         self:pushIndent(  )
         self:write( string.format( "_key, %s = ", tmpValName) )
         filter( node:get_exp(), self, node )
   ...
   do
      ...
      for __index, name in ipairs( __sorted ) do
         ...
         do
            ...
            for index, argType in pairs( typeInfo:get_argTypeInfoList() ) do
               if index ~= 1 then
                  self:write( ", " )
               end
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _6458 in pairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
...
function convFilter:outputConvItemTypeList( itemTypeInfoList, alt2type )
   ...
   if #itemTypeInfoList > 0 then
      ...
      self:pushIndent(  )
      for index, itemType in pairs( itemTypeInfoList ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputAdvertise( node )
   ...
   for __index, adv in pairs( node:get_advertiseList() ) do
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) and not symbol:get_staticFlag() then
                  ...
                  for index, _6548 in pairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
...
function convFilter:processExpCall( node, opt )
   ...
   if callKind == _lune.newAlge( CallKind.LuaCall) or callKind == _lune.newAlge( CallKind.RunLoaded) then
      if #funcType:get_retTypeInfoList() == 1 then
         if opt.node:get_kind() ~= Nodes.NodeKind.get_StmtExp() then
            self:write( "[0]" )
            local retTypeList = _lune.unwrap( Ast.convToExtTypeList( funcType:get_retTypeInfoList() ))
...
function convFilter:processExpSetVal( node, opt )
   ...
   if getExpListKind( node:get_exp1():get_expTypeList(), node:get_exp2() ) == ExpListKind.Direct then
      for _6672 = #node:get_exp1():get_expTypeList() + 1, #node:get_exp2():get_expTypeList() do
         self:write( ",_" )
      end
   ...
   self:write( " = " )
   self:processSetFromExpList( getConvExpName( node:get_id(), node:get_exp2() ), node:get_exp1():get_expTypeList(), node:get_exp2() )
   self:writeln( "" )
   ...
   do
      ...
      if _switchExp == Ast.TypeInfoKind.Ext then
         if node:get_nilAccess() then
            ...
            filter( node:get_val(), self, node )
            self:write( ") && " )
            self:write( "Lns_NilAccLast( Lns_NilAccPop().(*Lns_luaValue)" )
         else
...
function convFilter:processLiteralMap( node, opt )
   ...
   for __index, pair in pairs( node:get_pairList() ) do
