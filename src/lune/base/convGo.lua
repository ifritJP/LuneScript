...
function convFilter:processConvExp( nodeId, dstTypeList, argListNode )
   ...
   if restIndex ~= nil then
      self:write( "[]LnsAny{ " )
      for index, _5316 in pairs( argList:get_expList() ) do
...
function convFilter:outputConstructor( node )
   ...
   for index, _5720 in pairs( initFuncType:get_argTypeInfoList() ) do
      if index ~= 1 then
         self:write( ", " )
      end
   ...
   if not node:hasUserInit(  ) then
      ...
      for index, _5728 in pairs( initFuncType:get_argTypeInfoList() ) do
