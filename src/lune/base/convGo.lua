...
function Env:getCommonVm( threading )
...
function convFilter:getVM( threading, typeInfo )
   ...
   if typeInfo:get_asyncMode() == Ast.Async.Noasync then
      vmTxt = self.env:getCommonVm( threading )
   else
    
      vmTxt = self.env:getLuavm( threading )
   end
   
   return (txt:gsub( "GETVM", vmTxt ) )
end
...
}]==], self:getConv2formName( node ), self.env:getCommonVm( node:get_exp():isThreading(  ) )) )
      return 
   end
...
function convFilter:processRoot( node, opt )
   ...
   do
      local function procNode( workNode )
         ...
         local symTypeList = {}
         for _1125 = 1, #workNode:get_varSymList() do
   ...
   do
      local function procNode( workNode )
         ...
         self:processConvExp( workNode:get_id(), workNode:get_func():get_expType():get_argTypeInfoList(), workNode:get_argList(), needEnv )
      end
...
function convFilter:processIfUnwrap( node, opt )
   ...
   if getExpListKind( tempTypeList, node:get_expList(), self.option:get_addEnvArg() ) == ExpListKind.Direct then
      for _1579 = #node:get_varSymList() + 1, #node:get_expList():get_expTypeList() do
         self:write( ", _" )
      end
   ...
   if node:get_unwrapFlag() then
      do
         ...
         if expList ~= nil and  unwrapBlock ~= nil then
            ...
            for index, _1617 in ipairs( node:get_symbolInfoList() ) do
            ...
            if getExpListKind( tmpVarTypeList, expList, self.option:get_addEnvArg() ) == ExpListKind.Direct then
               for _1621 = #tmpVarTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
      ...
      do
         ...
         if expList ~= nil then
            ...
            if getExpListKind( varTypeList, expList, self.option:get_addEnvArg() ) == ExpListKind.Direct then
               for _1651 = #varTypeList + 1, #expList:get_expTypeList() do
                  self:write( ", _" )
               end
   ...
   do
      ...
      elseif _switchExp == Nodes.DeclVarMode.Unwrap then
         ...
         if getExpListKind( typeList, expList, self.option:get_addEnvArg() ) == ExpListKind.Direct then
            for _1680 = #node:get_symbolInfoList() + 1, #expList:get_expTypeList() do
               self:write( ",_" )
            end
...
function convFilter:processMatch( node, opt )
   ...
   local function hasAccessing(  )
      ...
      for __index, caseInfo in ipairs( node:get_caseList() ) do
         for _1811, symbol in ipairs( caseInfo:get_valParamNameList() ) do
...
function convFilter:processApply( node, opt )
   ...
   if node:get_expList():get_expType():get_kind() == Ast.TypeInfoKind.Ext then
      ...
      for index, _1875 in ipairs( node:get_varList() ) do
         if index > 1 then
            self:write( "," )
         end
...
function convFilter:outputForeachLua( node, sortFlag, exp, val, key, block )
   ...
   do
      ...
      if _switchExp == Ast.TypeInfoKind.List or _switchExp == Ast.TypeInfoKind.Map then
         ...
         if sortFlag then
            self:writeln( string.format( "%s := %s.SortMapKeyList( %s )", sorted, self.env:getCommonVm( node:isThreading(  ) ), tmpExp) )
...
function convFilter:outputConstructor( node )
   ...
   if not node:get_expType():get_abstractFlag() then
      ...
      for index, _2121 in ipairs( initFuncType:get_argTypeInfoList() ) do
         if index ~= 1 then
            self:write( ", " )
         end
         ...
         if scope ~= nil then
            scope:filterTypeInfoField( true, scope, Ast.ScopeAccess.Normal, function ( symbol )
               ...
               if symbol:get_kind() == Ast.SymbolKind.Mtd and symbol:get_name() ~= "__init" and not _lune._Set_has(methodNameSet, symbol:get_name() ) and not symbol:get_staticFlag() then
                  ...
                  self:write( getAddEnvArg( #funcType:get_argTypeInfoList(), self.option:get_addEnvArg() ) )
                  for index, _2326 in ipairs( funcType:get_argTypeInfoList() ) do
                     if index > 1 then
                        self:write( "," )
                     end
   ...
   do
      ...
      if fieldNode ~= nil then
         ...
         if funcType:get_kind() == Ast.TypeInfoKind.Ext then
            self:write( string.format( "%s.RunLoadedfunc(", self.env:getCommonVm( node:isThreading(  ) )) )
            addEnvArg = false
         end
...
function convFilter:processExpSetVal( node, opt )
   ...
   if getExpListKind( node:get_exp1():get_expTypeList(), node:get_exp2(), self.option:get_addEnvArg() ) == ExpListKind.Direct then
      for _2616 = #node:get_exp1():get_expTypeList() + 1, #node:get_exp2():get_expTypeList() do
         self:write( ",_" )
      end
...
function convFilter:processLuneControl( node, opt )
   ...
   do
      ...
      elseif _matchExp[1] == LuneControl.Pragma.run_async_runner[1] then
         ...
         if self.option:get_enableRunner() then
            self:writeln( "LnsRun( self )" )
         else
          
            if self.option:get_addEnvArg() then
               self:writeln( "self.Run( _env )" )
            else
