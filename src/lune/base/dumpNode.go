...
func dumpNode_filter_1036_(node *Nodes_Node,filter *dumpNode_dumpFilter,opt *DumpNode_Opt) {
    node.FP.ProcessFilter(&filter.Nodes_Filter, DumpNode_Opt2Stem(opt))
}

// 560: decl @lune.@base.@dumpNode.getTypeListTxt
func dumpNode_getTypeListTxt_1195_(typeList *LnsList) string {
    var txt string
    txt = ""
...
func dumpNode_Ast2Dump_1315_(ast *TransUnit_ASTInfo) string {
...
func (self *dumpNode_dumpFilter) dump(opt *DumpNode_Opt,node *Nodes_Node,txt string) {
    ...
    if expType.FP.Equals(self.processInfo, Ast_builtinTypeNone, nil, nil){
        typeStr = Lns_getVM().String_format("(%d:%s:%s)", []LnsAny{expType.FP.Get_typeId().Id, expType.FP.GetTxt(nil, nil, nil), expType.FP.Get_kind()})
...
func (self *dumpNode_dumpFilter) ProcessShebang(node *Nodes_ShebangNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, node.FP.Get_cmd())
}
...
func (self *dumpNode_dumpFilter) ProcessLuneKind(node *Nodes_LuneKindNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_exp(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessRoot(node *Nodes_RootNode,_opt LnsAny) {
    ...
    for _, _child := range( node.FP.Get_children().Items ) {
        ...
        dumpNode_filter_1036_(child, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessEnv(node *Nodes_EnvNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessBlockSub(node *Nodes_BlockNode,_opt LnsAny) {
    ...
    for _, _statement := range( node.FP.Get_stmtList().Items ) {
        ...
        dumpNode_filter_1036_(statement, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessDeclEnum(node *Nodes_DeclEnumNode,_opt LnsAny) {
    ...
    for _, _name := range( node.FP.Get_valueNameList().Items ) {
        ...
        var val LnsAny
        switch _exp971 := valInfo.FP.Get_val().(type) {
...
func (self *dumpNode_dumpFilter) ProcessDeclAlge(node *Nodes_DeclAlgeNode,_opt LnsAny) {
    ...
    {
        __collection1052 := algeTypeInfo.FP.Get_valInfoMap()
...
func (self *dumpNode_dumpFilter) ProcessNewAlgeVal(node *Nodes_NewAlgeValNode,_opt LnsAny) {
    ...
    for _, _exp := range( node.FP.Get_paramList().Items ) {
        ...
        dumpNode_filter_1036_(exp, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessDeclClass(node *Nodes_DeclClassNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, Lns_getVM().String_format("%s (%s)", []LnsAny{node.FP.Get_name().Txt, self.FP.GetFull(node.FP.Get_expType(), false)}))
    for _, _field := range( node.FP.Get_fieldList().Items ) {
        field := _field.(Nodes_NodeDownCast).ToNodes_Node()
        dumpNode_filter_1036_(field, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessDeclMember(node *Nodes_DeclMemberNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, node.FP.Get_name().Txt)
    dumpNode_filter_1036_(&node.FP.Get_refType().Nodes_Node, self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpMacroExp(node *Nodes_ExpMacroExpNode,_opt LnsAny) {
    ...
    for _, _stmt := range( stmtList.Items ) {
        ...
        dumpNode_filter_1036_(stmt, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessExpMacroStat(node *Nodes_ExpMacroStatNode,_opt LnsAny) {
    ...
    for _, _expStr := range( node.FP.Get_expStrList().Items ) {
        ...
        dumpNode_filter_1036_(expStr, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessUnwrapSet(node *Nodes_UnwrapSetNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(&node.FP.Get_dstExpList().Nodes_Node, self, opt.FP.NextOpt())
    dumpNode_filter_1036_(&node.FP.Get_srcExpList().Nodes_Node, self, opt.FP.NextOpt())
    if Lns_isCondTrue( node.FP.Get_unwrapBlock()){
        dumpNode_filter_1036_(&Lns_unwrap( node.FP.Get_unwrapBlock()).(*Nodes_BlockNode).Nodes_Node, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessIfUnwrap(node *Nodes_IfUnwrapNode,_opt LnsAny) {
    ...
    for _, _expNode := range( node.FP.Get_expList().FP.Get_expList().Items ) {
        ...
        dumpNode_filter_1036_(expNode, self, opt.FP.NextOpt())
    }
    dumpNode_filter_1036_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
    if Lns_isCondTrue( node.FP.Get_nilBlock()){
        dumpNode_filter_1036_(&Lns_unwrap( node.FP.Get_nilBlock()).(*Nodes_BlockNode).Nodes_Node, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessWhen(node *Nodes_WhenNode,_opt LnsAny) {
    ...
    dumpNode_filter_1036_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
    {
        ...
        if !Lns_IsNil( __exp ) {
            ...
            dumpNode_filter_1036_(&_exp.Nodes_Node, self, opt.FP.NextOpt())
        }
    }
}
...
func (self *dumpNode_dumpFilter) processDeclFuncInfo(node *Nodes_Node,declInfo *Nodes_DeclFuncInfo,opt *DumpNode_Opt) {
    ...
    for _, _arg := range( argList.Items ) {
        ...
        dumpNode_filter_1036_(arg, self, opt.FP.NextOpt())
    }
    {
...
func (self *dumpNode_dumpFilter) ProcessRefType(node *Nodes_RefTypeNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_name(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessIf(node *Nodes_IfNode,_opt LnsAny) {
    ...
    for _, _stmt := range( stmtList.Items ) {
        ...
        if stmt.FP.Get_exp().FP.Get_kind() != Nodes_nodeKindEnum__None{
            dumpNode_filter_1036_(stmt.FP.Get_exp(), self, opt.FP.NextOpt())
        }
        dumpNode_filter_1036_(&stmt.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
    }
}
...
func (self *dumpNode_dumpFilter) ProcessSwitch(node *Nodes_SwitchNode,_opt LnsAny) {
    ...
    for _, _caseInfo := range( caseList.Items ) {
        ...
        dumpNode_filter_1036_(&caseInfo.FP.Get_expList().Nodes_Node, self, opt.FP.NextOpt())
        dumpNode_filter_1036_(&caseInfo.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
    }
    {
...
func (self *dumpNode_dumpFilter) ProcessMatch(node *Nodes_MatchNode,_opt LnsAny) {
    ...
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_val(), self, opt.FP.NextOpt())
    var caseList *LnsList
    caseList = node.FP.Get_caseList()
    for _, _caseInfo := range( caseList.Items ) {
        caseInfo := _caseInfo.(Nodes_MatchCaseDownCast).ToNodes_MatchCase()
        dumpNode_filter_1036_(&caseInfo.FP.Get_valExpRef().Nodes_Node, self, opt.FP.NextOpt())
        dumpNode_filter_1036_(&caseInfo.FP.Get_block().Nodes_Node, self, NewDumpNode_Opt(prefix + "  " + caseInfo.FP.Get_valInfo().FP.Get_name(), depth + 1))
    }
    {
        __exp := node.FP.Get_defaultBlock()
        if !Lns_IsNil( __exp ) {
            _exp := __exp.(*Nodes_Node)
            dumpNode_filter_1036_(_exp, self, opt.FP.NextOpt())
        }
    }
}
...
func (self *dumpNode_dumpFilter) ProcessFor(node *Nodes_ForNode,_opt LnsAny) {
    ...
    dumpNode_filter_1036_(node.FP.Get_init(), self, opt.FP.NextOpt())
    dumpNode_filter_1036_(node.FP.Get_to(), self, opt.FP.NextOpt())
    {
...
func (self *dumpNode_dumpFilter) ProcessApply(node *Nodes_ApplyNode,_opt LnsAny) {
    ...
    dumpNode_filter_1036_(&node.FP.Get_expList().Nodes_Node, self, opt.FP.NextOpt())
    dumpNode_filter_1036_(&node.FP.Get_block().Nodes_Node, self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpCall(node *Nodes_ExpCallNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    var mess string
    mess = dumpNode_getTypeListTxt_1195_(node.FP.Get_expTypeList())
    self.FP.dump(opt, &node.Nodes_Node, mess)
    dumpNode_filter_1036_(node.FP.Get_func(), self, opt.FP.NextOpt())
    {
...
func (self *dumpNode_dumpFilter) ProcessExpMRet(node *Nodes_ExpMRetNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_mRet(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpCast(node *Nodes_ExpCastNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, Lns_getVM().String_format("%s(%d) -> %s(%d)", []LnsAny{node.FP.Get_exp().FP.Get_expType().FP.GetTxt(self.FP.Get_typeNameCtrl(), nil, nil), node.FP.Get_exp().FP.Get_expType().FP.Get_typeId().Id, node.FP.Get_castType().FP.GetTxt(self.FP.Get_typeNameCtrl(), nil, nil), node.FP.Get_castType().FP.Get_typeId().Id}))
    dumpNode_filter_1036_(node.FP.Get_exp(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpSetVal(node *Nodes_ExpSetValNode,_opt LnsAny) {
    ...
    dumpNode_filter_1036_(node.FP.Get_exp1(), self, opt.FP.NextOpt())
    dumpNode_filter_1036_(&node.FP.Get_exp2().Nodes_Node, self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpSetItem(node *Nodes_ExpSetItemNode,_opt LnsAny) {
    ...
    indexNode = nil
    switch _exp3848 := node.FP.Get_index().(type) {
    ...
    if indexNode != nil{
        ...
        dumpNode_filter_1036_(indexNode_612, self, opt.FP.NextOpt())
    }
    dumpNode_filter_1036_(node.FP.Get_exp2(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessExpNew(node *Nodes_ExpNewNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_symbol(), self, opt.FP.NextOpt())
    {
        __exp := node.FP.Get_argList()
        if !Lns_IsNil( __exp ) {
            _exp := __exp.(*Nodes_ExpListNode)
...
func (self *dumpNode_dumpFilter) ProcessRefField(node *Nodes_RefFieldNode,_opt LnsAny) {
    ...
    dumpNode_filter_1036_(node.FP.Get_prefix(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessBoxing(node *Nodes_BoxingNode,_opt LnsAny) {
    opt := _opt.(*DumpNode_Opt)
    self.FP.dump(opt, &node.Nodes_Node, "")
    dumpNode_filter_1036_(node.FP.Get_src(), self, opt.FP.NextOpt())
}
...
func (self *dumpNode_dumpFilter) ProcessLiteralMap(node *Nodes_LiteralMapNode,_opt LnsAny) {
    ...
    for _, _pair := range( pairList.Items ) {
        ...
        dumpNode_filter_1036_(pair.FP.Get_key(), self, opt.FP.NextOpt())
        dumpNode_filter_1036_(pair.FP.Get_val(), self, opt.FP.NextOpt())
    }
}
    ...
    self.FP.dump(opt, &node.Nodes_Node, node.FP.Get_token().Txt)
    {
        ...
        if !Lns_IsNil( _expList ) {
            ...
            for _, _param := range( expList.FP.Get_expList().Items ) {
                ...
                dumpNode_filter_1036_(param, self, opt.FP.NextOpt())
            }
        }
    }
}
