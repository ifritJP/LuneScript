...
func Macro_loadCode_1007_(code string) LnsAny {
    ...
    if loaded != nil{
        loaded_5116 := loaded.(*Lns_luaValue)
        {
            _obj := Macro_convExp41(Lns_2DDD(Lns_getVM().RunLoadedfunc(loaded_5116,Lns_2DDD([]LnsAny{}))[0]))
            if _obj != nil {
                obj := _obj
                return obj
            }
    ...
    _map := _exp494.Val1
        ...
        for _key, _val := range( _map.Items ) {
            ...
            if keyObj != nil && valObj != nil{
                keyObj_5210 := keyObj
                valObj_5211 := valObj
                newMap.Set(keyObj_5210,valObj_5211)
            }
        }
        return newMap
    }
...
func (self *Macro_MacroCtrl) EvalMacroOp(streamName string,firstToken *Types_Token,macroTypeInfo *Ast_TypeInfo,expList LnsAny)(LnsAny, LnsAny) {
    ...
    if expList != nil{
        expList_5306 := expList.(*Nodes_ExpListNode)
        for _, _exp := range( expList_5306.FP.Get_expList().Items ) {
    ...
    if expList != nil{
        expList_5319 := expList.(*Nodes_ExpListNode)
        for _index, _argNode := range( expList_5319.FP.Get_expList().Items ) {
            index := _index + 1
            ...
            if literal != nil{
                literal_5326 := literal
                {
                    _val := Macro_getLiteralMacroVal_1122_(literal_5326)
                    if _val != nil {
                        val := _val
...
func (self *Macro_MacroCtrl) ImportMacro(processInfo *Ast_ProcessInfo,macroInfoStem LnsAny,macroTypeInfo *Ast_TypeInfo,typeId2TypeInfo *LnsMap) {
    ...
    if macroInfo != nil{
        macroInfo_5366 := macroInfo.(*Macro_MacroMetaInfo)
        var argList *LnsList
        ...
        for _, _argInfo := range( macroInfo_5366.ArgList.Items ) {
        ...
        for _, _symInfo := range( macroInfo_5366.SymList.Items ) {
        ...
        var column LnsInt
        column = 1
        for _, _tokenInfo := range( macroInfo_5366.TokenList.Items ) {
            ...
            if txt == "\n"{
                ...
                pos = NewTypes_Position(lineNo, column, Lns_getVM().String_format("macro:%s", []LnsAny{macroInfo_5366.Name}))
        ...
        self.typeId2MacroInfo.Set(macroTypeInfo.FP.Get_typeId(),&NewMacro_ExtMacroInfo(macroInfo_5366.Name, self.macroEval.FP.EvalFromCode(processInfo, macroInfo_5366.Name, argNameList, macroInfo_5366.StmtBlock), symbol2MacroValInfoMap, argList, tokenList).Nodes_MacroInfo)
    } else {
    ...
    if prefixToken.Txt == ",,"{
        {
            ...
            if _refNode != nil {
                ...
                if macroInfo != nil{
                    macroInfo_5527 := macroInfo.(*Nodes_MacroValInfo)
                    var valType *Ast_TypeInfo
                    valType = macroInfo_5527.TypeInfo
                    if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
