...
func Macro_loadCode_1007_(code string) LnsAny {
    ...
    if loaded != nil{
        loaded_5166 := loaded.(*Lns_luaValue)
        {
            _obj := Macro_convExp41(Lns_2DDD(Lns_getVM().RunLoadedfunc(loaded_5166,Lns_2DDD([]LnsAny{}))[0]))
            if _obj != nil {
                obj := _obj
                return obj
            }
    ...
    _map := _exp494.Val1
        ...
        for _key, _val := range( _map.Items ) {
            ...
            if keyObj != nil && valObj != nil{
                keyObj_5260 := keyObj
                valObj_5261 := valObj
                newMap.Set(keyObj_5260,valObj_5261)
            }
        }
        return newMap
    }
...
func (self *Macro_MacroCtrl) EvalMacroOp(streamName string,firstToken *Types_Token,macroTypeInfo *Ast_TypeInfo,expList LnsAny)(LnsAny, LnsAny) {
    ...
    if expList != nil{
        expList_5356 := expList.(*Nodes_ExpListNode)
        for _, _exp := range( expList_5356.FP.Get_expList().Items ) {
    ...
    if expList != nil{
        ...
        for _index, _argNode := range( expList_5369.FP.Get_expList().Items ) {
            index := _index + 1
            ...
            if literal != nil{
                literal_5376 := literal
                {
                    _val := Macro_getLiteralMacroVal_1122_(literal_5376)
                    if _val != nil {
                        val := _val
...
func (self *Macro_MacroCtrl) ImportMacro(processInfo *Ast_ProcessInfo,macroInfoStem LnsAny,macroTypeInfo *Ast_TypeInfo,typeId2TypeInfo *LnsMap) {
    ...
    if macroInfo != nil{
        macroInfo_5416 := macroInfo.(*Macro_MacroMetaInfo)
        var argList *LnsList
        ...
        for _, _argInfo := range( macroInfo_5416.ArgList.Items ) {
        ...
        for _, _symInfo := range( macroInfo_5416.SymList.Items ) {
        ...
        var column LnsInt
        column = 1
        for _, _tokenInfo := range( macroInfo_5416.TokenList.Items ) {
            ...
            if txt == "\n"{
                ...
                pos = NewTypes_Position(lineNo, column, Lns_getVM().String_format("macro:%s", []LnsAny{macroInfo_5416.Name}))
        ...
        self.typeId2MacroInfo.Set(macroTypeInfo.FP.Get_typeId(),&NewMacro_ExtMacroInfo(macroInfo_5416.Name, self.macroEval.FP.EvalFromCode(processInfo, macroInfo_5416.Name, argNameList, macroInfo_5416.StmtBlock), symbol2MacroValInfoMap, argList, tokenList).Nodes_MacroInfo)
    } else {
    ...
    if prefixToken.Txt == ",,"{
        {
            ...
            if _refNode != nil {
                ...
                if macroInfo != nil{
                    macroInfo_5577 := macroInfo.(*Nodes_MacroValInfo)
                    var valType *Ast_TypeInfo
                    valType = macroInfo_5577.TypeInfo
                    if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
