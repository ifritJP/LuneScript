...
// for 443
func Util_convExp0_1936(arg1 []LnsAny) string {
...
// for 419
func Util_convExp0_1803(arg1 []LnsAny) LnsAny {
...
// for 471
func Util_convExp0_2081(arg1 []LnsAny) string {
    return Lns_getFromMulti( arg1, 0 ).(string)
}
...
func Util_readFile(_env *LnsEnv, path string) LnsAny {
    ...
    {
        _fileObj := Util_convExp0_1803(Lns_2DDD(Lns_io_open(path, nil)))
        if _fileObj == nil{
            return nil
        } else {
...
func Util_scriptPath2Module(_env *LnsEnv, path string) string {
    ...
    var mod string
    mod = Util_convExp0_1866(Lns_2DDD(_env.GetVM().String_gsub(Lns_car(_env.GetVM().String_gsub(path,"^./", "")).(string),"/", ".")))
...
func Util_scriptPath2ModuleFromProjDir(_env *LnsEnv, path string,projDir LnsAny) string {
    if projDir != nil{
        ...
        path = Util_convExp0_1936(Lns_2DDD(_env.GetVM().String_gsub(path,"^" + workpath, "")))
    }
...
func Util_parentPath(_env *LnsEnv, path string) string {
    if Lns_isCondTrue( Lns_car(_env.GetVM().String_find(path,"/$", nil, nil))){
        path = Util_convExp0_2023(Lns_2DDD(_env.GetVM().String_gsub(path,"/$", "")))
    }
...
func Util_searchProjDir(_env *LnsEnv, dir string) LnsAny {
    ...
    for work != "" {
        ...
        var parent string
        parent = Util_convExp0_2081(Lns_2DDD(_env.GetVM().String_gsub(work,"/[^/]+$", "")))
        if parent == work{
            return nil
        }
        work = parent
